...
export default function CompleteProfile() {
  ...
  // ─── KEY FIX: Do NOT read email from localStorage or location.state ───
  // For Apply Now (manual flow): email must be empty so user fills it manually.
  // For Google flow: user comes via /complete-profile from the popup — 
  //   we read name from localStorage but still let email be editable.
  const stored   = (() => { try { return JSON.parse(localStorage.getItem("lms_user") || "{}"); } catch { return {}; } })();
  const isGoogle = location.state?.isGoogleUser || stored?.isNewUser || false;
  ...
  const [fd, setFd] = useState({
    // ─── FIX: name pre-filled only for Google users, email always empty for manual entry ───
    name: isGoogle ? (stored.name || location.state?.name || "") : "",
    email: isGoogle ? (stored.email || location.state?.email || "") : "",  // Empty for manual Apply Now
    password: "", confirmPassword: "", mobile: "",
    dob:"", gender:"", city:"", state:"", country:"",
    qualification:"", collegeName:"", yearOfPassing:"", currentStatus:"",
    agreeTerms: false, agreePrivacy: false,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ─── FIX: Clear isNewUser flag after successful registration ───
      ...
      localStorage.setItem("lms_user", JSON.stringify({
        ...
        email: fd.email,
        role,
        isNewUser: false,  // ← Clear so popup doesn't show again
      }));
      localStorage.setItem("role", role.toUpperCase());
      alert("✅ Application submitted!\n\nPlease verify your email. Admin will approve your account.");
      navigate(`/verify-email?email=${encodeURIComponent(fd.email)}`);
            ...
            {step === 2 && (
              <div className="cp-section">
                ...
                {isGoogle && (
                  <div className="cp-note">
                    <IInfo s={14}/>
                    Signed in with Google. Your name and email are pre-filled.
                  </div>
                )}
                  ...
                  {subPanel === "account" && <>
                    <div className="cp-row2">
                      ...
                      <div className="cp-field">
                        ...
                        {/* ─── FIX: Email is editable for manual users, disabled for Google users ─── */}
                        <input
                          className={inp}
                          type="email"
                          name="email"
                          ...
                          disabled={isGoogle}  // Only disabled for Google SSO users
                        />
                        {E("email")}
                      </div>
                    </div>
