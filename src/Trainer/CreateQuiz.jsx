...
const inputStyle = (t) => ({
  width: "100%", padding: "11px 14px", borderRadius: RADIUS.button,
  border: `1px solid ${t.border}`, background: t.pillBg, color: t.text,
  fontFamily: FONT_FAMILY, fontSize: 13, outline: "none", boxSizing: "border-box",
});
const numInputStyle = (t) => ({ ...inputStyle(t), height: 38, padding: "0 12px" });

function FieldLabel({ t, children }) {
  return (
    <div style={{ fontSize: 10.5, fontWeight: 700, color: t.textMuted, fontFamily: FONT_FAMILY, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 6 }}>
      {children}
    </div>
  );
}
function PrimaryButton({ color, children, ...rest }) {
  return (
    <button {...rest} style={{
      display: "inline-flex", alignItems: "center", gap: 7,
      padding: "10px 18px", borderRadius: RADIUS.button, border: "none",
      background: color, color: "#fff", fontFamily: FONT_FAMILY,
...
/* ═══════════════════════════════════════════════════════════
   REUSABLE CUSTOM DROPDOWN WITH INLINE ADD
═══════════════════════════════════════════════════════════ */
const AddableDropdown = ({ t, isDark, label, value, onChange, options, onAdd, placeholder = "Select…" }) => {
  const [open, setOpen] = useState(false);
  const [newVal, setNewVal] = useState("");
  const [error, setError] = useState("");
  const [openUp, setOpenUp] = useState(false);
  ...
  useEffect(() => {
    const handler = (e) => {
      if (wrapperRef.current && !wrapperRef.current.contains(e.target)) {
        setOpen(false); setNewVal(""); setError("");
      }
    };
    document.addEventListener("mousedown", handler);
    return () => document.removeEventListener("mousedown", handler);
  ...
  const handleAdd = () => {
    ...
    if (options.includes(trimmed)) { setError("Already exists"); return; }
    onAdd(trimmed);
    setNewVal(""); setError(""); setOpen(false);
  };

  const hoverBg = `${ACCENT_PURPLE.base}12`;
  const selBg = `${ACCENT_PURPLE.base}1c`;
  const addBg = `${ACCENT_PURPLE.base}0a`;
  const addBorder = `${ACCENT_PURPLE.base}33`;
    ...
    <div ref={wrapperRef} style={{ width: "100%" }}>
      <div style={{ display: "flex", gap: 6, alignItems: "center", position: "relative" }}>
        <button
          ...
          style={{
            flex: 1, display: "flex", alignItems: "center", justifyContent: "space-between",
            height: 38, padding: "0 12px", borderRadius: RADIUS.button,
            border: `1px solid ${open ? ACCENT_PURPLE.base : t.border}`,
            background: t.pillBg, color: value ? t.text : t.textMuted,
        ...
        {open && (
          <div style={{
            position: "absolute", ...panelPos, left: 0, right: 0,
            background: t.cardBg, border: `1px solid ${t.border}`,
            borderRadius: RADIUS.chip, boxShadow: t.shadow,
            zIndex: 1000, overflow: "hidden", display: "flex", flexDirection: "column",
          }}>
            <div style={{ maxHeight: 200, overflowY: "auto" }}>
              <div
                onClick={() => { onChange(""); setOpen(false); }}
                style={{ padding: "9px 14px", fontSize: 12, fontFamily: FONT_FAMILY, color: t.textMuted, cursor: "pointer", borderBottom: `1px solid ${t.border}` }}
              >
                {placeholder}
              </div>
              {options.map((opt) => {
                ...
                return (
                  <div key={opt}
                    onClick={() => { onChange(opt); setOpen(false); }}
                    style={{ padding: "10px 14px", fontSize: 13, fontFamily: FONT_FAMILY, color: sel ? ACCENT_PURPLE.base : t.text, fontWeight: sel ? 700 : 500, background: sel ? selBg : "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}
                    onMouseEnter={(e) => { if (!sel) e.currentTarget.style.background = hoverBg; }}
                    onMouseLeave={(e) => { if (!sel) e.currentTarget.style.background = "transparent"; }}
                  >
                    {opt}
                    {sel && <span style={{ width: 7, height: 7, borderRadius: "50%", background: ACCENT_PURPLE.base, flexShrink: 0 }} />}
                  </div>
                );
              })}
            </div>
            <div style={{ borderTop: `1px solid ${t.border}`, padding: "10px 12px", background: addBg }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 7 }}>
                <Plus size={12} color={ACCENT_PURPLE.base} />
                <span style={{ fontSize: 11, fontWeight: 700, fontFamily: FONT_FAMILY, color: ACCENT_PURPLE.base, letterSpacing: "0.02em" }}>
                  ADD NEW {label.toUpperCase()}
                </span>
              </div>
              <input
                ...
                onKeyDown={(e) => { if (e.key === "Enter") { e.stopPropagation(); handleAdd(); } if (e.key === "Escape") setOpen(false); }}
                placeholder={`Enter new ${label.toLowerCase()}`}
                style={{ width: "100%", boxSizing: "border-box", height: 32, padding: "0 10px", borderRadius: RADIUS.chip, border: `1px solid ${error ? "#f87171" : addBorder}`, background: t.cardBg, fontFamily: FONT_FAMILY, fontSize: 12, color: t.text, outline: "none", marginBottom: 7 }}
              />
              <button type="button" onClick={handleAdd}
                style={{ width: "100%", height: 30, borderRadius: RADIUS.chip, border: "none", cursor: "pointer", background: ACCENT_PURPLE.base, color: "#fff", fontFamily: FONT_FAMILY, fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                <Plus size={12} /> Add {label}
              </button>
              {error && <p style={{ margin: "5px 0 0", fontSize: 11, fontFamily: FONT_FAMILY, color: "#f87171" }}>{error}</p>}
            </div>
          </div>
        )}
      </div>
...
const CreateQuiz = () => {
  ...
  const panelMeta = {
    details: { label: "Quiz Details", sub: "Title, type, difficulty & settings", color: "#22d3ee", num: 1, nextLabel: "Next: Add Questions →" },
    questions: { label: "Questions", sub: `${questions.length} question${questions.length !== 1 ? "s" : ""} added`, color: ACCENT_PURPLE.base, num: 2, nextLabel: "Next: Review →" },
    review: { label: "Review & Publish", sub: "Confirm and save", color: "#34d399", num: 3, nextLabel: null },
  };
    ...
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      <style>{`
        * { box-sizing: border-box; }
        ...
        @media (max-width:700px) { .cq-grid-2 { grid-template-columns: 1fr !important; } .cq-grid-3 { grid-template-columns: 1fr !important; } .cq-opt-grid { grid-template-columns: 1fr !important; } }
      `}</style>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {PANELS.map((key) => {
          ...
          const isOpen = open === key;
          return (
            <div key={key} style={{ background: t.cardBg, border: `1px solid ${isOpen ? meta.color + "40" : t.border}`, borderRadius: RADIUS.standardCard, overflow: "hidden", boxShadow: t.shadow, transition: "all 0.2s" }}>
              <button onClick={() => toggle(key)}
                style={{ width: "100%", display: "flex", alignItems: "center", gap: 14, padding: "16px 20px", textAlign: "left", background: isOpen ? `${meta.color}08` : "transparent", border: "none", cursor: "pointer", borderBottom: isOpen ? `1px solid ${t.border}` : "none" }}>
                <div style={{ width: 32, height: 32, borderRadius: 999, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: `${meta.color}20`, border: `1px solid ${meta.color}40` }}>
                  <span style={{ fontSize: 12, fontWeight: 800, color: meta.color, fontFamily: FONT_FAMILY }}>{meta.num}</span>
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{meta.label}</p>
                  <p style={{ fontSize: 11, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>{meta.sub}</p>
                </div>
                <div style={{ display: "flex", gap: 4, marginRight: 8 }}>
                  {PANELS.map((p, pi) => (
                    <span key={p} style={{ width: p === key ? 18 : 6, height: 6, borderRadius: 999, background: p === key ? meta.color : pi < PANELS.indexOf(key) ? t.textMuted : t.border, transition: "all 0.3s" }} />
                  ))}
                </div>
                <ChevronDown size={16} color={t.textMuted} style={{ transform: isOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s", flexShrink: 0 }} />
              </button>
              ...
              {isOpen && (
                <div style={{ padding: "20px 20px 24px" }}>
                  ...
                  {key === "details" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div className="cq-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        <div>
                          <FieldLabel t={t}>Quiz Title</FieldLabel>
                          <input style={inputStyle(t)} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. React Fundamentals" />
                        </div>
                        <div>
                          <FieldLabel t={t}>Select Batch</FieldLabel>
                          <select style={inputStyle(t)} value={batchId} onChange={(e) => setBatchId(Number(e.target.value))}>
                            <option value="">Select Batch</option>
                            {batches.map((b) => <option key={b.id} value={b.id}>Batch {b.id}</option>)}
                          </select>
                        </div>
                      ...
                      <div className="cq-grid-3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
                        <div>
                          <FieldLabel t={t}>Quiz Type</FieldLabel>
                          <AddableDropdown t={t} isDark={dark} label="Quiz Type" placeholder="Select Type" value={quizType} onChange={setQuizType} options={quizTypes} onAdd={(v) => { setQuizTypes((p) => [...p, v]); setQuizType(v); }} />
                        </div>
                        <div>
                          <FieldLabel t={t}>Difficulty</FieldLabel>
                          <AddableDropdown t={t} isDark={dark} label="Difficulty" placeholder="Select Difficulty" value={difficulty} onChange={setDifficulty} options={difficulties} onAdd={(v) => { setDifficulties((p) => [...p, v]); setDifficulty(v); }} />
                        </div>
                        <div>
                          <FieldLabel t={t}>Category</FieldLabel>
                          <AddableDropdown t={t} isDark={dark} label="Category" placeholder="Select Category" value={category} onChange={setCategory} options={categories} onAdd={(v) => { setCategories((p) => [...p, v]); setCategory(v); }} />
                        </div>
                      </div>
                      ...
                      <div className="cq-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                        <div>
                          <FieldLabel t={t}>⏱ Time Limit (minutes)</FieldLabel>
                          <div style={{ position: "relative" }}>
                            <input type="number" min="1" value={timeLimit} onChange={(e) => setTimeLimit(e.target.value)} placeholder="e.g. 30" style={numInputStyle(t)} />
                            {timeLimit && <span style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", fontSize: 11, color: t.textMuted, fontFamily: FONT_FAMILY, pointerEvents: "none" }}>min</span>}
                          </div>
                        </div>
                        <div>
                          <FieldLabel t={t}>Total Marks</FieldLabel>
                          <input type="number" min="1" value={totalMarks} onChange={(e) => setTotalMarks(e.target.value)} placeholder="e.g. 100" style={numInputStyle(t)} />
                        </div>
                      </div>

                      <div>
                        <FieldLabel t={t}>Instructions</FieldLabel>
                        <textarea rows={3} style={{ ...inputStyle(t), resize: "vertical" }} value={instructions} onChange={(e) => setInstructions(e.target.value)} placeholder="Quiz instructions for students…" />
                      </div>
                      <div style={{ display: "flex", justifyContent: "flex-end" }}>
                        <PrimaryButton color={meta.color} onClick={() => setOpen("questions")}>{meta.nextLabel}</PrimaryButton>
                      </div>
                  ...
                  {key === "questions" && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
                        style={{ border: `2px dashed ${dragOver ? "#22d3ee" : uploadStatus === "success" ? "#34d399" : uploadStatus === "error" ? "#f87171" : `${ACCENT_PURPLE.base}55`}`, borderRadius: RADIUS.chip, padding: "20px 16px", background: dragOver ? "rgba(34,211,238,0.06)" : uploadStatus === "success" ? "rgba(52,211,153,0.06)" : uploadStatus === "error" ? "rgba(248,113,113,0.06)" : "transparent", transition: "all 0.2s", textAlign: "center" }}>
                        <input ref={fileInputRef} type="file" accept=".pdf,.doc,.docx,.txt,.csv" style={{ display: "none" }} onChange={(e) => handleFileChosen(e.target.files?.[0])} />
                        {uploading ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                            <Loader2 size={22} color={ACCENT_PURPLE.base} style={{ animation: "cq-spin 1s linear infinite" }} />
                            <p style={{ fontSize: 12, color: t.textMuted, margin: 0, fontFamily: FONT_FAMILY }}>Parsing quiz file with AI…</p>
                          </div>
                        ) : uploadedFile && uploadStatus === "success" ? (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <FileText size={16} color="#34d399" />
                              <span style={{ fontSize: 12, fontWeight: 600, color: "#34d399", fontFamily: FONT_FAMILY }}>{uploadMsg}</span>
                              <button onClick={clearUpload} style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}><X size={14} color={t.textMuted} /></button>
                            </div>
                            <button onClick={() => fileInputRef.current?.click()} style={{ fontSize: 11, color: t.textMuted, background: "none", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY, textDecoration: "underline" }}>Upload a different file</button>
                          </div>
                        ) : (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 40, height: 40, borderRadius: RADIUS.chip, background: `${ACCENT_PURPLE.base}18`, border: `1px solid ${ACCENT_PURPLE.base}30`, display: "flex", alignItems: "center", justifyContent: "center" }}><Upload size={18} color={ACCENT_PURPLE.base} /></div>
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 600, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>Upload Quiz File</p>
                              <p style={{ fontSize: 11, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY }}>Drag & drop or click to browse · .pdf .doc .docx .txt .csv</p>
                            </div>
                            {uploadStatus === "error" && <p style={{ fontSize: 11, color: "#f87171", margin: 0, fontFamily: FONT_FAMILY, fontWeight: 500 }}>{uploadMsg}</p>}
                            <button onClick={() => fileInputRef.current?.click()} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", borderRadius: RADIUS.chip, border: `1px solid ${ACCENT_PURPLE.base}55`, background: `${ACCENT_PURPLE.base}12`, color: ACCENT_PURPLE.base, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: FONT_FAMILY }}><Upload size={13} /> Choose File</button>
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        <div style={{ flex: 1, height: 1, background: t.border }} />
                        <span style={{ fontSize: 11, color: t.textMuted, fontFamily: FONT_FAMILY, whiteSpace: "nowrap" }}>or add questions manually</span>
                        <div style={{ flex: 1, height: 1, background: t.border }} />
                      </div>

                      {questions.map((q, index) => (
                        <div key={index} style={{ background: t.actBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, padding: 16 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
                            <div style={{ width: 24, height: 24, borderRadius: RADIUS.chip, background: `${ACCENT_PURPLE.base}18`, border: `1px solid ${ACCENT_PURPLE.base}30`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                              <span style={{ fontSize: 10, fontWeight: 800, color: ACCENT_PURPLE.base, fontFamily: FONT_FAMILY }}>{index + 1}</span>
                            </div>
                            <span style={{ fontSize: 11, fontWeight: 600, color: t.textMuted, textTransform: "uppercase", letterSpacing: "0.08em", fontFamily: FONT_FAMILY }}>Question {index + 1}</span>
                          </div>

                          <input style={{ ...inputStyle(t), marginBottom: 12 }} value={q.text}
                            onChange={(e) => { const u = [...questions]; u[index] = { ...u[index], text: e.target.value }; setQuestions(u); }}
                            placeholder="Enter question text" />

                          {(quizType === "Multiple Choice" || quizType === "") && (
                            <div className="cq-opt-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                              {["A", "B", "C", "D"].map((optKey) => (
                                <label key={optKey} style={{ display: "flex", alignItems: "center", gap: 10, borderRadius: RADIUS.chip, padding: "10px 12px", cursor: "pointer", border: `1px solid ${q.correctOption === optKey ? "#34d399" : t.border}`, background: q.correctOption === optKey ? "rgba(52,211,153,0.08)" : t.pillBg, transition: "all 0.15s" }}>
                                  <input type="radio" name={`correct-${index}`} checked={q.correctOption === optKey}
                                    onChange={() => { const u = [...questions]; u[index] = { ...u[index], correctOption: optKey }; setQuestions(u); }}
                                    style={{ accentColor: "#34d399" }} />
                                  <span style={{ width: 20, height: 20, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: q.correctOption === optKey ? "#34d399" : t.barBg, fontSize: 10, fontWeight: 800, color: q.correctOption === optKey ? "#fff" : t.textMuted, fontFamily: FONT_FAMILY }}>{optKey}</span>
                                  <input value={q.options[optKey] || ""}
                                    onChange={(e) => { const u = [...questions]; u[index] = { ...u[index], options: { ...u[index].options, [optKey]: e.target.value } }; setQuestions(u); }}
                                    placeholder={`Option ${optKey}`}
                                    style={{ flex: 1, background: "transparent", outline: "none", border: "none", fontSize: 13, color: t.text, fontFamily: FONT_FAMILY }} />
                                </label>
                              ))}
                            </div>
                          )}
                              ...
                              {["A", "B"].map((optKey) => (
                                <label key={optKey} style={{ display: "flex", alignItems: "center", gap: 10, borderRadius: RADIUS.chip, padding: "10px 12px", cursor: "pointer", border: `1px solid ${q.correctOption === optKey ? "#34d399" : t.border}`, background: q.correctOption === optKey ? "rgba(52,211,153,0.08)" : t.pillBg, transition: "all 0.15s" }}>
                                  <input type="radio" name={`correct-${index}`} checked={q.correctOption === optKey}
                                    onChange={() => { const u = [...questions]; u[index] = { ...u[index], correctOption: optKey }; setQuestions(u); }}
                                    style={{ accentColor: "#34d399" }} />
                                  <span style={{ width: 20, height: 20, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: q.correctOption === optKey ? "#34d399" : t.barBg, fontSize: 10, fontWeight: 800, color: q.correctOption === optKey ? "#fff" : t.textMuted, fontFamily: FONT_FAMILY }}>
                                    {optKey === "A" ? "T" : "F"}
                                  </span>
                                  <span style={{ fontSize: 13, fontWeight: 600, color: t.text, fontFamily: FONT_FAMILY }}>
                                    {optKey === "A" ? "True" : "False"}
                                  </span>
                                </label>
                              ))}
                          ...
                          {(quizType === "Fill in the Blank" || quizType === "Short Answer") && (
                            <div>
                              <p style={{ fontSize: 11, color: t.textMuted, fontFamily: FONT_FAMILY, margin: "0 0 6px 0" }}>
                                {quizType === "Fill in the Blank" ? "Correct Answer (blank)" : "Model Answer"}
                              </p>
                              <input style={inputStyle(t)} value={q.answer || ""}
                                onChange={(e) => { const u = [...questions]; u[index] = { ...u[index], answer: e.target.value }; setQuestions(u); }}
                                placeholder={quizType === "Fill in the Blank" ? "Enter the correct word/phrase" : "Enter the expected answer"} />
                            </div>
                          )}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <button onClick={addNewQuestion} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 16px", borderRadius: RADIUS.chip, border: `2px dashed ${ACCENT_PURPLE.base}55`, background: "transparent", color: ACCENT_PURPLE.base, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: FONT_FAMILY }}>
                          <PlusCircle size={14} /> Add Question
                        </button>
                        <PrimaryButton color={meta.color} onClick={() => setOpen("review")}>{meta.nextLabel}</PrimaryButton>
                      </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 12 }}>
                        {[
                          ...
                          <div key={label} style={{ background: t.actBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, padding: "12px 16px" }}>
                            <p style={{ fontSize: 10, color: t.textMuted, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", margin: 0, fontFamily: FONT_FAMILY }}>{label}</p>
                            <p style={{ fontSize: 14, fontWeight: 700, color: t.text, margin: "4px 0 0", fontFamily: FONT_FAMILY, wordBreak: "break-word" }}>{val}</p>
                          </div>
                        ))}
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <button onClick={() => setOpen("questions")} style={{ fontSize: 12, color: t.textMuted, background: "none", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY, fontWeight: 500 }}>← Back to Questions</button>
                        <PrimaryButton color="#34d399" onClick={handleSaveQuiz} disabled={loading}>
                          {loading ? <><Loader2 size={14} style={{ animation: "cq-spin 1s linear infinite" }} /> Saving…</> : <><CheckCircle2 size={14} /> Publish Quiz</>}
                        </PrimaryButton>
                      </div>
                    </div>
                  )}
