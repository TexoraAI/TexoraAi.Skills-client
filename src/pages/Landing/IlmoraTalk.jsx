...
import {
  MessageSquare, Mic, Briefcase, Building2, Target,
  ArrowRight, CheckCircle2, Star, Users, Award, TrendingUp,
  ChevronDown, Zap, Globe, Clock, BookOpen,
} from "lucide-react";

// ✅ Shared shell used by every other public page (Careers, Platforms,
// About, Pricing, Contact, FAQ, etc). It renders the ONE global
// AnnouncementBanner → Navbar (with "All Courses" mega menu, Mentors,
// Success Stories, School Programs) → ... → Footer for the whole site.
// Adjust this relative path if IlmoraTalk.jsx lives somewhere other
// than the same folder as Careers.jsx / Platforms.jsx.
...
/* ─── Page-scoped CSS ───────────────────────────────────────────────────
   ✅ ALIGNMENT FIX: previously this file's own bare `*,*::before,*::after
   { margin:0;padding:0; }` reset, plus the page's own Navbar (with its
   `.nav-lnk` / `html.dark` rules), leaked outside this component and
   could strip margin/padding or override colors on PublicLayout's shared
   navbar/footer on OTHER pages too. Everything is now scoped under
   `.ilmt-page`, and the custom Navbar has been removed entirely — this
   page now gets the exact same shared navbar/footer/banner as Careers
   and Platforms, with identical alignment.
──────────────────────────────────────────────────────────────────────── */
const GLOBAL = `
  .ilmt-page, .ilmt-page *, .ilmt-page *::before, .ilmt-page *::after{box-sizing:border-box;}
`;
...
/* ─── MAIN ──────────────────────────────────────────────────────────────
   ✅ Custom Navbar (MegaMenu + Sheet mobile drawer + theme toggle button)
   removed. This page is now rendered INSIDE PublicLayout, so it gets the
   exact same AnnouncementBanner → Navbar → ... → Footer shell (and
   identical alignment) as the Careers and Platforms pages. `theme` is
   now taken from PublicLayout's shared toggle instead of local state.
   Pass these props from your route the same way you already do for
   Careers / Platforms, e.g.:
     <Route path="/ilmora-talk" element={
       <IlmOraTalk theme={theme} toggleTheme={toggleTheme}
                   setShowLoginModal={setShowLoginModal}
                   scrollToSection={scrollToSection} />
     } />
──────────────────────────────────────────────────────────────────────── */
export default function IlmOraTalk({ theme, toggleTheme, setShowLoginModal, scrollToSection }) {
  const navigate = useNavigate();
  const [faq,    setFaq]    = useState(null);
  const [active, setActive] = useState(0);
  ...
  return (
    ...
    >
      <style>{GLOBAL}</style>
      <div className="ilmt-page" style={{ fontFamily:F, background:BG, minHeight:"100vh", color:TP, transition:"background .3s,color .3s" }}>
