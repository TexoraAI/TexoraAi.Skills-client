...
// spacing, radius, PageContainer and Hero. This page must not
// redeclare tokens that already live there (see AdminDashboard.jsx,
// the Golden Reference, which this page now visually matches).
import {
  T,
  FONT_FAMILY,
  FONT_WEIGHT,
...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   RecentRow / EmptyBlock are page-local but token-driven.
───────────────────────────────────────────────────────────────────────── */
...
function SectionHeader({ t, icon: Icon, color, title, right }) {
  return (
...
function Pill({ t, children, color }) {
  if (color) {
    return (
      <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", padding: "4px 10px", borderRadius: RADIUS.pill, fontFamily: FONT_FAMILY, background: `${color}15`, border: `1px solid ${color}30`, color }}>
        {children}
      </span>
    );
  }
  return (
    <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", padding: "4px 10px", borderRadius: RADIUS.pill, background: t.pillBg, border: `1px solid ${t.pillBorder}`, color: t.pillText, fontFamily: FONT_FAMILY }}>
      {children}
    </span>
  );
}
  ...
  return (
    <button
      ...
      style={{
        position: "relative", width: 40, height: 22, borderRadius: 11,
        border: "none", cursor: "pointer", flexShrink: 0,
        background: checked ? ACCENT_PURPLE.base : t.pillBorder,
        transition: "background 0.25s",
      }}
    >
      <span style={{ position: "absolute", top: 3, width: 16, height: 16, borderRadius: "50%", background: "#fff", boxShadow: "0 1px 4px rgba(0,0,0,0.3)", left: checked ? 21 : 3, transition: "left 0.2s" }} />
    </button>
  );
}
...
function StatusBadge({ status }) {
  ...
  const s = map[status] || map.SUBMITTED;
  return (
    <span style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", padding: "3px 9px", borderRadius: RADIUS.pill, fontFamily: FONT_FAMILY, background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
      {status}
    </span>
  );
}
...
function RatingBar({ label, val, t }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
      <span style={{ fontSize: 11, color: t.textMuted, width: 130, flexShrink: 0, fontFamily: FONT_FAMILY }}>{label}</span>
      <div style={{ flex: 1, height: 6, borderRadius: 4, background: t.barBg, overflow: "hidden" }}>
        <div style={{ width: `${(val / 5) * 100}%`, height: 6, borderRadius: 4, background: color, boxShadow: `0 0 8px ${color}60`, transition: "width 0.6s ease" }} />
      </div>
      <span style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color, width: 36, textAlign: "right", fontFamily: FONT_FAMILY }}>{val}/5</span>
    </div>
  );
}

/* ─── Stat Card — page-local, token-driven (mirrors the shared StatCard's
   visuals for the string/percent-valued stats this page needs) ─── */
function FeedbackStatCard({ label, value, delta, up, fillPct, color, icon: Icon, t }) {
  const [hov, setHov] = useState(false);
  return (
    <div
      onMouseEnter={() => setHov(true)}
    ...
    >
      <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: color, filter: "blur(36px)", opacity: hov ? 0.18 : 0.05, transition: "opacity 0.4s", pointerEvents: "none" }} />
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <IconBadge icon={Icon} color={color} size={38} iconSize={17} />
        <span style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, textTransform: "uppercase", padding: "3px 9px", borderRadius: RADIUS.pill, background: t.pillBg, border: `1px solid ${t.pillBorder}`, color: t.pillText, fontFamily: FONT_FAMILY }}>Live</span>
      </div>
      <div>
        <p style={{ fontSize: 34, fontWeight: FONT_WEIGHT.heroTitle, lineHeight: 1, fontFamily: FONT_FAMILY, color: t.text, margin: 0 }}>{value}</p>
        <p style={{ fontSize: 10, marginTop: 4, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, textTransform: "uppercase", color: t.textMuted, fontFamily: FONT_FAMILY, margin: "5px 0 0" }}>{label}</p>
      </div>
      <div>
        <p style={{ fontSize: 10, color: up === true ? t.liveText : up === false ? "#f87171" : t.textMuted, fontFamily: FONT_FAMILY, margin: "0 0 8px", fontWeight: FONT_WEIGHT.medium }}>{delta}</p>
        <div style={{ height: 2, background: t.barBg, borderRadius: 99, overflow: "hidden" }}>
          <div style={{ height: "100%", borderRadius: 99, background: color, width: hov ? `${Math.min(fillPct + 20, 100)}%` : `${fillPct}%`, transition: "width 0.65s ease", opacity: 0.85 }} />
        </div>
      </div>
...
function TrainerRow({ trainer, index, onClick, t }) {
  ...
  return (
    <div
      ...
      style={{
        display: "flex", alignItems: "center", gap: 10,
        padding: "9px 10px", borderRadius: RADIUS.chip,
        background: hov ? t.recentItemBgHov : t.recentItemBg,
        border: `1px solid ${hov ? t.recentItemBorder : "transparent"}`,
        transition: "all 0.15s", cursor: "pointer", marginBottom: 6,
      }}
    >
      <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.heroTitle, color: rankColors[index] || t.textMuted, width: 20, fontFamily: FONT_FAMILY, flexShrink: 0 }}>#{index + 1}</span>
      <div style={{ width: 34, height: 34, borderRadius: RADIUS.chip, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: FONT_WEIGHT.bold, color: "#fff", flexShrink: 0, background: GRAD[index] || GRAD[3] }}>
        {trainer.name?.charAt(0) || "?"}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{trainer.name}</p>
        <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>{trainer.total} reviews</p>
      </div>
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        <p style={{ fontSize: 16, fontWeight: FONT_WEIGHT.heroTitle, color: parseFloat(trainer.rating) >= 4 ? t.liveText : "#f59e0b", margin: 0, fontFamily: FONT_FAMILY }}>{trainer.rating}</p>
        <p style={{ fontSize: 9, color: t.textMuted, margin: "1px 0 0", fontFamily: FONT_FAMILY }}>avg</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div
      ...
      style={{
        display: "grid", gridTemplateColumns: "40px 1fr 1fr 1fr 80px 110px 70px 80px",
        alignItems: "center", gap: 8,
        padding: "11px 16px", borderRadius: RADIUS.chip,
        background: hov ? t.recentItemBgHov : "transparent",
        border: `1px solid ${hov ? t.recentItemBorder : "transparent"}`,
        transition: "all 0.15s",
      }}
    >
      <span style={{ fontSize: 18 }}>{f.mood}</span>
      <p style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{f.batch}</p>
      <p style={{ fontSize: 11, color: t.textSub, margin: 0, fontFamily: FONT_FAMILY }}>{f.trainer}</p>
      <p style={{ fontSize: 11, color: t.textSub, margin: 0, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.student}</p>
      <p style={{ fontSize: 16, fontWeight: FONT_WEIGHT.heroTitle, color: ac, margin: 0, fontFamily: FONT_FAMILY }}>{avg(f)}</p>
      <div><StatusBadge status={f.status} /></div>
      <p style={{ fontSize: 10, color: t.textMuted, margin: 0, fontFamily: FONT_FAMILY }}>{f.date}</p>
      <button
        onClick={() => onView(f)}
        style={{
          padding: "5px 12px", borderRadius: RADIUS.button,
          border: `1px solid ${t.border}`, background: t.pillBg,
          color: t.textSub, fontSize: 10, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer",
          fontFamily: FONT_FAMILY, transition: "all 0.15s",
        }}
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = `${ACCENT_PURPLE.base}55`; e.currentTarget.style.color = ACCENT_PURPLE.base; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = t.border; e.currentTarget.style.color = t.textSub; }}
      >
        View →
      </button>
...
/* ─── Modal — page-local, styled from shared tokens ─── */
function Modal({ t, title, onClose, children, maxWidth = 560 }) {
  return (
    <div
      onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.7)", backdropFilter: "blur(12px)" }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.standardCard, padding: 24, maxWidth, width: "92%", maxHeight: "90vh", overflowY: "auto", boxShadow: t.shadowHov }}
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <h3 style={{ fontSize: 14, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{title}</h3>
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: RADIUS.chip, border: `1px solid ${t.border}`, background: t.actBg, color: t.textMuted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <X size={14} />
          </button>
        </div>
        {children}
...
══════════════════════════════════════════════════════ */
export default function AdminFeedback() {
  const [isDark, setIsDark] = useState(isDarkDoc);
  useEffect(() => {
    setIsDark(isDarkDoc());
    const obs = new MutationObserver(() => setIsDark(isDarkDoc()));
    obs.observe(document.documentElement, { attributes: true, attributeFilter: ["class", "data-theme"] });
    return () => obs.disconnect();
  }, []);
  const t = isDark ? T.dark : T.light;
  ...
  ).map((tr) => ({ ...tr, rating: (tr.sum / tr.total).toFixed(1) })).sort((a, b) => b.rating - a.rating).slice(0, 4);
  ...
  const inp = {
    ...
    color: t.text, borderRadius: RADIUS.button, padding: "9px 12px",
    fontSize: 12, fontFamily: FONT_FAMILY, outline: "none",
    width: "100%", boxSizing: "border-box",
  };
  ...
  const pill = {
    ...
    textTransform: "uppercase", padding: "4px 10px", borderRadius: RADIUS.pill,
    background: t.pillBg, border: `1px solid ${t.pillBorder}`, color: t.pillText, fontFamily: FONT_FAMILY,
  };
    ...
    <PageContainer mode={isDark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      <style>{`
        @media (max-width:560px){
          .feedback-hero-badges{width:100%;}
        }
        @media (max-width:768px){
          .feedback-analytics-grid{grid-template-columns:1fr !important;}
          .feedback-table-row{grid-template-columns:32px 1fr 1fr 70px 70px !important;}
          .feedback-col-trainer,.feedback-col-status{display:none;}
        }
        @media (max-width:480px){
          .feedback-table-row{grid-template-columns:28px 1fr 60px 60px !important;}
          .feedback-col-student{display:none;}
        }
      `}</style>
      ...
      <Hero borderHero={t.borderHero}>
        <div>
          ...
          <p style={{ fontSize: FONT_SIZE.bodySmall, color: t.textSub, margin: "0 0 12px", fontWeight: FONT_WEIGHT.medium, fontFamily: FONT_FAMILY }}>
            Platform-wide feedback intelligence · Real-time insights
          </p>

          {/* Batch selector + quick actions */}
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, background: t.heroStatBg, border: `1px solid ${t.heroStatBorder}`, borderRadius: RADIUS.button, padding: "6px 12px" }}>
              <Layers size={11} color={t.heroActIcon} />
              <select
                value={selectedBatchId ?? ""}
                onChange={(e) => setSelectedBatchId(Number(e.target.value))}
                style={{ background: "transparent", border: "none", color: t.text, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY, outline: "none" }}
              >
                {batches.length === 0 && <option value="">No batches</option>}
                {batches.map((b) => <option key={b.id} value={b.id}>{b.name || `Batch #${b.id}`}</option>)}
              </select>
            ...
            <button
              onClick={() => {}}
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 14px", borderRadius: RADIUS.button, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, border: `1px solid ${t.heroBtnBorder}`, background: t.heroBtnBg, color: t.heroBtnColor, cursor: "pointer", fontFamily: FONT_FAMILY }}
            >
              <Download size={11} /> Export CSV
            </button>
            <button
              onClick={openAlertModal}
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 14px", borderRadius: RADIUS.button, fontSize: 11, fontWeight: FONT_WEIGHT.bold, border: "none", background: `linear-gradient(135deg,${ACCENT_PURPLE.base},${ACCENT_PURPLE.to || ACCENT_PURPLE.base})`, color: "#fff", cursor: "pointer", fontFamily: FONT_FAMILY, boxShadow: `0 3px 10px ${ACCENT_PURPLE.base}66` }}
            >
              <Bell size={11} /> Configure Alerts
            </button>
          </div>
        ...
        <div className="hero-badges feedback-hero-badges">
          <div style={{ display: "flex", alignItems: "center", gap: 12, background: t.actBg, border: `1px solid ${t.actBorder}`, borderRadius: RADIUS.chip, padding: "8px 16px", fontSize: 11, fontWeight: FONT_WEIGHT.semibold, fontFamily: FONT_FAMILY, color: t.textSub, flexWrap: "wrap" }}>
            <span>{feedbackList.length} entries</span>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.3)", borderRadius: RADIUS.pill, padding: "8px 18px", color: ACCENT_PURPLE.base, fontSize: 11, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, fontFamily: FONT_FAMILY }}>
      ...
      {/* ═══ STAT CARDS — shared .stat-grid layout ═══ */}
      <div className="stat-grid" style={{ marginBottom: 20 }}>
        {statsData.map((s, i) => (
          <FeedbackStatCard key={i} {...s} t={t} />
        ))}
      </div>

      {/* ═══ ANALYTICS ROW ═══ */}
      <div className="dash-row-grid feedback-analytics-grid" style={{ marginBottom: 14 }}>
        {/* Trainer Leaderboard */}
        <SectionCard t={t}>
          <SectionHeader t={t} icon={Star} color="#f59e0b" title="Trainer Leaderboard" right={<span style={pill}>This Month</span>} />
          {trainerStats.length === 0 ? (
        ...
        {/* Batch Performance */}
        <SectionCard t={t}>
          <SectionHeader t={t} icon={BarChart3} color="#22d3ee" title="Batch Performance" right={<span style={pill}>Platform</span>} />
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {batchPerformance.map((b) => (
              <div key={b.name}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                  <p style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{b.name}</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY }}>{b.count} reviews</span>
                    <Pill t={t} color={b.rating >= 4 ? "#34d399" : b.rating >= 3 ? "#f59e0b" : "#f87171"}>{b.rating}</Pill>
                  </div>
                </div>
                <div style={{ height: 5, background: t.barBg, borderRadius: 99, overflow: "hidden" }}>
        ...
        {/* Top Tags */}
        <SectionCard t={t}>
          <SectionHeader t={t} icon={Tag} color="#a78bfa" title="Top Feedback Tags" right={<span style={pill}>All time</span>} />
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
            {(tagStats.length > 0 ? tagStats : [
              { text: "Just right ×24", count: 24 }, { text: "Great demos ×18", count: 18 },
              { text: "Too fast ×12", count: 12 }, { text: "Notes & resources ×8", count: 8 },
              ...
              const c = TAG_COLORS[i % TAG_COLORS.length];
              return (
                <span key={tag.text} style={{ fontSize: 10, fontWeight: FONT_WEIGHT.semibold, padding: "4px 10px", borderRadius: RADIUS.pill, background: c.bg, color: c.color, border: `1px solid ${c.border}`, fontFamily: FONT_FAMILY }}>
                  {tag.text}
                </span>
              );
            })}
          ...
          <div style={{ borderTop: `1px solid ${t.border}`, paddingTop: 14 }}>
            <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, color: t.textLabel, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, fontFamily: FONT_FAMILY, margin: "0 0 10px" }}>Improvement Requests</p>
            {[
              ["Recorded sessions", "+12%", "#f59e0b"],
              ["Weekend doubt sessions", "+8%", ACCENT_PURPLE.base],
              ["Better notes", "+5%", ACCENT_PURPLE.base],
            ].map(([label, val, color]) => (
              <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 7 }}>
                <span style={{ fontSize: 11, color: t.textSub, fontFamily: FONT_FAMILY }}>{label}</span>
                <Pill t={t} color={color}>{val}</Pill>
              </div>
            ))}
          </div>
      ...
      <SectionCard t={t} style={{ padding: 0, overflow: "hidden", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 20px", borderBottom: `1px solid ${t.border}`, flexWrap: "wrap", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <IconBadge icon={Filter} color={ACCENT_PURPLE.base} />
            <span style={{ fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.bold, fontSize: 13, color: t.text }}>All Feedback</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            {/* Search */}
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {[["all", "All"], ["new", "New"], ["reviewed", "Reviewed"], ["flag", "⚠ Low"]].map(([v, l]) => (
                <button
                  ...
                  onClick={() => setFilter(v)}
                  style={{
                    padding: "6px 14px", borderRadius: RADIUS.button, fontSize: 10, fontWeight: FONT_WEIGHT.bold,
                    cursor: "pointer", fontFamily: FONT_FAMILY, transition: "all 0.15s",
                    border: filter === v ? `1px solid ${ACCENT_PURPLE.base}66` : `1px solid ${t.border}`,
                    background: filter === v ? `${ACCENT_PURPLE.base}1f` : "transparent",
                    color: filter === v ? ACCENT_PURPLE.base : t.textMuted,
                  }}
                >
                  {l}
                </button>
        ...
        {/* Table column headers */}
        <div className="feedback-table-row" style={{ display: "grid", gridTemplateColumns: "40px 1fr 1fr 1fr 80px 110px 70px 80px", gap: 8, padding: "10px 16px", borderBottom: `1px solid ${t.border}` }}>
          {["Mood", "Batch", "Trainer", "Student", "Avg", "Status", "Date", ""].map((h) => (
            <p key={h} className={h === "Trainer" ? "feedback-col-trainer" : h === "Status" ? "feedback-col-status" : h === "Student" ? "feedback-col-student" : undefined} style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", color: t.textLabel, fontFamily: FONT_FAMILY, margin: 0 }}>
              {h}
            </p>
          ))}
        </div>
        ...
        <div style={{ padding: "8px 4px" }}>
          {loadingFeedback ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 0", gap: 12 }}>
              ...
              <p style={{ fontSize: 12, color: t.textMuted, fontFamily: FONT_FAMILY, margin: 0, fontWeight: FONT_WEIGHT.medium }}>No feedback found</p>
            </div>
          ) : (
            filtered.map((f) => (
              <FeedbackRow key={f.id} f={f} onView={(fb) => { setSelected(fb); setActiveTab("overview"); }} t={t} />
      ...
      {/* ═══ QUICK ACTIONS BAR ═══ */}
      <SectionCard t={t} style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10, padding: "14px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginRight: 10 }}>
          <Zap size={14} color={t.textLabel} />
          <span style={{ fontSize: 10, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, textTransform: "uppercase", color: t.textLabel, fontFamily: FONT_FAMILY }}>Quick Actions</span>
        </div>
        {[
          { label: "Export Report", color: "#22d3ee" },
          { label: "Configure Alerts", color: ACCENT_PURPLE.base, action: openAlertModal },
          { label: "View Trainers", color: "#34d399" },
          { label: "Batch Summary", color: "#f59e0b" },
          { label: "Flag Low Ratings", color: "#f87171", action: () => setFilter("flag") },
        ].map((a) => (
          <QuickActionButton key={a.label} action={a} t={t} />
        ))}
      </SectionCard>
      ...
      {selected && (
        <Modal t={t} title={`Feedback #${selected.id} — ${selected.batch}`} onClose={() => setSelected(null)} maxWidth={560}>
          {/* Tabs */}
          <div style={{ display: "flex", gap: 4, marginBottom: 20, background: t.actBg, border: `1px solid ${t.actBorder}`, borderRadius: RADIUS.chip, padding: 4 }}>
            {["overview", "ratings", "comment"].map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                style={{
                  flex: 1, padding: "8px", borderRadius: RADIUS.button, fontSize: 11, fontWeight: FONT_WEIGHT.bold,
                  cursor: "pointer", border: "none", transition: "all 0.2s", textTransform: "capitalize",
                  background: activeTab === tab ? `linear-gradient(135deg,${ACCENT_PURPLE.base},${ACCENT_PURPLE.to || ACCENT_PURPLE.base})` : "transparent",
                  color: activeTab === tab ? "#fff" : t.textSub,
                  boxShadow: activeTab === tab ? `0 3px 10px ${ACCENT_PURPLE.base}66` : "none",
          ...
          {activeTab === "overview" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[
                ...
                <div key={l} style={{ background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, borderRadius: RADIUS.chip, padding: "12px 14px" }}>
                  <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 4px" }}>{l}</p>
                  <p style={{ fontSize: 13, fontWeight: FONT_WEIGHT.semibold, color: t.text, fontFamily: FONT_FAMILY, margin: 0 }}>{v}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {activeTab === "ratings" && (
            <div>
              ...
              <RatingBar label="Energy & engagement" val={selected.energy} t={t} />
              <RatingBar label="Technical depth" val={selected.depth} t={t} />
              <div style={{ marginTop: 16, padding: 14, background: t.recentItemBg, borderRadius: RADIUS.chip, border: `1px solid ${t.recentItemBorder}`, textAlign: "center" }}>
                <p style={{ fontSize: 28, fontWeight: FONT_WEIGHT.heroTitle, color: ratingColor(parseFloat(avg(selected))), margin: 0, fontFamily: FONT_FAMILY }}>{avg(selected)}</p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY }}>Overall average</p>
              </div>
            </div>
          )}
          ...
          {activeTab === "comment" && (
            <div>
              <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 10px" }}>Tags</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 18 }}>
                {(selected.tags || []).map((tag, i) => {
                  const c = TAG_COLORS[i % TAG_COLORS.length];
                  return <span key={tag} style={{ fontSize: 10, fontWeight: FONT_WEIGHT.semibold, padding: "4px 12px", borderRadius: RADIUS.pill, background: c.bg, color: c.color, border: `1px solid ${c.border}`, fontFamily: FONT_FAMILY }}>{tag}</span>;
                })}
              </div>
              <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 10px" }}>Comment</p>
              {selected.comment
                ? <div style={{ background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, borderRadius: RADIUS.chip, padding: 14, fontSize: 12, color: t.textSub, fontFamily: FONT_FAMILY, lineHeight: 1.7, fontStyle: "italic" }}>"{selected.comment}"</div>
                : <p style={{ fontSize: 12, color: t.textMuted, fontFamily: FONT_FAMILY }}>No comment provided.</p>
              }
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
            {[
              ...
              <button
                ...
                onClick={action}
                style={{
                  flex: 1, padding: "10px", borderRadius: RADIUS.button, fontSize: 11, fontWeight: FONT_WEIGHT.bold,
                  cursor: "pointer", fontFamily: FONT_FAMILY, transition: "all 0.2s",
                  display: "flex", alignItems: "center", justifyContent: "center", gap: 6,
                  background: outline ? "transparent" : `${color}15`,
                  border: `1px solid ${color}40`,
                  color,
        ...
        <Modal t={t} title={trainerModal.name} onClose={() => setTrainerModal(null)} maxWidth={480}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 20 }}>
            {[["Total Reviews", trainerModal.total], ["Avg Rating", `${trainerModal.rating} / 5.0`], ["Active Batches", "2"], ["Rank", "#1"]].map(([l, v]) => (
              <div key={l} style={{ background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, borderRadius: RADIUS.chip, padding: "12px 14px" }}>
                <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 4px" }}>{l}</p>
                <p style={{ fontSize: 14, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY, margin: 0 }}>{v}</p>
              </div>
            ))}
          </div>
          <RatingBar label="Clarity" val={4} t={t} />
          ...
          <RatingBar label="Technical depth" val={4} t={t} />
          <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
            <button onClick={() => setTrainerModal(null)} style={{ flex: 1, padding: "10px", borderRadius: RADIUS.button, border: "none", background: `linear-gradient(135deg,${ACCENT_PURPLE.base},${ACCENT_PURPLE.to || ACCENT_PURPLE.base})`, color: "#fff", fontSize: 11, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY }}>Send Appreciation</button>
            <button onClick={() => setTrainerModal(null)} style={{ flex: 1, padding: "10px", borderRadius: RADIUS.button, border: `1px solid ${t.border}`, background: t.actBg, color: t.textSub, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>View All Sessions</button>
          </div>
        </Modal>
      )}
      ...
      {alertModal && (
        <Modal t={t} title="Configure Alerts" onClose={() => setAlertModal(false)} maxWidth={560}>
          <p style={{ fontSize: 11, color: t.textMuted, fontFamily: FONT_FAMILY, margin: "-10px 0 16px" }}>{selectedBatchName}</p>

          {alertSaveMsg && (
            <div style={{
              padding: "10px 14px", borderRadius: RADIUS.chip, marginBottom: 16, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, fontFamily: FONT_FAMILY, textAlign: "center",
              background: alertSaveMsg.type === "success" ? "rgba(52,211,153,0.1)" : "rgba(248,113,113,0.1)",
              border: `1px solid ${alertSaveMsg.type === "success" ? "rgba(52,211,153,0.3)" : "rgba(248,113,113,0.3)"}`,
              color: alertSaveMsg.type === "success" ? t.liveText : "#f87171",
            }}>{alertSaveMsg.text}</div>
          )}

          {alertLoading ? (
            <div style={{ padding: "20px 0", display: "flex", flexDirection: "column", gap: 12 }}>{[1, 2, 3].map((i) => <SkeletonRow key={i} t={t} />)}</div>
          ) : (
            <>
              {/* Trainer email */}
              <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrowWide, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 8px" }}>Trainer Email</p>
              <input
                value={alertConfig.trainerEmail}
                onChange={(e) => setAlertConfig((p) => ({ ...p, trainerEmail: e.target.value }))}
                placeholder="trainer@example.com"
              ...
              {/* Recipients */}
              <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrowWide, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "0 0 10px" }}>Notify Recipients</p>
              {recipientRows.map(({ key, label, sub, msgKey, icon: Icon, color }) => (
                <div key={key} style={{ marginBottom: 10 }}>
                  <div style={{
                    display: "flex", alignItems: "center", justifyContent: "space-between",
                    padding: "11px 14px", borderRadius: RADIUS.chip,
                    background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`,
                    marginBottom: alertConfig[key] ? 6 : 0,
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <IconBadge icon={Icon} color={color} size={30} iconSize={13} />
                      <div>
                        <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{label}</p>
              ...
              {/* Low rating threshold */}
              <p style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrowWide, color: t.textLabel, fontFamily: FONT_FAMILY, margin: "16px 0 10px" }}>Low Rating Alerts</p>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 14px", borderRadius: RADIUS.chip, background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, marginBottom: 8 }}>
                <div>
                  <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>Alert on Low Ratings</p>
                  <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>Trigger alert when avg drops below threshold</p>
                </div>
                <Toggle checked={alertConfig.alertLowRatings} onChange={(val) => setAlertConfig((p) => ({ ...p, alertLowRatings: val }))} t={t} />
              </div>
              {alertConfig.alertLowRatings && (
                <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", borderRadius: RADIUS.chip, background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, marginBottom: 8 }}>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>Low Rating Threshold</p>
                    <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>Alert triggers below this value</p>
                  </div>
                  <input
                    type="number" min="1" max="5" step="0.1"
                    value={alertConfig.lowRatingThreshold}
                    onChange={(e) => setAlertConfig((p) => ({ ...p, lowRatingThreshold: e.target.value }))}
                    style={{ width: 70, padding: "8px 10px", borderRadius: RADIUS.button, background: `${ACCENT_PURPLE.base}1a`, border: `1.5px solid ${ACCENT_PURPLE.base}4d`, color: ACCENT_PURPLE.base, fontSize: 13, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY, outline: "none", textAlign: "center" }}
                  />
                </div>
              )}
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 20, alignItems: "center" }}>
                <button
                  ...
                  disabled={alertSaving}
                  style={{
                    flex: 1, padding: "11px", borderRadius: RADIUS.button, border: "none",
                    background: alertSaving ? `${ACCENT_PURPLE.base}4d` : `linear-gradient(135deg,${ACCENT_PURPLE.base},${ACCENT_PURPLE.to || ACCENT_PURPLE.base})`,
                    color: "#fff", fontSize: 12, fontWeight: FONT_WEIGHT.bold, cursor: "pointer",
                    fontFamily: FONT_FAMILY, boxShadow: `0 4px 14px ${ACCENT_PURPLE.base}59`,
                  }}
                >
                ...
                <button
                  onClick={() => setAlertModal(false)}
                  style={{ flex: 1, padding: "11px", borderRadius: RADIUS.button, border: `1px solid ${t.border}`, background: t.actBg, color: t.textSub, fontSize: 12, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}
                >
                  Cancel
                </button>
                <button
                  onClick={handleDeleteAlertConfig}
                  style={{ padding: "11px 16px", borderRadius: RADIUS.button, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.25)", color: "#f87171", fontSize: 12, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY, display: "flex", alignItems: "center", gap: 5 }}
                >
                  <Trash2 size={13} /> Delete
                </button>
              </div>
...
/* ─── Quick action pill button (needs its own hover state) ─── */
function QuickActionButton({ action, t }) {
  const [hov, setHov] = useState(false);
  return (
    <button
      ...
      onMouseLeave={() => setHov(false)}
      style={{
        display: "flex", alignItems: "center", gap: 8, padding: "7px 16px", borderRadius: RADIUS.button,
        border: `1px solid ${hov ? action.color + "55" : t.border}`,
        background: hov ? `${action.color}12` : "transparent",
        color: hov ? action.color : t.textMuted,
        fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer",
        transition: "all 0.2s", fontFamily: FONT_FAMILY,
      }}
    >
      <span style={{ width: 5, height: 5, borderRadius: "50%", background: hov ? action.color : t.textMuted, transition: "background 0.2s", flexShrink: 0 }} />
      {action.label}
    </button>
  );
}