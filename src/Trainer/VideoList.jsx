      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ margin: "2px 0 0", fontSize: 11, color: t.textSub, fontFamily: FF }}>
              {isUrlVideo ? "URL-based video" : "File upload video"} · ID {video.id}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {isUrlVideo ? (
            <div style={rowStyle}>
              ...
              />
              {video.videoUrl && (
                <span style={{ fontSize: 10, color: t.textMuted, marginTop: 2, fontFamily: FF }}>
                  Current: {video.videoUrl.length > 55 ? video.videoUrl.slice(0, 52) + "…" : video.videoUrl}
                </span>
              )}
            </div>
          ) : (
            <div style={rowStyle}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 11, color: t.textMuted, fontFamily: FF }}>
                  {newFile ? newFile.name : video.originalFileName || "no file chosen"}
                </span>
              </div>
              <input
                ref={fileRef}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={rowStyle}>
              ...
              <input style={inputStyle} value={tags} onChange={(e) => setTags(e.target.value)} placeholder="java, spring, oop" />
            </div>
            <div style={rowStyle}>
              <label style={labelStyle}>Category</label>
              <input style={inputStyle} value={category} onChange={(e) => setCategory(e.target.value)} placeholder="Education" />
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={rowStyle}>
              ...
              <select style={{ ...inputStyle, cursor: "pointer" }} value={language} onChange={(e) => setLanguage(e.target.value)}>
                {["English", "Hindi", "Telugu", "Tamil", "Kannada", "Malayalam", "Other"].map((l) => (
            ...
            <div style={rowStyle}>
              <label style={labelStyle}>Visibility</label>
              <select style={{ ...inputStyle, cursor: "pointer" }} value={visibility} onChange={(e) => setVisibility(e.target.value)}>
                <option value="public">Public</option>
                <option value="private">Private</option>
                <option value="unlisted">Unlisted</option>
              </select>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <div style={rowStyle}>
              <label style={labelStyle}>Course</label>
              <input style={inputStyle} value={course} onChange={(e) => setCourse(e.target.value)} placeholder="e.g. Java Bootcamp" />
            </div>
            <div style={rowStyle}>
              <label style={labelStyle}>Status</label>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", userSelect: "none" }}>
            <input
              ...
              style={{ width: 14, height: 14, cursor: "pointer" }}
            />
            <span style={{ fontSize: 12, color: t.textSub, fontFamily: FF }}>Age Restricted (18+)</span>
          </label>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 6 }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: RADIUS.button,
                border: "none",
                background: saving ? "#1d4ed8aa" : "linear-gradient(135deg,#2563eb,#1d4ed8)",
                color: "#fff",
                fontSize: 12,
                fontWeight: FONT_WEIGHT.bold,
                cursor: saving ? "not-allowed" : "pointer",
...
const VideoList = ({ refreshKey, trainerMode }) => {
  ...
  const handleVideoUpdated = (updatedVideo) => {
    setVideos((p) => p.map((v) => (v.id === updatedVideo.id ? updatedVideo : v)));
  };
  ...
  const stats = [
    { label: "Total Videos", numericValue: totalVideos, change: `${totalVideos} uploaded`, icon: Film, colorKey: "blue" },
    { label: "Published", numericValue: publishedCount, change: `${publishedCount} live`, icon: Send, colorKey: "green" },
    { label: "Drafts", numericValue: draftCount, change: `${draftCount} pending`, icon: Pencil, colorKey: "orange" },
    { label: "Needs Batch", numericValue: noBatchCount, change: noBatchCount > 0 ? `${noBatchCount} unassigned` : "All assigned", icon: Layers, colorKey: "purple" },
  ];
    ...
    <PageContainer mode={isDark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      <Hero borderHero={t.borderHero}>
        <div>
          ...
          <p style={{ fontSize: 12, color: t.textSub, margin: 0, fontWeight: 500, fontFamily: FF }}>
            {trainerMode ? "Upload, organize, and publish your recorded lessons" : "Watch lectures uploaded by your trainers"}
          </p>
        </div>
      ...
      {loading ? (
        <div className="vl-grid">
          {[1, 2, 3].map((i) => (
            <div key={i} style={{ ...card, padding: CARD_PADDING.standardCard }}>
              <div style={{ height: 144, borderRadius: RADIUS.chip, background: t.barBg, marginBottom: 12 }} />
              <div style={{ height: 12, width: "70%", borderRadius: 6, background: t.barBg, marginBottom: 8 }} />
              <div style={{ height: 10, width: "50%", borderRadius: 5, background: t.barBg, marginBottom: 6 }} />
              <div style={{ height: 10, width: "40%", borderRadius: 5, background: t.barBg }} />
            </div>
          ))}
        </div>
      ) : videos.length === 0 ? (
        ...
        >
          ...
          <p style={{ fontSize: 13, fontWeight: FONT_WEIGHT.bold, color: t.textSub, margin: 0, fontFamily: FF }}>No videos yet</p>
          <p style={{ fontSize: 11, color: t.textMuted, margin: 0, textAlign: "center", fontFamily: FF }}>
              ...
              >
                ...
                >
                  {playerReady ? (
                    ...
                    >
                      ...
                      {isPlaying && !isUrlVideo && !blobUrl && (
                        <div style={{ position: "absolute", bottom: 8, fontSize: 10, color: "rgba(255,255,255,0.6)", fontFamily: FF, zIndex: 2 }}>
                          Loading…
                        </div>
                      )}
                    </div>
                ...
                {tags.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4, alignItems: "center" }}>
                    ...
                    {tags.length > 4 && (
                      <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FF }}>+{tags.length - 4}</span>
                    )}
                  </div>
                )}
                ...
                >
                  <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FF }}>
                    {isUrlVideo ? (
                ...
                {showPublishBtn && <PublishButton video={video} onPublished={handleVideoUpdated} />}
                {showAssignBtn && <AssignBatchButton video={video} onAssigned={handleVideoUpdated} t={t} />}
