...
//             micOn ? "bg-gray-100 dark:bg-[#1F2937]" : "bg-red-600 text-white"
//           }`}
//         >
...
//           className={`p-4 rounded-full transition ${
//             camOn ? "bg-gray-100 dark:bg-[#1F2937]" : "bg-red-600 text-white"
//           }`}
//         >
...
//           className="p-4 rounded-full bg-gray-100 dark:bg-[#1F2937]"
//         >
//           <FaDotCircle />
//         </button>
  ...
  useEffect(() => {
    const startLive = async () => {
      try {
        ...
        const res = await startLiveSession(id); // ✅ FIXED
    ...
    <div className="fixed inset-0 flex flex-col bg-gray-100 text-gray-900 dark:bg-[#0B1120] dark:text-white">
      ...
      <div className="flex justify-between items-center px-8 py-4 bg-white shadow-md dark:bg-[#111827] dark:border-b dark:border-white/10">
        ...
        <div className="flex items-center gap-6">
          ...
          <button
            onClick={handleEndSession}
            className="px-4 py-2 rounded-xl bg-red-600 text-white text-sm hover:bg-red-500"
          >
            <FaPhoneSlash size={12} /> End Session
          </button>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* VIDEO AREA */}
        <div className="flex-1 p-6 grid grid-cols-4 grid-rows-2 gap-5">
          <div className="col-span-3 row-span-2 bg-black rounded-3xl flex items-center justify-center text-white relative">
            {/* ✅ LIVEKIT */}
            {token ? (
              <LiveKitRoomComponent token={token} />
            ) : (
          ...
          {/* PARTICIPANTS */}
          <div className="rounded-3xl flex items-center justify-center text-gray-400">
            Waiting...
          </div>
        </div>
      ...
      {/* ================= CONTROLS ================= */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white px-6 py-3 rounded-full flex gap-4">
        ...
        <button onClick={handleEndSession} className="text-red-600">
          <FaPhoneSlash />
        </button>
      </div>
    </div>
