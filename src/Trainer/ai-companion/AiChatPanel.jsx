...
export default function AiChatPanel({
  ...
  const [aiUsage, setAiUsage] = useState(null); // { used, limit, tier, period }
  ...
  const loadAiUsage = useCallback(async () => {
    try {
      const res = await getAiCompanionUsage();
      setAiUsage(res.data);
    } catch (err) {
      console.error("Failed to load AI Companion usage:", err);
    }
  }, []);
  ...
  const sendMessage = useCallback(
    async (modeObj, customMessage) => {
      ...
      // Some modes (Recording/Whiteboard/Chat Summary, etc.) need a specific
      // source enabled even if the trainer never touched the source
      // dropdown. Union it in so the request always carries what the
      // selected mode actually needs.
      const activeMode = modeObj || selectedMode;
      const requiredSources = activeMode?.requiredSources || [];
      const effectiveSources = Array.from(
        new Set([...selectedSources, ...requiredSources]),
      );
      if (effectiveSources.length !== selectedSources.length) {
        setSelectedSources(effectiveSources);
      }
      ...
      try {
        ...
        if (planError) {
          // Roll back the optimistic user bubble we already pushed — a
          // plan-blocked request never actually reached the AI, so it
          // shouldn't sit in the transcript looking like a sent message.
          ...
          <div style={{ width: "100%", maxWidth: 600, marginBottom: 24 }}>
            ...
            {aiUsage && aiUsage.limit === 0 && (
              ...
              >
                ...
                >
                  AI Companion isn't available on your current plan.
                </span>
              </div>
            )}
