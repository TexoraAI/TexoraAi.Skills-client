...
export default function ContactUs() {
  ...
  const [loading, setLoading] = useState(false);
  const [sent, setSent] = useState(false);
  // ✅ New: track API-level errors separately from validation errors
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { ok, status, data } = await submitContactForm({
        ...
        phoneNumber: form.phone.trim() || undefined,   // optional field
        topic:       form.topic || undefined,           // optional field
        message:     form.message.trim(),
      });

      if (ok) {
        // 201 Created — backend also sends auto-reply + support alert emails
        setSent(true);
      } else if (status === 400) {
        // Validation failure from backend (e.g. constraint violation)
