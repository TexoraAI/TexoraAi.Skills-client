...
const ACCENT = {
  violet: {
    ...
    ring: "focus:ring-violet-400",
    iconBg: "bg-violet-100 dark:bg-violet-500/20",
    highlight: "bg-violet-50 dark:bg-violet-500/10 text-violet-700 dark:text-violet-300",
  },
  emerald: {
    btn: "bg-emerald-600 hover:bg-emerald-700",
    text: "text-emerald-600 dark:text-emerald-400",
    ...
    ring: "focus:ring-emerald-400",
    iconBg: "bg-emerald-100 dark:bg-emerald-500/20",
    highlight: "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300",
  },
  rose: {
    btn: "bg-rose-600 hover:bg-rose-700",
    text: "text-rose-600 dark:text-rose-400",
    ...
    ring: "focus:ring-rose-400",
    iconBg: "bg-rose-100 dark:bg-rose-500/20",
    highlight: "bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-300",
  },
  amber: {
    btn: "bg-amber-500 hover:bg-amber-600",
    text: "text-amber-600 dark:text-amber-400",
    ...
    ring: "focus:ring-amber-400",
    iconBg: "bg-amber-100 dark:bg-amber-500/20",
    highlight: "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-300",
  },
};
...
/* ══════════════════════════════════════════════════════════════
   SEARCHABLE DROPDOWN COMPONENT — NEW
   Props:
     name        – field name (used to fire handleChange-compatible event)
     value       – current selected value (string)
     onChange    – (e: {target:{name,value}}) => void  ← same API as native inputs
     options     – string[]
     placeholder – string
     error       – boolean
     accentRing  – Tailwind ring class  e.g. "focus:ring-violet-400"
     addNewLabel – e.g. "Add New Country"
     disabled    – boolean
══════════════════════════════════════════════════════════════ */
const SearchableDropdown = ({
  ...
  // keep options in sync if parent passes new list
  useEffect(() => {
    setOptions((prev) => {
...
/* ══════════════════════════════════════════════════════════════
   MULTI-SELECT SEARCHABLE DROPDOWN — NEW (for Trainer Platforms)
══════════════════════════════════════════════════════════════ */
const MultiSearchableDropdown = ({
  ...
  useEffect(() => {
    setOptions((prev) => {
      const merged = [...initialOptions];
      prev.forEach((o) => { if (!merged.includes(o)) merged.push(o); });
      return merged;
    });
  }, [initialOptions]);
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        <span className={value.length ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-white/30"}>
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mt-2">
          {value.map((v) => (
            ...
            >
              {v}
              <button type="button" onClick={() => toggle(v)} className="ml-0.5 opacity-70 hover:opacity-100">
      ...
      {open && (
        ...
        >
          ...
          <div className="max-h-48 overflow-y-auto">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-gray-50 dark:hover:bg-white/5 flex items-center gap-3 ${
                    checked ? "font-medium text-gray-900 dark:text-white" : "text-gray-700 dark:text-white/80"
                  }`}
                >
                  <span
            ...
            {filtered.length === 0 && (
              <p className="px-4 py-3 text-xs text-gray-400 dark:text-slate-500">No results</p>
            )}
          </div>
          <div className="border-t border-gray-100 dark:border-white/10 p-2">
            {addingNew ? (
              <div className="flex gap-2 px-1">
                <input
                  ...
                  className="flex-1 px-3 py-2 rounded-lg text-sm bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/30 focus:outline-none"
                />
                <button type="button" onClick={handleAddNew} className="px-3 py-2 rounded-lg text-xs font-semibold bg-gray-900 dark:bg-white/20 text-white">Add</button>
                <button type="button" onClick={() => { setAddingNew(false); setNewVal(""); }} className="px-2 py-2 rounded-lg text-xs text-gray-400">
                  <X className="w-3.5 h-3.5" />
                </button>
              </div>
            ) : (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Manage your profile details</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={() => setEditing(false)} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">
                <X className="w-3.5 h-3.5" /> Cancel
              </button>
              <button onClick={handleSave} disabled={saving} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm text-white ${ac.btn} shadow transition-colors disabled:opacity-60`}>
            ...
            <button onClick={() => setEditing(true)} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/20 text-gray-700 dark:text-white/80 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">
              <Edit3 className="w-3.5 h-3.5" /> Edit Profile
            </button>
          )}
        </div>
      ...
      <div className="flex items-center gap-5 p-4 rounded-2xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
        <Avatar initials={user.avatar} size={72} editable online shape="rounded" />
        <div>
          <p className="text-sm font-semibold text-gray-800 dark:text-white mb-1">Profile Photo</p>
          <p className="text-xs text-gray-400 dark:text-slate-400 mb-3">JPG or PNG · Saved locally · Updates Navbar & Sidebar instantly</p>
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => fileInputRef.current?.click()} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-white ${ac.btn} transition-colors`}>
              <Upload className="w-3 h-3" /> Upload Photo
            </button>
            {profileImage && (
              <button onClick={removeImage} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/30 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors">
                <X className="w-3 h-3" /> Remove Photo
              </button>
            )}
          </div>
          <input ref={fileInputRef} type="file" accept="image/jpeg,image/png" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f && (f.type === "image/jpeg" || f.type === "image/png")) uploadImage(f); e.target.value = ""; }} />
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="space-y-1.5">
          ...
          {editing ? (
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl text-sm transition-all bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/30 focus:outline-none focus:ring-2 ${ac.ring}`} />
          ) : (
            <p className="px-4 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-800 dark:text-white/85">{name || "—"}</p>
          )}
        </div>
        <div className="space-y-1.5">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-widest">
          ...
          <div className="relative">
            <p className="px-4 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 select-none">{user.label}</p>
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs px-2 py-0.5 rounded-full bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/40 border border-gray-300 dark:border-white/10">Locked</span>
          </div>
          <p className="text-xs text-gray-400 dark:text-slate-500 pl-1">Role cannot be changed from this page</p>
        </div>
        <div className="space-y-1.5">
          <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-widest">
            <Mail className="w-3.5 h-3.5" /> Email Address
          ...
          <div className="relative">
            <p className="px-4 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 select-none">{user.email}</p>
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs px-2 py-0.5 rounded-full bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/40 border border-gray-300 dark:border-white/10">Locked</span>
          </div>
          <p className="text-xs text-gray-400 dark:text-slate-500 pl-1">Email cannot be changed from this page</p>
        </div>
      </div>
    </div>
  );
...
/* ══════════════════════════════════════════════════════════════
   DETAILS TAB HELPERS — MODULE LEVEL, UNCHANGED
══════════════════════════════════════════════════════════════ */
const LABEL_CLS =
  "flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-widest mb-1.5";
...
const DD_COUNTRY = [
  ...
  "Saudi Arabia", "Malaysia", "Nepal", "Sri Lanka", "Pakistan",
  "Bangladesh", "South Africa", "Brazil", "New Zealand", "Netherlands",
];
...
const DD_CITY = [
  ...
  "Noida", "Gurgaon", "Pune", "Kolkata", "Lucknow",
  "Ahmedabad", "Surat", "Indore", "Jaipur", "Patna",
  "Ranchi", "Kochi", "Chandigarh", "Visakhapatnam", "Coimbatore",
];

const DD_QUALIFICATION = [
  "10th", "12th", "Diploma", "ITI", "Polytechnic",
  "B.Tech", "B.E.", "BCA", "B.Sc", "B.Com",
  "BA", "BBA", "MBA", "MCA", "M.Tech",
  "M.Sc", "MA", "PhD", "Post Doctorate", "Other",
];

const DD_COLLEGE = [
  "IIT Delhi", "IIT Bombay", "IIT Madras", "IIT Kanpur", "IIT Kharagpur",
  "BITS Pilani", "NIT Trichy", "NIT Surathkal", "Delhi University", "Anna University",
  "VIT", "SRM", "KIIT", "Amity University", "LPU",
  "Chandigarh University", "Manipal University", "JNTU Hyderabad", "AMU", "Jamia",
];

const DD_DOMAIN = [
  "Full Stack Development", "Frontend Development", "Backend Development",
  "React.js", "Angular", "Vue.js", "Java", "Spring Boot", "Python",
  "Django", "Node.js", "DevOps", "AWS", "Azure",
  "Cyber Security", "Data Science", "Artificial Intelligence", "Machine Learning",
  "UI/UX Design", "Testing / QA", "Mobile Development",
];

const DD_EXPERIENCE = [
  "Fresher", "Internship", "Less than 1 Year",
...
const DD_COURSE_TOPIC = [
  "React.js", "Angular", "Vue.js", "Python", "Java",
  "Spring Boot", "Node.js", "AWS", "Azure", "Cloud Computing",
  "Networking", "Cyber Security", "AI / ML", "Data Science", "Testing / QA",
  "DevOps", "Docker & Kubernetes", "SQL / Databases", "UI/UX Design", "Blockchain",
];

const DD_AUDIENCE_SIZE = ["0–1K", "1K–10K", "10K–50K", "50K–100K", "100K+", "500K+"];

const DD_FULL_TIME_ROLE = ["Yes", "No", "Part Time", "Freelancer", "Consultant"];

const DD_PLATFORMS = [
  "YouTube", "Instagram", "LinkedIn", "Facebook", "Twitter/X",
  "Podcast", "Telegram", "WhatsApp", "Discord", "TikTok",
  "Blog", "Newsletter", "Other",
];

const DD_INDUSTRY = [
  "EdTech", "FinTech", "Healthcare", "Retail", "Manufacturing",
  "Consulting", "Government", "IT Services", "Media", "Real Estate",
  "Education", "Banking", "Insurance", "Telecom", "Logistics",
  "NGO", "E-Commerce", "Other",
];

const DD_LOCATION = [
  "Hyderabad, India", "Bangalore, India", "Mumbai, India", "Delhi, India",
...
const DD_ORG_DOMAIN = [
  "Education", "Software", "Healthcare", "Finance", "Manufacturing",
  "Retail", "Consulting", "Enterprise", "Startup", "Government",
];

/* ══════════════════════════════════════════════════════════════
   STUDENT DETAILS TAB — ENHANCED
══════════════════════════════════════════════════════════════ */
const StudentDetailsTab = ({ accent, returnTo }) => {
  const navigate = useNavigate();
  const ac = ACCENT[accent];
  ...
  // refs for auto-focus on first error
  ...
  // unified onChange — works for both native inputs and SearchableDropdown
  const handleChange = useCallback((e) => {
    const { name, value } = e.target;
    setDraft((p) => ({ ...p, [name]: value }));
    setErrors((p) => ({ ...p, [name]: "" }));
  ...
  const validate = (d) => {
    ...
    else if (!/^\d{6,15}$/.test(d.localNumber.trim())) errs.localNumber = "Enter 6–15 digits only";
    ...
    if (!d.qualification.trim()) errs.qualification = "Qualification is required";
    if (!d.collegeName.trim()) errs.collegeName = "College / Institute is required";
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Personal, location and education details</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={cancelEdit} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">
                <X className="w-3.5 h-3.5" /> Cancel
              </button>
              <button onClick={handleSave} disabled={saving} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm text-white ${ac.btn} shadow transition-colors disabled:opacity-60`}>
                {saving ? (<><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Saving…</>) : (<><Save className="w-3.5 h-3.5" /> Save Details</>)}
              </button>
            </>
          ) : (
            <button onClick={startEdit} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/20 text-gray-700 dark:text-white/80 hover:bg-gray-200 dark:hover:bg-white/20 transition-colors">
              <Edit3 className="w-3.5 h-3.5" /> Edit Details
            </button>
          )}
        </div>
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      handleChange({ target: { name: "localNumber", value: val } });
                    }}
                    maxLength={15}
                    className={inputCls("localNumber")}
                  />
...
const TrainerDetailsTab = ({ accent, returnTo }) => {
  ...
  // unified onChange — handles both normal fields and multi-select array fields
  const handleChange = useCallback((e) => {
    const { name, value } = e.target;
    setDraft((p) => ({ ...p, [name]: value }));
    setErrors((p) => ({ ...p, [name]: "" }));
  ...
  const validate = (d) => {
    ...
    if (!d.fullTimeRole) errs.fullTimeRole = "Full-time role selection is required";
    return errs;
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // payload unchanged — same structure as before
      await userService.updateTrainerProfile(draft);
      setForm({ ...draft });
      setEditing(false);
      showToast("Trainer profile saved");
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">Trainer Profile</h3>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Your training background and platform details</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
              <button onClick={cancelEdit} disabled={saving} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/10 text-gray-600 dark:text-white/70 hover:bg-gray-200 transition-colors">
                <X className="w-3.5 h-3.5" /> Cancel
              </button>
              <button onClick={handleSave} disabled={saving} className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm text-white ${ac.btn} shadow transition-colors disabled:opacity-60`}>
            ...
            <button onClick={startEdit} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-200 dark:border-white/20 text-gray-700 dark:text-white/80 hover:bg-gray-200 transition-colors">
              <Edit3 className="w-3.5 h-3.5" /> Edit Profile
            </button>
          )}
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {/* LinkedIn URL — keep as text input (URL) */}
          <DetailField icon={Globe} label="LinkedIn URL">
            {editing ? (
              <div>
                <input
          ...
          {/* Course Topic — Searchable Dropdown */}
          <DetailField icon={BookOpen} label="Course Topic">
            {editing ? (
              <div>
                <SearchableDropdown
          ...
          {/* Audience Size — Searchable Dropdown */}
          <DetailField icon={Users} label="Audience Size">
            {editing ? (
              <div>
                <SearchableDropdown
          ...
          {/* Full-Time Role — Searchable Dropdown */}
          <DetailField icon={Briefcase} label="Full-Time Role?">
            {editing ? (
              <div>
                <SearchableDropdown
      ...
      {/* ── Platforms — Multi-Select Searchable Dropdown ── */}
      <div>
        <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
          <TrendingUp className="w-3.5 h-3.5" /> Platforms
        </p>
        {editing ? (
          ...
          <div className="flex flex-wrap gap-2">
            {(form.platforms || []).length === 0 ? (
              ...
              (form.platforms || []).map((p) => (
                <span key={p} className={`px-3 py-1 rounded-full text-xs font-semibold border ${ac.badge}`}>{p}</span>
              ))
            )}
          </div>
        )}
...
const AdminDetailsTab = ({ accent, returnTo }) => {
  ...
  useEffect(() => {
    ...
    authService
      ...
      .then((capacityData) =>
        getOrgSummary()
          .catch(() => ({ currentDepartments: 0, currentBranches: 0, currentBatches: 0 }))
          .then((summaryData) => {
            const loaded = {
              organizationName: capacityData.organizationName || "",
              domain: capacityData.domain || "",
              ...
              status: capacityData.status || "",
              planExpiryDate: capacityData.planExpiryDate || "",
              maxStudents: capacityData.maxStudents != null ? String(capacityData.maxStudents) : "",
              maxTrainers: capacityData.maxTrainers != null ? String(capacityData.maxTrainers) : "",
              currentStudents: capacityData.currentStudents != null ? String(capacityData.currentStudents) : "",
              currentTrainers: capacityData.currentTrainers != null ? String(capacityData.currentTrainers) : "",
              maxDepartments: capacityData.maxDepartments != null ? String(capacityData.maxDepartments) : "",
              maxBranchesPerDept: capacityData.maxBranchesPerDept != null ? String(capacityData.maxBranchesPerDept) : "",
              maxBatchesPerBranch: capacityData.maxBatchesPerBranch != null ? String(capacityData.maxBatchesPerBranch) : "",
              currentDepartments: summaryData.currentDepartments != null ? String(summaryData.currentDepartments) : "0",
              currentBranches: summaryData.currentBranches != null ? String(summaryData.currentBranches) : "0",
              currentBatches: summaryData.currentBatches != null ? String(summaryData.currentBatches) : "0",
            };
            setForm(loaded);
            setDraft(loaded);
          })
  ...
  const validate = (d) => {
    const errs = {};
    if (!d.organizationName.trim()) errs.organizationName = "Organization name is required";
    if (!d.industry) errs.industry = "Industry is required";
    if (!d.location.trim()) errs.location = "Location is required";
    if (d.contactEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(d.contactEmail.trim()))
      errs.contactEmail = "Enter a valid email address";
    if (d.mobileNumber && !/^\+?\d{7,15}$/.test(d.mobileNumber.replace(/\s/g, "")))
  ...
  const handleSave = async () => {
    const errs = validate(draft);
    if (Object.keys(errs).length) { setErrors(errs); showToast("Please fix all errors before saving", "error"); return; }
    setSaving(true);
    try {
      const orgId = localStorage.getItem("organizationId");
      const payload = {
  ...
  if (loading) {
    return (
      <div className="space-y-4 animate-pulse">
        <div className="h-8 w-48 rounded-xl bg-gray-200 dark:bg-white/10" />
        <div className="grid grid-cols-2 gap-4">
          {[...Array(6)].map((_, i) => <div key={i} className="h-12 rounded-xl bg-gray-200 dark:bg-white/10" />)}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Your organization information</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <DetailField icon={Building2} label="Organization Name">
            {editing ? (
              <div>
                <input type="text" name="organizationName" placeholder="e.g. Texora AI" value={draft.organizationName} onChange={handleChange} className={inputCls("organizationName")} />
          ...
          <DetailField icon={Mail} label="Contact Email">
            {editing ? (
              <div>
                <input type="email" name="contactEmail" placeholder="e.g. admin@texora.ai" value={draft.contactEmail} onChange={handleChange} className={inputCls("contactEmail")} />
          ...
          <DetailField icon={Phone} label="Mobile Number">
            {editing ? (
              <div>
                <input type="tel" name="mobileNumber" placeholder="+91 9876543210" value={draft.mobileNumber} onChange={handleChange} className={inputCls("mobileNumber")} />
          ...
          <div className="sm:col-span-2">
            <DetailField icon={BookOpen} label="Description">
              {editing ? (
                <textarea name="description" rows={3} placeholder="Brief description of your organization" value={draft.description} onChange={handleChange} className={`${inputCls("description")} resize-none`} />
              ) : <StaticVal val={form.description} />}
            </DetailField>
          </div>
        </div>
      ...
      <div>
        <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
          <Lock className="w-3.5 h-3.5" /> Plan & Limits
          <span className="ml-1 text-xs font-normal normal-case text-gray-400 dark:text-slate-500">— managed by SuperAdmin</span>
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {[
            { icon: CreditCard, label: "Plan", val: form.plan },
            ...
            <div key={label} className="space-y-1.5">
              <label className={LABEL_CLS}><Ic className="w-3.5 h-3.5" /> {label}</label>
              <div className="relative">
                <p className="px-4 py-2.5 rounded-xl text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-gray-500 dark:text-white/50 select-none pr-16">{val || "—"}</p>
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs px-2 py-0.5 rounded-full bg-gray-200 dark:bg-white/10 text-gray-500 dark:text-white/40 border border-gray-300 dark:border-white/10">Locked</span>
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 gap-4 mt-4">
          {[
            ...
            <div key={label} className="space-y-1.5">
              <label className={LABEL_CLS}><Ic className="w-3.5 h-3.5" /> {label}</label>
              <p className={`${STATIC_CLS} text-emerald-600 dark:text-emerald-400 font-semibold`}>{val || "0"}</p>
            </div>
          ))}
        </div>
      </div>
...
/* ══════════════════════════════════════════════════════════════
   BUSINESS DETAILS TAB — UNCHANGED (no dropdown requirements specified)
══════════════════════════════════════════════════════════════ */
const BusinessDetailsTab = ({ accent, returnTo }) => {
  const navigate = useNavigate();
  const ac = ACCENT[accent];
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Your company and contact information</p>
        </div>
        <div className="flex gap-2">
          {editing ? (
            <>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <DetailField icon={Building2} label="Company Name">
            {editing ? <input type="text" name="companyName" placeholder="e.g. Acme Corp" value={draft.companyName} onChange={handleChange} className={inputCls} /> : <StaticVal val={form.companyName} />}
          </DetailField>
          <DetailField icon={Briefcase} label="Industry">
            {editing ? (
              <SearchableDropdown name="industry" value={draft.industry} onChange={handleChange} options={DD_INDUSTRY} placeholder="Select industry" accentRing={ac.ring} addNewLabel="Add New Industry" />
            ) : <StaticVal val={form.industry} />}
          </DetailField>
          <DetailField icon={MapPin} label="Location">
            {editing ? <input type="text" name="location" placeholder="e.g. Mumbai, India" value={draft.location} onChange={handleChange} className={inputCls} /> : <StaticVal val={form.location} />}
          </DetailField>
          <DetailField icon={Globe} label="Website">
            {editing ? <input type="url" name="website" placeholder="https://example.com" value={draft.website} onChange={handleChange} className={inputCls} /> : <StaticVal val={form.website} />}
          </DetailField>
          <DetailField icon={Mail} label="Contact Email">
            {editing ? <input type="email" name="contactEmail" placeholder="e.g. contact@company.com" value={draft.contactEmail} onChange={handleChange} className={inputCls} /> : <StaticVal val={form.contactEmail} />}
          </DetailField>
          <DetailField icon={Phone} label="Mobile Number">
            {editing ? <input type="tel" name="mobileNumber" placeholder="+91 9876543210" value={draft.mobileNumber} onChange={handleChange} className={inputCls} /> : <StaticVal val={form.mobileNumber} />}
          </DetailField>
          <div className="sm:col-span-2">
            <DetailField icon={BookOpen} label="Description">
              {editing ? <textarea name="description" placeholder="Brief description of your business" value={draft.description} onChange={handleChange} rows={3} className={`${inputCls} resize-none`} /> : <StaticVal val={form.description} />}
            </DetailField>
          </div>
        </div>
      </div>
...
const SecurityTab = ({ accent }) => {
  ...
  const handlePasswordChange = async () => {
    ...
    try {
      ...
      console.error("Change password error:", err);
      const msg = err?.response?.data?.message || err?.response?.data || "Failed to update password. Please try again.";
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-6 shadow-sm bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10">
        <div className="mb-5">
          ...
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Choose a strong password with at least 6 characters</p>
        </div>
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-widest">
            ...
            <input type="password" placeholder="Enter new password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/30 focus:outline-none focus:ring-2 ${ac.ring}`} />
          </div>
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-slate-400 uppercase tracking-widest">
              <Lock className="w-3.5 h-3.5" /> Confirm New Password
            </label>
            <input type="password" placeholder="Confirm new password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={`w-full px-4 py-2.5 rounded-xl text-sm bg-gray-100 dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-white/30 focus:outline-none focus:ring-2 ${ac.ring}`} />
          </div>
          {confirmPassword && (
            <p className={`text-xs flex items-center gap-1.5 ${newPassword === confirmPassword ? "text-emerald-500" : "text-red-500"}`}>
              {newPassword === confirmPassword ? (<><CheckCircle className="w-3.5 h-3.5" /> Passwords match</>) : (<><X className="w-3.5 h-3.5" /> Passwords do not match</>)}
            </p>
          )}
        </div>
        <button onClick={handlePasswordChange} disabled={loading} className={`mt-6 flex items-center gap-2 px-6 py-2.5 rounded-xl text-white text-sm font-medium ${ac.btn} shadow transition-colors disabled:opacity-60`}>
          {loading ? (<><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Updating…</>) : (<><Shield className="w-4 h-4" /> Update Password</>)}
        </button>
      </div>
      <div className="rounded-2xl p-5 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
        <h4 className="text-sm font-semibold text-gray-700 dark:text-white/80 mb-3">Password Tips</h4>
        <ul className="space-y-2">
          {["Use at least 8 characters", "Mix uppercase, lowercase, numbers & symbols", "Avoid using personal info like your name or birthday", "Don't reuse passwords from other sites"].map((tip, i) => (
            <li key={i} className="flex items-start gap-2 text-xs text-gray-500 dark:text-slate-400">
              <CheckCircle className="w-3.5 h-3.5 mt-0.5 shrink-0 text-gray-400 dark:text-slate-500" />{tip}
            </li>
          ))}
        </ul>
      </div>
...
const BillingTab = ({ user, accent }) => {
  ...
  return (
    <div className="space-y-6">
      <div className={`relative overflow-hidden rounded-2xl p-6 bg-gradient-to-r ${user.heroGradient} shadow-xl`}>
        <div className="absolute -top-16 -right-16 w-56 h-56 bg-white/5 rounded-full blur-3xl pointer-events-none" />
        <div className="relative flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Zap className="w-4 h-4 text-white/80" />
              <span className="text-xs font-semibold text-white/80 uppercase tracking-widest">Current Plan</span>
            </div>
            <h3 className="text-2xl font-bold text-white">{user.plan}</h3>
            <p className="text-white/70 text-sm mt-1">{user.planPrice} · Billed monthly</p>
            <div className="flex items-center gap-1.5 mt-3">
              <BadgeCheck className="w-4 h-4 text-white/80" />
              <span className="text-xs text-white/80">Renews on May 1, 2025</span>
            </div>
          </div>
          <button className="px-4 py-2 rounded-xl bg-white/20 backdrop-blur-sm text-white text-sm font-medium border border-white/30 hover:bg-white/30 transition-colors">Upgrade Plan</button>
        </div>
      </div>
      <div className="rounded-2xl p-6 shadow-sm bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900 dark:text-white">Payment Method</h3>
          <button className={`text-xs ${ac.text} hover:underline`}>Change</button>
        </div>
        <div className="flex items-center gap-4 p-4 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10">
          <div className={`p-2.5 rounded-xl ${ac.iconBg}`}><CreditCard className={`w-5 h-5 ${ac.text}`} /></div>
          <div>
            <p className="text-sm font-medium text-gray-900 dark:text-white">Visa ending in 4242</p>
            <p className="text-xs text-gray-400 dark:text-slate-400">Expires 08/2027</p>
          </div>
          <span className={`ml-auto text-xs px-2 py-1 rounded-full border ${ac.badge}`}>Default</span>
        </div>
      </div>
      <div className="rounded-2xl p-6 shadow-sm bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 space-y-4">
        ...
        <div className="space-y-1">
          {history.map((h, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
              <div className="flex items-center gap-3">
                <div className="p-1.5 rounded-lg bg-gray-100 dark:bg-white/10"><CheckCircle className="w-3.5 h-3.5 text-emerald-500" /></div>
                <div>
                  <p className="text-sm text-gray-800 dark:text-white/90">{h.inv}</p>
                  <p className="text-xs text-gray-400 dark:text-slate-500">{h.date}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm font-semibold text-gray-900 dark:text-white">{h.amount}</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 border border-emerald-200 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border-emerald-500/30">{h.status}</span>
              </div>
            </div>
          ))}
        </div>
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    userService
      ...
      .then((res) => {
        ...
        const apiId = data?.userId || ROLE_CONFIG[roleKey].id;
        const initial = apiName.charAt(0).toUpperCase();
        setUser((prev) => ({ ...prev, name: apiName, email: apiEmail, id: apiId, label: roleLabel, avatar: initial }));
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0f] transition-colors duration-300">
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        {error && (
          <div className="px-4 py-3 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 text-sm">{error}</div>
        )}
        {showReturnBanner && (
          <div className="px-4 py-3 rounded-2xl bg-orange-50 dark:bg-orange-500/10 border border-orange-200 dark:border-orange-500/30 text-orange-700 dark:text-orange-400 text-sm flex items-center justify-between gap-3">
            <span>Complete your profile below to continue to your dashboard.</span>
            <button onClick={() => setShowReturnBanner(false)} className="text-xs underline opacity-70 hover:opacity-100 shrink-0">Dismiss</button>
          </div>
        )}

        {/* HERO BANNER */}
        <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-r ${user.heroGradient} p-8 shadow-2xl`}>
          <div className="absolute -top-20 -right-20 w-80 h-80 bg-white/10 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -bottom-10 -left-10 w-60 h-60 bg-black/10 rounded-full blur-2xl pointer-events-none" />
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center gap-6">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                <span className="px-3 py-1 rounded-full bg-white/20 backdrop-blur-sm text-xs font-bold text-white border border-white/30 uppercase tracking-wider">{user.label}</span>
              </div>
              <div className="flex flex-wrap gap-x-5 gap-y-1.5 text-xs text-white/70 mt-2">
                <span className="flex items-center gap-1.5"><IdCard className="w-3.5 h-3.5" /> {user.id}</span>
