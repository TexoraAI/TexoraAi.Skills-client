...
export default function CompleteProfile({
  ...
  const [profileStage, setProfileStage] = useState(() => {
    return fromSignup ? "onboarding" : "role";
  });
  ...
  const [role, setRole] = useState(() => {
    // Signup se role
    const fromSignup = normalizeStoredRole(location.state?.role);
    ...
    const fromLocal = normalizeStoredRole(
      localStorage.getItem("role")
    );
  
    return fromSignup || fromSession || fromLocal || "";
  });
  ...
  // BUG FIX 3: manual-signup users have no Google credential but DO have a
  // valid JWT (lms_token, set by SignupModal's register()). Persist the quiz
  // answers via /me/onboarding instead of bailing out.
  //
  // IMPORTANT: role is deliberately NOT sent here. SignupModal's register()
  // already set the correct backend Role (STUDENT/TRAINER/BUSINESS). The
  // Google-path mapping below sends "Manager" → "TENANT_ADMIN", which is
  // wrong for manual signup (where the Business role is BUSINESS, not
  // TENANT_ADMIN) — reusing that mapping here would silently corrupt the
  // user's role.
  if (!credential) {
    const existingToken = localStorage.getItem("lms_token");
    if (existingToken) {
      try {
        await authService.saveOnboarding({ onboardingAnswers: answers });
      } catch (err) {
        console.error("❌ Failed to save onboarding (manual signup):", err);
        // swallow — never block navigation on a failed save
      }
    } else {
      console.warn(
        "⚠️ No Google credential and no token — navigating to /ilm-demo",
      );
    }
    if (onSkip) onSkip();
    navigate("/ilm-demo", { replace: true });
