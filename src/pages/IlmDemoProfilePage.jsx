...
/* ── Role-change one-time lock flag ── */
...
const getRoleChangeCount = () => {
  try {
    const u = JSON.parse(localStorage.getItem("lms_user") || "{}");
    return Number(u.roleChangeCount) || 0;
  } catch {
    return 0;
  }
};

const hasUsedRoleChangeFlag = () => getRoleChangeCount() >= MAX_ROLE_CHANGES;
...
/* ══════════════════════════════════════════════════════════════
   SWAP ROLE MODAL — matches Image 1 reference design
══════════════════════════════════════════════════════════════ */
const ROLE_SWAP_OPTIONS = [
  {
    key: "student",
    title: "STUDENT",
    desc: "Access student dashboard and learning tools",
    Icon: GraduationCap,
    color: "#7c3aed",
  },
  {
    key: "trainer",
    title: "TRAINER",
    desc: "Create content, manage learners and classes",
    Icon: Briefcase,
    color: "#2563eb",
  },
  {
    ...
    desc: "Manage organization, teams and partnerships",
    Icon: Building2,
    color: "#F97316",
  },
];
...
const SwapRoleModal = ({ currentRoleKey, remaining, onClose, onConfirm, saving }) => {
  ...
  const currentOption = ROLE_SWAP_OPTIONS.find((r) => r.key === currentRoleKey);
      ...
      >
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
          ...
          <button
            ...
            disabled={saving || !selected || selected === currentRoleKey}
            style={{
              flex: 1,
              padding: "9px",
              borderRadius: 10,
              border: "none",
              background:
                saving || !selected || selected === currentRoleKey
                  ? "#c4b5fd"
...
const ProfileInfoTab = ({
  ...
  const [roleChangeCount, setRoleChangeCount] = useState(getRoleChangeCount());
  const roleLocked = roleChangeCount >= MAX_ROLE_CHANGES;
  const { uploadImage, removeImage, profileImage } = useAvatarContext();
  const fileInputRef = useRef(null);
  const ac = ACCENT[accent];
  ...
  const handleSwapRoleConfirm = async (newRoleKey) => {
    if (roleLocked || newRoleKey === roleKey) return;
    setSwapSaving(true);
    try {
      if (hasAuthToken()) {
        try {
      ...
      const nextCount = roleChangeCount + 1;
      localStorage.setItem("role", newRoleKey.toUpperCase());
      try {
        const cached = JSON.parse(localStorage.getItem("lms_user") || "{}");
        localStorage.setItem(
        ...
        localStorage.setItem(
          "lms_user",
          JSON.stringify({ role: newRoleKey, roleChangeCount: nextCount }),
        );
      }
      ...
      showToast(
        nextCount >= MAX_ROLE_CHANGES
          ...
          : `Role swapped successfully. ${MAX_ROLE_CHANGES - nextCount} change(s) remaining.`,
      );
    } catch (err) {
      console.error("Role swap failed:", err);
      showToast("Failed to swap role. Please try again.", "error");
    } finally {
      setSwapSaving(false);
    }
  };
    ...
    try {
      if (hasAuthToken()) {
        try {
          ...
          console.error("updateMyProfile failed:", apiErr);
        }
      }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 sm:gap-5">
        ...
        <div className="space-y-3.5 min-w-0">
          ...
          {/* ── Role — Swap Role button, up to 3 changes ── */}
          <div className="space-y-1" id="profile-role-field">
            <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 flex items-center gap-1.5">
              Role
              {roleLocked && <Lock className="w-3 h-3 text-gray-400" />}
...
/* ══════════════════════════════════════════════════════════════
   SECURITY TAB — MODIFIED: Change Password card now carries the
   `profile-password-section` id so the "Change Password" navbar
   shortcut can smooth-scroll straight to it.
══════════════════════════════════════════════════════════════ */
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        className="rounded-2xl p-4 sm:p-6 shadow-sm bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10"
      >
        <div className="mb-5">
...
/* ══════════════════════════════════════════════════════════════
   PROFILE PAGE — MAIN.
   MODIFIED: accepts a new `scrollTarget` prop ("role" | "password" |
   null). When set, this page opens on the correct tab (Profile Info
   for "role", Security for "password") and, once the tab content has
   mounted, smooth-scrolls to the exact field/section so the user can
   immediately see where to make the change.
══════════════════════════════════════════════════════════════ */
const IlmDemoProfilePage = ({
  ...
  // "Edit Role" / "Change Password" navbar shortcuts land here with a
