...
/* ─── small style helpers for the filter/report section ─── */
const labelStyle = (t) => ({
  display: "block",
  fontSize: 9,
  fontWeight: 700,
...
/* ─── colorful gradient stat card — same visual language as the
   SuperAdminAssessmentSystem stat cards (solid gradient, white icon
   chip, bold white number). One flat gradient per card = no more
   two-tone / blurred-corner look. ─── */
const GRADIENTS = {
  "#f59e0b": "linear-gradient(135deg,#f59e0b,#ea580c)", // amber → orange
  "#22d3ee": "linear-gradient(135deg,#22d3ee,#0284c7)", // cyan → blue
  "#a78bfa": "linear-gradient(135deg,#a78bfa,#7c3aed)", // violet → purple
  "#34d399": "linear-gradient(135deg,#34d399,#059669)", // emerald → green
  "#f87171": "linear-gradient(135deg,#f87171,#dc2626)", // red
};

function StatCard({ label, value, color, icon: Icon, sub, t }) {
  const gradient = GRADIENTS[color] || `linear-gradient(135deg,${color},${color})`;
  return (
    <div
...
export default function SuperAdminAttendance() {
  ...
  // ─── Reports & Analytics state (superadmin/history + superadmin/download) ───
  const [filterType, setFilterType] = useState("LAST_7_DAYS");
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");
  const [filterBatchId, setFilterBatchId] = useState("");
  ...
  // if a batch is already expanded, refetch its detail whenever the filter changes
  useEffect(() => {
    if (expandedBatchId == null) return;
  ...
  // ─── fetch filtered history + analytics from /superadmin/history ───
  const fetchHistory = async () => {
    setHistoryLoading(true);
    setHistoryError(null);
    try {
  ...
  // ─── trigger Excel download from /superadmin/download ───
  const handleDownload = async () => {
    setDownloading(true);
    setHistoryError(null);
    try {
  ...
  return (
    <>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        @media (max-width: 1024px) {
          .sa-wrap{ padding:16px !important; }
          .sa-stats-grid{ grid-template-columns:repeat(3,1fr) !important; }
        }

        /* ═══════════ RESPONSIVE — small tablet / iPad mini (≤820px) ═══════════ */
        @media (max-width: 820px) {
          .sa-title{ font-size:20px !important; }
          .sa-filters-grid{ grid-template-columns:repeat(2,1fr) !important; }
          .sa-analytics-grid{ grid-template-columns:repeat(3,1fr) !important; }
        }

        /* ═══════════ RESPONSIVE — large phones / small tablets (≤640px) ═══════════ */
        @media (max-width: 640px) {
          .sa-wrap{ padding:12px !important; }
          .sa-title{ font-size:19px !important; }
          .sa-subtitle{ font-size:12px !important; }
          .sa-stats-grid{ grid-template-columns:repeat(2,1fr) !important; gap:8px !important; }
          .sa-analytics-grid{ grid-template-columns:repeat(2,1fr) !important; }
          .sa-filters-grid{ grid-template-columns:1fr 1fr !important; }
          .sa-section-header{ padding:14px 14px !important; flex-wrap:wrap !important; }
          .sa-reports-body{ padding:14px !important; }
          .sa-actions-row{ flex-direction:column !important; align-items:stretch !important; }
          .sa-actions-row button{ width:100% !important; }
          .sa-batch-header{ padding:12px 14px !important; flex-wrap:wrap !important; row-gap:8px !important; }
          .sa-batch-header .sa-batch-info{ flex-basis:100% !important; order:1 !important; }
          .sa-batch-pills{ order:2 !important; margin-left:44px !important; }
          .sa-batch-detail{ padding:0 14px 16px !important; }
          .sa-history-row{ font-size:11px !important; padding:8px 10px !important; }
        }

        /* ═══════════ RESPONSIVE — phones (≤480px), incl. Pixel / iPhone ═══════════ */
        @media (max-width: 480px) {
          .sa-title{ font-size:17px !important; }
          .sa-stats-grid{ grid-template-columns:1fr 1fr !important; gap:8px !important; }
          .sa-filters-grid{ grid-template-columns:1fr !important; }
          .sa-analytics-grid{ grid-template-columns:1fr 1fr !important; }
          .sa-stat-card{ padding:10px 12px !important; }
          .sa-batch-pills{ margin-left:0 !important; }
          .sa-modal{ padding:16px !important; }
        }

        /* ═══════════ RESPONSIVE — very small phones (≤360px) ═══════════ */
        @media (max-width: 360px) {
          .sa-stats-grid{ grid-template-columns:1fr !important; }
          .sa-analytics-grid{ grid-template-columns:1fr !important; }
        }
      `}</style>
      ...
      >
        ...
        >
          {/* HEADING — plain, compact style matching the rest of the admin panel */}
          ...
          >
            ...
            <div className="sa-reports-body" style={{ padding: 20 }}>
              <div
                className="sa-filters-grid"
                style={{
                  display: "grid",
                  gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))",
                  gap: 10,
                ...
                >
                  {historyData.records.map((r, i) => (
                    ...
                    >
                      ...
                      <span style={{ color: t.textSub, wordBreak: "break-all" }}>
                        {r.studentEmail}
                      </span>
                      <span style={{ color: t.textMuted, wordBreak: "break-all" }}>
                        trainer: {r.trainerEmail}
                      </span>
                      <span style={{ marginLeft: "auto", color: t.textSub }}>
                        {r.status}
                  ...
                  >
                    ...
                    >
                      {isOpen ? (
                        ...
                        <ChevronRight size={15} color={t.textMuted} style={{ flexShrink: 0 }} />
                      )}
                      <div
                        style={{
                          width: 34,
                      ...
                      <div className="sa-batch-info" style={{ flex: 1, minWidth: 0 }}>
                        <p
                          style={{
                            fontSize: 13,
                            fontWeight: 700,
