...
const inputCls =
  "w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-[#F97316] focus:outline-none transition text-[#1E293B] placeholder-gray-400 disabled:opacity-50 bg-white";
...
function CoursesList({ onBack }) {
  ...
  const handleDelete = async (courseId) => {
    try {
      ...
      setCourses((prev) =>
        prev.filter((c) => c.courseId !== courseId && c.id !== courseId),
      );
      setConfirmId(null);
    } catch (err) {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#1E293B]">My Courses</h1>
          ...
          <button
            onClick={onBack}
            className="flex items-center gap-2 px-4 py-2 border-2 border-gray-200 text-[#1E293B] font-semibold rounded-xl hover:bg-gray-50 transition"
          >
        ...
        {!loading && courses.length === 0 && !error && (
          <div className="text-center py-16 bg-white rounded-2xl border-2 border-gray-100">
            ...
            <p className="text-sm text-gray-400 mt-1">
              Click "Upload New" to add your first course.
            </p>
          </div>
        )}
          ...
          <div className="space-y-4">
            {courses.map((course) => {
              ...
              return (
                <div
                  key={id}
                  className="bg-white rounded-2xl border-2 border-gray-100 p-5 flex items-start gap-4 shadow-sm hover:shadow-md transition"
                >
                  {/* Icon */}
                  ...
                  {/* Info */}
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-[#1E293B] truncate text-base">
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {course.learnersCount != null && (
                        <span className="text-xs bg-orange-50 text-[#F97316] px-2 py-0.5 rounded-full font-semibold">
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2">
                    {!isConfirming ? (
                      <button
                        ...
                        disabled={isDeleting}
                        className="w-9 h-9 flex items-center justify-center rounded-xl border-2 border-red-100 text-red-400 hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition"
                        title="Delete course"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          disabled={isDeleting}
                          className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-lg transition flex items-center gap-1 disabled:opacity-60"
                        >
                          {isDeleting ? (
...
export default function AdminCourseUploadForm({ onSubmit, onClose }) {
  ...
  // ── Show list view ────────────────────────────────────────────────
  if (view === "list") {
    return <CoursesList onBack={() => setView("form")} />;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const filteredPoints = learnPoints.filter((p) => p.title.trim());
    formData.append("learnPoints", JSON.stringify(filteredPoints));

    try {
      await videoService.uploadAdminCourse(formData, (pct) =>
      ...
      setErrorMsg(
        typeof msg === "string" ? msg : "Upload failed. Check console.",
      );
      setStatus("error");
    }
  };
      ...
      <div className="min-h-screen bg-[#F6EDE6] flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-12 max-w-md w-full text-center">
          ...
          <p className="text-gray-500 mb-8">
            "{videoTitle}" has been successfully published.
          </p>
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="px-6 py-3 border-2 border-gray-200 text-[#1E293B] font-semibold rounded-xl hover:bg-gray-50 transition"
              >
                Close
              </button>
            )}
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => setView("list")}
              className="flex items-center gap-2 px-4 py-2 border-2 border-[#F97316] text-[#F97316] font-semibold rounded-xl hover:bg-[#F97316] hover:text-white transition text-sm"
            >
              <List className="w-4 h-4" /> My Courses
            </button>
            {onClose && (
              <button
                onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Section icon={Film} number="2" title="Main Video Card">
            ...
            <Field
              label="Video File"
              required
              hint="Upload a file or provide a URL — choose one"
            >
              <div className="space-y-3">
                {!videoFile ? (
                  <div
                    ...
                    className={`border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${
                      dragOver
                        ...
                        : "border-gray-300 hover:border-[#F97316] hover:bg-orange-50/30"
                    }`}
                  >
                    <Upload
                      className={`w-8 h-8 mx-auto mb-2 ${dragOver ? "text-[#F97316]" : "text-gray-400"}`}
                    />
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Show a badge at the top-right corner of the video player
                </p>
              </div>
              <button
                type="button"
                onClick={() => setShowInstructorLive(!showInstructorLive)}
                className={`w-12 h-6 rounded-full transition-all relative ${
                  showInstructorLive ? "bg-[#F97316]" : "bg-gray-300"
                }`}
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${
                    showInstructorLive ? "left-[26px]" : "left-0.5"
                  }`}
                />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <Field label="Instructor Role">
                <input
                  ...
                  placeholder="e.g. Software Engineer & Instructor"
                  className={inputCls}
                />
              </Field>
              <Field label="Experience" hint="e.g. 10+ years">
          ...
          <Section icon={BookOpen} number="4" title="Description Section">
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Show a "Show more" button below the description
                </p>
              </div>
              <button
                type="button"
                ...
                className={`w-12 h-6 rounded-full transition-all relative ${
                  showMoreEnabled ? "bg-[#F97316]" : "bg-gray-300"
                }`}
              >
                <span
                  className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all ${
                    showMoreEnabled ? "left-[26px]" : "left-0.5"
                  }`}
                />
              </button>
            </div>
                ...
                >
                  ...
                  <div className="flex-1 space-y-2">
                    <input
                      value={point.title}
                      onChange={(e) =>
                        updateLearnPoint(point.id, "title", e.target.value)
                      }
                      placeholder={`Point ${idx + 1} Title...`}
                      className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-[#F97316] focus:outline-none text-sm font-semibold text-[#1E293B] placeholder-gray-400"
                    />
                    <input
                      value={point.desc}
                      onChange={(e) =>
                        updateLearnPoint(point.id, "desc", e.target.value)
                      }
                      placeholder="Short description (optional)..."
                      className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-[#F97316] focus:outline-none text-sm text-gray-600 placeholder-gray-400"
                    />
