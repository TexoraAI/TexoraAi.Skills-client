...
// ─── Global Design System — typography (Poppins) + the shared purple
// accent (ACCENT_PURPLE) used for primary actions everywhere else
// (Attendance.jsx / AdminDashboard.jsx / AdminBatches.jsx). No logic
// changes — only fonts/colors.
import { FONT_FAMILY, FONT_IMPORT, ACCENT_PURPLE } from "@/design-system";
...
const CreateBatchModal = ({ onClose, onSuccess, inline = false }) => {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <Input
          ...
          onChange={(e) => setForm({ ...form, batchName: e.target.value })}
          placeholder="Enter batch name"
          className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-violet-500 focus:border-transparent"
        />
      </div>
      ...
      <div>
        ...
        <select
          value={form.branchId}
          onChange={(e) => setForm({ ...form, branchId: e.target.value })}
          className="w-full rounded-xl px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-violet-500 focus:outline-none"
        >
          <option value="">Select branch</option>
          {Array.isArray(branches) &&
            branches.map((b) => (
      ...
      <div className="mt-6 flex justify-end gap-3 pt-2">
        ...
        <button
          onClick={handleSubmit}
          disabled={loading}
          className="px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-violet-600 to-purple-600 text-white hover:opacity-90 transition-all disabled:opacity-60"
        >
          {loading ? "Creating..." : "Create"}
        </button>
      </div>
  ...
  if (inline) {
    return (
      <>
        ...
        <div className="p-5 bg-white dark:bg-slate-900" style={{ fontFamily: FONT_FAMILY }}>{formContent}</div>
        {limitModal}
      </>
    );
  }
  ...
  return (
    <>
      <style>{FONT_IMPORT}</style>
      <div className="fixed inset-0 z-40 bg-black/60" onClick={onClose} />
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ fontFamily: FONT_FAMILY }}>
        <div className="w-full max-w-md rounded-xl p-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 shadow-2xl">
          <div className="flex items-center justify-between mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">
            <h2 className="text-lg font-semibold">Create Batch</h2>
            <button
