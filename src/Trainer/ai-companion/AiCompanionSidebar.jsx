...
export default function AiCompanionSidebar({
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          padding: collapsed ? "18px 8px 12px" : "18px 16px 12px",
          borderBottom: `1px solid ${border}`,
          display: "flex",
          alignItems: "center",
          justifyContent: collapsed ? "center" : "flex-start",
      ...
      >
        ...
        >
          <Menu size={15} color={textSecondary} />
        </button>
      </div>

      {/* Nav */}
      <div style={{ padding: collapsed ? "10px 6px 4px" : "10px 8px 4px", flexShrink: 0 }}>
        <button
          onClick={() => {
            if (onNewChat) onNewChat();
            onNavigate("home");
        ...
        <button
          onClick={() => {
            onNavigate("workflows");
            if (!collapsed) setWorkflowOpen(true); // always open when clicking parent
          }}
        ...
        >
          <Zap
            ...
            color={isWorkflowActive ? activeColor : textSecondary}
          />
          {!collapsed && <span style={{ flex: 1 }}>Workflows</span>}
          {/* Chevron toggles the submenu without navigating */}
          {!collapsed && (
            <span
              onClick={(e) => {
