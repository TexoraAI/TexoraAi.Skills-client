/**
 * TrainerMeetings.jsx
 * ILMORA Meetings — Trainer workspace (frontend only, mock data, no API/LiveKit calls)
 *
 * ── Nav integration (paste into your real files) ─────────────────────────────
 * Sidebar.jsx  (trainerMenus, right after the Dashboard entry):
 *   { name: "Meetings", path: "/trainer/meetings", icon: Video }
 *
 * App.jsx:
 *   const TrainerMeetings = lazyLoad(() => import("./Trainer/Meetings.jsx"));
 *   ...inside the /trainer nested <Route> block...
 *   <Route path="meetings" element={<TrainerMeetings />} />
 * ──────────────────────────────────────────────────────────────────────────────
 *
 ...
 *   --accent-2:#8b5cf6   (violet — "Batches live now", Trainer-only 4th stat)
...
function SessionRow({ s, onJoin, onViewDetails, onDelete }) {
  ...
  //   const evt = { title: s.title, start, end, details: `${s.batch}` };
  //   return (
  ...
  //       </div>
  const evt = { title: s.title, start, end, details: `${s.creatorEmail}` };
...
export default function TrainerMeetings() {
  ...
  return (
    <div
      className="ilmora-trainer-meetings"
      ...
      <div className="page-inner">
        <header className="page-header">
          <div className="brand-lockup">
            ...
            <div>
              ...
              <div className="subtitle">Trainer workspace</div>
            </div>
          </div>
        ...
        <section className="hero">
          <div className="hero-copy">
            ...
            <h1>Start class, your batch is waiting.</h1>
            <p>
              Create a live session, share the link with your batch, track who's
              in the room.
            </p>
            ...
            <div className="pill-row">
              ...
              4 batches · 128 students · All clear ✓
            </div>
          </div>
        </section>
...
const STYLES = `
.ilmora-trainer-meetings {
  --brand: #2563eb; --brand-2: #6366f1; --live: #14b8a6; --accent: #f59e0b; --accent-2: #8b5cf6;
...
.ilmora-trainer-meetings[data-dark] { --bg: #000000; --card: #101319; --text: #f1f5f9; --muted: #94a3b8; --border: rgba(255,255,255,.08); }
...
.carousel-stage { position: relative; flex: 1; height: 360px; border-radius: 24px; overflow: hidden; background: linear-gradient(135deg, rgba(37,99,235,.06), rgba(139,92,246,.08)); border: 1px solid var(--border); }
...
.illo--attendance { background: linear-gradient(160deg, rgba(139,92,246,.16), rgba(37,99,235,.08)); }
...
.illo--attendance .illo-badge { background: linear-gradient(135deg, var(--accent-2), var(--brand-2)); box-shadow: 0 8px 18px rgba(139,92,246,.3); }
...
.tone-violet .stat-icon { background: rgba(139,92,246,.16); color: #6d28d9; }
...
.tone-violet .stat-underline { background: linear-gradient(90deg, var(--accent-2), var(--brand-2)); }
