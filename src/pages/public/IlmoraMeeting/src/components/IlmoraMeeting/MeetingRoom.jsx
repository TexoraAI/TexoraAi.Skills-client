...
import { useAudioTrackHealth } from "./hooks/useAudioTrackHealth";
...
export function MeetingRoom({
  ...
  const handleAudioTrackDead = useCallback(() => {
    console.error("⚠️ Audio track died, needs recovery");
    setMediaError(
      "Your microphone stopped working. Click the mic button to re-enable it.",
    );
    setMicOn(false);
  }, []);

  useAudioTrackHealth(localMicRef.current, connected, handleAudioTrackDead);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        if (!audioTrack) {
          console.error(
            "❌ Audio track creation failed - permission denied or no device",
          );
          setMicOn(false);
          setMediaError(
            "🎤 Microphone access was blocked. Check your browser's site settings, then click the mic button below to try again.",
          );
        }
        if (!videoTrack) {
          console.warn("⚠️ Video track creation failed - camera not available");
          setCamOn(false);
        }
        ...
        camReadyPromiseRef.current = (async () => {
          if (videoTrack) {
            ...
            if (initialAV && initialAV.camOn === false) await videoTrack.mute();
          }
        })();
        ...
        if (audioTrack) {
          try {
            ...
            audioTrack.on("muted", () => {
              console.warn("⚠️ Audio track was muted externally");
              setMediaError(
                "Your microphone was unexpectedly muted. Click the mic button to re-enable.",
              );
            });
            ...
            if (initialAV && initialAV.micOn === false) {
            ...
            setMicOn(false);
            setMediaError(
              "Failed to enable microphone. Check permissions and try again.",
            );
      ...
      {mediaError && (
        <div style={{ padding: "8px 16px 0 16px" }}>
          <AudioPermissionAlert
            error={mediaError}
            onRetryMic={toggleMic}
            onRetryAudio={() => setMediaError(null)}
          />
        </div>
      )}
