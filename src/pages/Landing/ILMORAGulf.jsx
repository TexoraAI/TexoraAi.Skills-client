...
import {
  ArrowRight, Star, Users, Award, TrendingUp, ChevronDown,
  Globe, Clock, BookOpen, MapPin, Phone,
  CheckCircle2, Zap, Video, Wifi, Monitor, Building2,
  GraduationCap, Target, BarChart2, Mic,
} from "lucide-react";

// ✅ Same shared shell used by every other public page (Careers, About,
// Pricing, Contact, FAQ, etc). Guarantees ILM ORA Gulf gets the exact same
// AnnouncementBanner → Navbar → ... → Footer as the rest of the site, with
// identical alignment/spacing — no more custom, duplicate navbar.
...
/* ─────────────────────────────────────────────────────────────
   ✅ ALIGNMENT FIX: everything scoped under `.gulf-page` instead
   of bare `*` selectors. The old global reset (`*,*::before,*::
   after{margin:0;padding:0}`) leaked outside this component and
   stripped margin/padding off PublicLayout's shared navbar and
   footer. The old custom Navbar (with MegaMenu / mobile Sheet)
   has also been removed entirely — PublicLayout already renders
   the exact same navbar + footer used by Careers and every other
   public page.
───────────────────────────────────────────────────────────── */
const GLOBAL = `
  .gulf-page, .gulf-page *, .gulf-page *::before, .gulf-page *::after{box-sizing:border-box;}
  .gulf-page h1,.gulf-page h2,.gulf-page h3,.gulf-page h4,
  .gulf-page p,.gulf-page ul,.gulf-page ol,
  .gulf-page button,.gulf-page span,.gulf-page div,.gulf-page a{margin:0;padding:0;}
`;
...
═══════════════════════════════════════════════════════════ */
export default function ILMORAGulf({ theme, toggleTheme, setShowLoginModal, scrollToSection }) {
  ...
  return (
    ...
    >
      <style>{GLOBAL}</style>
      <div className="gulf-page" style={{ fontFamily: F, background: BG, minHeight: "100vh", color: TP, transition: "background .3s,color .3s" }}>
