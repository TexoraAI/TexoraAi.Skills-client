...
const Attendance = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try { const r = await videoService.getTrainerBatches(); setBatches(r.data || []); }
  ...
  const submitAttendance = async () => {
    try {
      ...
      await attendanceService.markAttendance({
        ...
        attendances: students.map(s => ({ studentUserId: s.userId, studentEmail: s.email, status: s.status })),
      });
      setShowSuccess(true);
          ...
          <div className="at-tbl-wrap">
            ...
            <div className="at-toolbar">
              ...
              <div className="at-tr">
                ...
                {["PRESENT", "ABSENT", "LATE"].map(s => (
                  <button key={s} className="at-mpill"
            ...
            <div>
              {filtered.length === 0 ? (
                <div style={{ padding: "40px 20px", textAlign: "center", color: "var(--mu)", fontSize: 13, fontWeight: 600 }}>
                ...
                return (
                  <div key={s.userId} className="at-row">
                    <div className="at-rav" style={{ background: `${cfg.dot}cc` }}>
            ...
            <div className="at-foot">
              ...
              <button className="at-sbtn" onClick={submitAttendance} disabled={saving}>
                {saving ? (
                  <><div style={{ width: 14, height: 14, border: "2px solid rgba(255,255,255,.4)", borderTopColor: "white", borderRadius: "50%", animation: "at-spin 0.8s linear infinite" }} /> Saving…</>
                ) : (
                  <><CheckCircle size={14} /> Submit Attendance ({students.length})</>
                )}
              </button>
            </div>
