...
// redeclare tokens or components that already live there. AdminDashboard.jsx
// is the Golden Reference this page now visually matches.
import {
  T,
  FONT_FAMILY,
  FONT_WEIGHT,
...
/* ══════════════════════════════════════════════════════════════════════════════
   ...
   (helpers, API utilities, matching logic — identical to the original file)
══════════════════════════════════════════════════════════════════════════════ */
function fmtN(v) { return typeof v === "number" ? Math.round(v) : 0; }
...
/* ══════════════════════════════════════════════════════════════════════════════
   PAGE-LOCAL, TOKEN-DRIVEN PRIMITIVES
   No color/spacing/radius/typography values are invented here — everything is
   sourced from the shared theme object (t) or the FONT_FAMILY / FONT_WEIGHT /
   FONT_SIZE / RADIUS / CARD_PADDING / ACCENT_PURPLE tokens, exactly the way
   AdminDashboard.jsx's SectionCard / SectionHeader / IconBadge / EmptyBlock
   are page-local but token-driven.
══════════════════════════════════════════════════════════════════════════════ */
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div>
          ...
          {sub && (
            <div style={{ fontSize: 11, color: t.textMuted, fontFamily: FONT_FAMILY, marginTop: 2 }}>
              {sub}
            </div>
          )}
        </div>
...
function EmptyBlock({ t, icon: Icon, title, sub }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "28px 0", gap: 12, textAlign: "center" }}>
      <div style={{
      ...
      <p style={{ fontSize: 13, color: t.textMuted, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY, margin: 0 }}>{title}</p>
      {sub && <p style={{ fontSize: 12, color: t.textMuted, fontFamily: FONT_FAMILY, margin: 0 }}>{sub}</p>}
    </div>
  );
}
...
function Pill({ children, tone = "neutral", t }) {
  const styles = {
    neutral: { background: t.pillBg, border: `1px solid ${t.pillBorder}`, color: t.pillText },
    accent:  { background: `${ACCENT_PURPLE.base}14`, border: `1px solid ${ACCENT_PURPLE.base}30`, color: ACCENT_PURPLE.base },
  };
  return (
    <span style={{
      ...
      fontFamily: FONT_FAMILY, whiteSpace: "nowrap", ...styles[tone],
    }}>{children}</span>
  );
}

/** Percentage status pill — color is data-driven (semantic), not a theme token */
function SPill({ value, style = {} }) {
  const pc = pillColors(value);
  return (
    <span style={{
      display: "inline-flex", alignItems: "center", fontSize: 11, padding: "4px 12px",
      borderRadius: RADIUS.pill, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
...
function Skel({ height = 200, t }) {
  return (
    <div style={{ height, borderRadius: RADIUS.standardCard, marginBottom: 16, background: t.barBg, animation: "org-pulse 1.5s ease infinite" }} />
  );
}
...
function ErrBox({ msg, t }) {
  return (
    <div style={{
      ...
      borderRadius: RADIUS.standardCard, padding: "1rem 1.25rem",
      color: "#ef4444", fontFamily: FONT_FAMILY, fontSize: 13, marginBottom: 16,
    }}>{msg}</div>
  );
}

function BackLink({ label, onClick, t }) {
  return (
    <button
      onClick={onClick}
      style={{
        display: "inline-flex", alignItems: "center", gap: 6,
        fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: t.textMuted,
        background: "none", border: "none", cursor: "pointer", padding: "6px 0",
        marginBottom: 16, fontFamily: FONT_FAMILY, lineHeight: 1,
      }}
    >
      <ArrowLeft size={13} style={{ flexShrink: 0 }} />
      {label}
...
function PBar({ value, color, t }) {
  return (
    <div style={{ flex: 1, background: t.barBg, borderRadius: RADIUS.pill, height: 6, overflow: "hidden" }}>
      <div style={{
        width: `${Math.min(value || 0, 100)}%`, height: 6, borderRadius: RADIUS.pill,
        background: color, transition: "width .7s cubic-bezier(0.4,0,0.2,1)",
      }} />
    </div>
  );
}
...
function CBadge({ icon: Icon, label, count, color, t }) {
  return (
    <div style={{
      ...
      borderRadius: RADIUS.pill, padding: "6px 12px", lineHeight: 1,
    }}>
      <Icon size={12} color={color} style={{ flexShrink: 0 }} />
      <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.bold, color, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{count}</span>
      <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{label}</span>
    </div>
  );
}
...
function MoreLabel({ count, t }) {
  return (
    <p style={{ fontSize: 10, color: t.textMuted, textAlign: "center", margin: "6px 0 0", fontFamily: FONT_FAMILY }}>
      +{count} more
    </p>
  );
}
...
function CRow({ icon: Icon, color, title, sub, t }) {
  return (
    <div style={{
      ...
      borderRadius: RADIUS.chip, background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, marginBottom: 6,
    }}>
      <IconBadge icon={Icon} color={color} size={30} iconSize={14} />
      <span style={{
        fontSize: 13, color: t.text, flex: 1, overflow: "hidden", textOverflow: "ellipsis",
        whiteSpace: "nowrap", fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.semibold, lineHeight: 1.3,
      }}>{title}</span>
      {sub && (
        <span style={{ fontSize: 11, color: t.textMuted, flexShrink: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{sub}</span>
      )}
    </div>
  );
}
...
function MetricTile({ label, value, color, t }) {
  return (
    <div style={{
      background: t.cardBg, border: `1px solid ${t.border}`,
      borderRadius: RADIUS.standardCard, padding: "16px 18px", boxShadow: t.shadow,
    }}>
      <p style={{
        fontSize: 10, color: t.textMuted, textTransform: "uppercase",
        letterSpacing: LETTER_SPACING.eyebrowWide, margin: "0 0 8px", fontWeight: FONT_WEIGHT.bold,
        fontFamily: FONT_FAMILY, lineHeight: 1,
      }}>{label}</p>
      <p style={{ fontSize: 24, fontWeight: FONT_WEIGHT.heroTitle, color: color || t.text, margin: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>
        {value}
      </p>
    </div>
  );
...
/* ──────────────────────────────────────────────────────────────────────────────
   ARC METER  (theme-aware track via t.border, semantic value color as prop)
────────────────────────────────────────────────────────────────────────────── */
function ArcMeter({ value, color, t, size = 100 }) {
  const r = 38, cx = 50, cy = 52;
  const circ = Math.PI * r;
  const progress = (value / 100) * circ;
  return (
    <svg viewBox="0 0 100 60" style={{ width: size, height: size * 0.6 }}>
      ...
      <text x={cx} y={cy - 4} textAnchor="middle" fontSize="13" fontWeight="700"
        fill={color} fontFamily={FONT_FAMILY}>{fmtN(value)}%</text>
    </svg>
  );
}

/* ──────────────────────────────────────────────────────────────────────────────
   RADAR CHART  (theme-aware grid/axis lines via t.border)
────────────────────────────────────────────────────────────────────────────── */
function RadarChart({ data, t }) {
  ...
  return (
    <svg viewBox="0 0 280 260" style={{ width: "100%", maxWidth: 280 }}>
      <defs>
        <radialGradient id="rfill-or" cx="50%" cy="50%" r="50%">
          <stop offset="0%"   stopColor={ACCENT_PURPLE.base} stopOpacity="0.22" />
          <stop offset="100%" stopColor={ACCENT_PURPLE.base} stopOpacity="0.04" />
        </radialGradient>
      </defs>
      {[20, 40, 60, 80, 100].map(ring => (
      ...
      {axes.map((_, i) => {
        const [x2, y2] = pt(i, 100);
        return <line key={i} x1={cx} y1={cy} x2={x2} y2={y2} stroke={t.border} strokeWidth="1" />;
      })}
      <polygon points={poly} fill="url(#rfill-or)" stroke={ACCENT_PURPLE.base} strokeWidth="2" />
      {axes.map((a, i) => {
        const [x, y] = pt(i, a.val);
        return <circle key={i} cx={x} cy={y} r={4} fill={ACCENT_PURPLE.base} />;
      })}
      {axes.map((a, i) => {
        const [x, y] = pt(i, 118);
        return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle"
          fontSize="10" fill={t.textMuted} fontFamily={FONT_FAMILY}>{a.label}</text>;
      })}
    </svg>
  );
}
...
function StudentDetailPanel({ student, batchId, onBack, t }) {
  ...
  const d = detail || student;
  const arcMeters = [
    { label: "Videos",      val: d.videoWatchPercentage,          color: "#22d3ee", sub: `${d.videosWatched??0}/${d.totalVideos??0}` },
    { label: "Files",       val: d.fileDownloadPercentage,        color: "#38BDF8", sub: `${d.filesDownloaded??0}/${d.totalFiles??0}` },
    { label: "Quizzes",     val: d.quizCompletionPercentage,      color: "#A78BFA", sub: `${d.quizzesCompleted??0}/${d.totalQuizzes??0}` },
    { label: "Assignments", val: d.assignmentCompletionPercentage,color: "#FB923C", sub: `${d.assignmentsCompleted??0}/${d.totalAssignments??0}` },
    { label: "Course",      val: d.courseProgressPercentage,      color: "#FBBF24", sub: `${d.courseContentCompleted??0}/${d.totalCourseContent??0}` },
  ...
  const breakdownBars = [
    { label: "Video watch",  val: d.videoWatchPercentage,          color: "#22d3ee" },
    { label: "File download",val: d.fileDownloadPercentage,        color: "#38BDF8" },
    { label: "Quiz",         val: d.quizCompletionPercentage,      color: "#A78BFA" },
    { label: "Assignments",  val: d.assignmentCompletionPercentage,color: "#FB923C" },
    { label: "Course",       val: d.courseProgressPercentage,      color: "#FBBF24" },
  ...
  return (
    <div>
      <BackLink label="Back to batch students" onClick={onBack} t={t} />
      {loading ? <Skel height={440} t={t} /> : (
        <SectionCard t={t}>
          {/* Banner */}
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
            <div style={{
              width: 52, height: 52, borderRadius: "50%", flexShrink: 0,
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 18, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
              background: ACCENT_PURPLE.base, color: "#ffffff", lineHeight: 1,
            }}>{initials(d.studentEmail)}</div>
            <div>
              <h2 style={{ fontFamily: FONT_FAMILY, fontSize: 18, fontWeight: FONT_WEIGHT.bold, margin: 0, color: t.text, lineHeight: 1.2 }}>{d.studentEmail}</h2>
              <p style={{ fontSize: 12, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY, lineHeight: 1 }}>
                Batch <strong style={{ color: t.textSub }}>{d.batchId || batchId}</strong>
              </p>
            </div>
            <div style={{
              ...
              border: `2px solid ${progColor(d.overallProgressPercentage)}44`,
              borderRadius: RADIUS.standardCard, padding: "12px 22px", background: t.recentItemBg,
            }}>
              <span style={{ fontFamily: FONT_FAMILY, fontSize: 30, fontWeight: FONT_WEIGHT.heroTitle, lineHeight: 1, color: progColor(d.overallProgressPercentage) }}>
                {fmtN(d.overallProgressPercentage)}%
              </span>
              <span style={{ fontSize: 9, color: t.textMuted, marginTop: 4, letterSpacing: LETTER_SPACING.eyebrowWide, textTransform: "uppercase", fontFamily: FONT_FAMILY, lineHeight: 1 }}>overall</span>
            </div>
          </div>

          {/* Arc meter row */}
          <div className="org-arc-grid" style={{ marginBottom: 20 }}>
            {arcMeters.map(m => (
              <div key={m.label} style={{
                background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`,
                borderRadius: RADIUS.chip, padding: "12px 8px 10px", textAlign: "center",
                display: "flex", flexDirection: "column", alignItems: "center",
              }}>
                <ArcMeter value={m.val || 0} color={m.color} t={t} size={90} />
                <p style={{ fontSize: 10, fontWeight: FONT_WEIGHT.bold, color: t.textMuted, margin: "4px 0 2px", textTransform: "uppercase", letterSpacing: LETTER_SPACING.eyebrow, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{m.label}</p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{m.sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="org-two-col">
            <div style={{ background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, borderRadius: RADIUS.chip, padding: 18 }}>
              <SectionHeader t={t} title="Progress Breakdown" />
              {breakdownBars.map(m => (
                <div key={m.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  <span style={{ fontSize: 11, color: t.textMuted, width: 105, flexShrink: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{m.label}</span>
                  <PBar value={m.val} color={m.color} t={t} />
                  <span style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: m.color, width: 40, textAlign: "right", fontFamily: FONT_FAMILY, lineHeight: 1 }}>{fmtN(m.val)}%</span>
                </div>
              ))}
            </div>
            <div style={{
              background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, borderRadius: RADIUS.chip,
              padding: 18, display: "flex", flexDirection: "column", alignItems: "center",
            }}>
              <div style={{ alignSelf: "flex-start", marginBottom: 8 }}>
                <SectionHeader t={t} title="Skill Radar" />
              </div>
              <RadarChart data={d} t={t} />
            </div>
          </div>
        </SectionCard>
      )}
...
function TrainerDetail({ trainer, onBack, t }) {
  ...
  return (
    <div>
      <BackLink label="Back to all trainers" onClick={onBack} t={t} />

      {/* Trainer hero card */}
      <SectionCard t={t} style={{
        display: "flex", alignItems: "center", justifyContent: "space-between",
        flexWrap: "wrap", gap: 14, marginBottom: 16,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{
            width: 52, height: 52, borderRadius: RADIUS.chip, flexShrink: 0,
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 16, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
            background: AV_BG[0], color: AV_FG[0], lineHeight: 1,
          }}>{initials(trainerEmail)}</div>
          <div>
            <h2 style={{ fontFamily: FONT_FAMILY, fontSize: 18, fontWeight: FONT_WEIGHT.bold, margin: 0, color: t.text, lineHeight: 1.2 }}>{trainerEmail}</h2>
            <p style={{ fontSize: 12, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY, lineHeight: 1 }}>{trainer.displayName || "Trainer"}</p>
          </div>
        </div>
        {progressReport && <SPill value={progressReport.avgOverallProgressPercentage} style={{ fontSize: 12, padding: "5px 16px" }} />}
      </SectionCard>
      ...
      {/* Content stat badges */}
      <div className="org-badge-row" style={{ marginBottom: 16 }}>
        {contentLoading
          ? [1,2,3,4,5,6].map(i => <div key={i} style={{ width: 90, height: 32, borderRadius: RADIUS.pill, background: t.barBg, animation: "org-pulse 1.5s ease infinite" }} />)
          : <>
            <CBadge icon={Video}        label="Videos"      count={content.videos.length}      color="#f43f5e" t={t} />
            <CBadge icon={FolderOpen}   label="Files"       count={content.files.length}       color="#2dd4bf" t={t} />
            <CBadge icon={BookOpen}     label="Courses"     count={content.courses.length}     color="#c084fc" t={t} />
      ...
      {/* Tab bar */}
      <div className="org-tabs" style={{
        background: t.pillBg, border: `1px solid ${t.pillBorder}`,
        borderRadius: RADIUS.chip, padding: 4, width: "fit-content", marginBottom: 16,
      }}>
        {tabs.map(({ key, label }) => (
          <button key={key} onClick={() => setActiveTab(key)} style={{
            padding: "8px 18px", borderRadius: RADIUS.button, fontSize: 12, fontWeight: FONT_WEIGHT.bold,
            cursor: "pointer", border: "none", transition: "all .2s", fontFamily: FONT_FAMILY, lineHeight: 1,
            background: activeTab === key ? ACCENT_PURPLE.base : "transparent",
            color: activeTab === key ? "#ffffff" : t.textMuted,
          }}>{label}</button>
        ))}
      </div>
        ...
        <>
          ...
          {!progressLoading && !progressError && progressReport && (
            <>
              ...
              {/* Breakdown + Score tiles */}
              <div className="org-two-col" style={{ marginBottom: 16 }}>
                <SectionCard t={t} style={{ margin: 0 }}>
                  <SectionHeader t={t} title="Progress Breakdown" />
                  {areas.map(a => (
                    <div key={a.label} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                      <span style={{ fontSize: 11, color: t.textMuted, width: 110, flexShrink: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{a.label}</span>
                      <PBar value={a.val} color={a.color} t={t} />
                      <span style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, width: 38, textAlign: "right", fontFamily: FONT_FAMILY, lineHeight: 1 }}>{fmtN(a.val)}%</span>
                    </div>
                  ))}
                </SectionCard>
                <SectionCard t={t} style={{ margin: 0 }}>
                  <SectionHeader t={t} title="Score Tiles" />
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                    {areas.map(a => (
                      <div key={a.label} style={{
                        flex: "1 1 80px", background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`,
                        borderRadius: RADIUS.chip, padding: 12, textAlign: "center",
                        display: "flex", flexDirection: "column", alignItems: "center", gap: 4,
                      }}>
                        <div style={{ fontSize: 20, fontWeight: FONT_WEIGHT.heroTitle, color: a.color, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{fmtN(a.val)}%</div>
                        <div style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{a.label}</div>
                      </div>
                    ))}
                  </div>
                </SectionCard>
              ...
              {/* Batch list */}
              <SectionCard t={t}>
                <SectionHeader t={t} title={`Batches (${progressReport.batchReports?.length || 0})`} />
                {(progressReport.batchReports || []).map((batch, bi) => (
                  <div key={batch.batchId} style={{
                    display: "flex", alignItems: "center", justifyContent: "space-between",
                    flexWrap: "wrap", gap: 12, padding: "12px 0",
                    borderBottom: bi < (progressReport.batchReports.length - 1) ? `1px solid ${t.border}` : "none",
                  }}>
                    <div>
                      <p style={{ fontSize: 13, fontWeight: FONT_WEIGHT.semibold, margin: 0, color: t.text, fontFamily: FONT_FAMILY, lineHeight: 1.2 }}>Batch {batch.batchId}</p>
                      <p style={{ fontSize: 12, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY, lineHeight: 1 }}>{batch.totalStudents} students</p>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                      <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 130 }}>
                        {[
                          ...
                          <div key={a.label} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            <span style={{ fontSize: 10, color: t.textMuted, width: 32, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{a.label}</span>
                            <div style={{ width: 72, background: t.barBg, borderRadius: RADIUS.pill, height: 5 }}>
                              <div style={{ width: `${Math.min(a.val, 100)}%`, height: 5, borderRadius: RADIUS.pill, background: a.color, transition: "width 0.6s ease" }} />
                            </div>
                            <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY, lineHeight: 1 }}>{fmtN(a.val)}%</span>
                          </div>
                        ))}
                      </div>
                      <SPill value={batch.avgOverallProgressPercentage} />
          ...
          {contentLoading ? <Skel height={340} t={t} /> : (
            <>
              <SectionCard t={t} style={{ marginBottom: 16 }}>
                <SectionHeader t={t} icon={Video} color="#f43f5e" title={`Videos (${content.videos.length})`} />
                {content.videos.length === 0 ? <EmptyBlock icon={Video} title="No videos found" t={t} />
                  : content.videos.slice(0, 6).map((v, i) => (
                    <CRow key={v.id || i} icon={Video} color="#f43f5e" t={t}
                      title={v.title || v.fileName || v.videoTitle || v.originalFileName || "Untitled Video"}
                      sub={v.uploadedAt ? new Date(v.uploadedAt).toLocaleDateString() : v.createdAt ? new Date(v.createdAt).toLocaleDateString() : ""} />
              ...
              <SectionCard t={t} style={{ marginBottom: 16 }}>
                <SectionHeader t={t} icon={BookOpen} color="#c084fc" title={`Courses (${content.courses.length})`} />
                {content.courses.length === 0 ? <EmptyBlock icon={BookOpen} title="No courses found" t={t} />
                  : content.courses.slice(0, 6).map((c, i) => (
                    <CRow key={c.id || i} icon={BookOpen} color="#c084fc" t={t}
                      title={c.title || c.courseName || c.name || "Untitled Course"}
                      sub={c.studentsEnrolled != null ? `${c.studentsEnrolled} enrolled` : ""} />
              ...
              <div className="org-two-col" style={{ marginBottom: 16 }}>
                <SectionCard t={t} style={{ margin: 0 }}>
                  <SectionHeader t={t} icon={FolderOpen} color="#2dd4bf" title={`Files (${content.files.length})`} />
                  {content.files.length === 0 ? <EmptyBlock icon={FolderOpen} title="No files found" t={t} />
                    : content.files.slice(0, 5).map((f, i) => (
                      <CRow key={f.id || f.fileId || i} icon={FolderOpen} color="#2dd4bf" t={t}
                        title={f.title || f.fileName || f.originalFileName || f.name || "Untitled"}
                        sub={f.uploadedAt ? new Date(f.uploadedAt).toLocaleDateString() : f.createdAt ? new Date(f.createdAt).toLocaleDateString() : ""} />
                ...
                <SectionCard t={t} style={{ margin: 0 }}>
                  <SectionHeader t={t} icon={BarChart3} color="#a78bfa" title={`Quizzes (${content.quizzes.length})`} />
                  {content.quizzes.length === 0 ? <EmptyBlock icon={BarChart3} title="No quizzes found" t={t} />
                    : content.quizzes.slice(0, 5).map((q, i) => (
                      <CRow key={q.id || i} icon={BarChart3} color="#a78bfa" t={t}
                        title={q.title || q.quizName || q.name || "Untitled Quiz"} sub="" />
                    ))}
              ...
              <SectionCard t={t}>
                <SectionHeader t={t} icon={ClipboardList} color="#f59e0b" title={`Assignments (${content.assignments.length})`} />
                {content.assignments.length === 0 ? <EmptyBlock icon={ClipboardList} title="No assignments found" t={t} />
                  : content.assignments.slice(0, 6).map((a, i) => (
                    <CRow key={a.id || a.assignmentId || i} icon={ClipboardList} color="#f59e0b" t={t}
                      title={a.title || a.assignmentName || a.name || "Untitled"}
                      sub={a.dueDate ? (new Date(a.dueDate) < new Date() ? "Overdue" : `Due ${new Date(a.dueDate).toLocaleDateString()}`) : ""} />
      ...
      {activeTab === "batches" && (
        <div>
            ...
            <div className="org-badge-row" style={{ marginBottom: 16 }}>
              {content.batches.map(b => {
                ...
                return (
                  <button key={bid} onClick={() => setSelectedBatch(b)} style={{
                    display: "inline-flex", alignItems: "center", gap: 6,
                    padding: "8px 18px", borderRadius: RADIUS.pill, fontSize: 12, fontWeight: FONT_WEIGHT.bold,
                    cursor: "pointer", fontFamily: FONT_FAMILY, transition: "all .2s",
                    border: `1px solid ${active ? "#10b981" : t.pillBorder}`,
                    background: active ? "rgba(16,185,129,0.12)" : t.pillBg,
          ...
          {!batchStudLoading && !batchStudError && batchStudents.length > 0 && (
            <SectionCard t={t}>
              <SectionHeader t={t}
                title={`Students in Batch ${selectedBatch?.name || selectedBatch?.id || selectedBatch?.batchId} (${batchStudents.length})`}
                sub="Click a student to view their report" />
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {batchStudents.map((s, i) => (
                  <StudentListRow key={s.studentEmail} student={s} index={i} onClick={() => setSelectedStudent(s)} t={t} />
                ))}
          ...
          {!batchStudLoading && !batchStudError && batchStudents.length === 0 && selectedBatch && (
            <SectionCard t={t} style={{ textAlign: "center", padding: "3rem 1rem" }}>
              <EmptyBlock icon={Users}
                title={`No students found in Batch ${selectedBatch?.name || selectedBatch?.id || selectedBatch?.batchId}`}
                t={t} />
            </SectionCard>
          )}
          ...
          {!batchStudLoading && content.batches.length === 0 && (
            <SectionCard t={t} style={{ textAlign: "center", padding: "3rem 1rem" }}>
              <EmptyBlock icon={Users} title="No batches found for this trainer" t={t} />
            </SectionCard>
          )}
        </div>
      )}
...
function StudentListRow({ student: s, index: i, onClick, t }) {
  return (
    <div
      ...
      style={{
        display: "flex", alignItems: "center", gap: 12,
        padding: "12px 14px", borderRadius: RADIUS.chip, cursor: "pointer",
        transition: "all .2s", background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`,
      }}
    >
      <div style={{
        width: 38, height: 38, borderRadius: "50%", flexShrink: 0,
        display: "flex", alignItems: "center", justifyContent: "center",
        fontSize: 13, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
        background: AV_BG[i % 6], color: AV_FG[i % 6], lineHeight: 1,
      }}>{initials(s.studentEmail)}</div>

      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{
          fontSize: 13, fontWeight: FONT_WEIGHT.semibold, margin: "0 0 6px", color: t.text,
          overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
          fontFamily: FONT_FAMILY, lineHeight: 1,
        }}>{s.studentEmail}</p>
        <div style={{ display: "flex", gap: 4 }}>
          {[
            ...
            <div key={mi} style={{ flex: 1, height: 3, background: t.barBg, borderRadius: 2, overflow: "hidden" }}>
              <div style={{ width: `${Math.min(m.val || 0, 100)}%`, height: 3, borderRadius: 2, background: m.color, transition: "width 0.6s ease" }} />
            </div>
          ))}
        </div>
      ...
      <div style={{
        display: "inline-flex", alignItems: "center",
        fontSize: 13, fontWeight: FONT_WEIGHT.bold, color: progColor(s.overallProgressPercentage),
        border: `1px solid ${progColor(s.overallProgressPercentage)}44`,
        borderRadius: RADIUS.pill, padding: "4px 12px", flexShrink: 0,
        fontFamily: FONT_FAMILY, lineHeight: 1,
      }}>{fmtN(s.overallProgressPercentage)}%</div>

      <Eye size={14} color={t.textMuted} style={{ flexShrink: 0 }} />
    </div>
  );
}
...
function TrainerCard({ trainer, index, onClick, t }) {
  return (
    <div
      ...
      style={{
        background: t.cardBg, border: `1px solid ${t.border}`,
        borderRadius: RADIUS.chip, padding: "1.1rem", marginBottom: 10,
        cursor: "pointer", transition: "all .2s ease", boxShadow: t.shadow,
      }}
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 14, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
            background: AV_BG[index % 6], color: AV_FG[index % 6], lineHeight: 1,
          }}>{initials(trainer.email)}</div>
          <div>
            <p style={{ fontSize: 14, fontWeight: FONT_WEIGHT.semibold, margin: 0, color: t.text, fontFamily: FONT_FAMILY, lineHeight: 1.2 }}>{trainer.email}</p>
            <p style={{ fontSize: 12, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY, lineHeight: 1 }}>{trainer.displayName || "Trainer"}</p>
          </div>
        </div>
        ...
        <button style={{
          display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6,
          padding: "7px 14px", fontSize: 12, borderRadius: RADIUS.button, cursor: "pointer",
          fontWeight: FONT_WEIGHT.bold, border: `1px solid ${t.border}`,
          background: t.pillBg, color: ACCENT_PURPLE.base,
          fontFamily: FONT_FAMILY, whiteSpace: "nowrap", lineHeight: "normal",
        }}>
          <Eye size={14} strokeWidth={1.8} style={{ flexShrink: 0 }} />
          <span style={{ lineHeight: 1 }}>View Details</span>
          <ArrowUpRight size={13} strokeWidth={1.8} style={{ flexShrink: 0 }} />
        </button>
      </div>
    </div>
  );
...
const OrgReports = () => {
  ...
  /* ─────────────────────────────────────────────────────────
     THEME DETECTION — same source of truth as the Golden
     Reference (AdminDashboard.jsx): only the "dark" class on
     <html> is authoritative, no ambiguous fallback.
     ───────────────────────────────────────────────────────── */
  ...
  /* Load trainers — UNCHANGED business logic */
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
  ...
  const trainerStat = [
    { label: "Total Trainers", numericValue: trainers.length, icon: Users, colorKey: "purple", change: "Active on platform" },
  ];

  return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      <style>{`
        @keyframes org-pulse{0%,100%{opacity:1}50%{opacity:0.45}}

        .org-two-col{ display:grid; grid-template-columns:1fr 1fr; gap:16px; }
        @media (max-width:900px){ .org-two-col{ grid-template-columns:1fr; } }

        .org-arc-grid{ display:grid; grid-template-columns:repeat(5,1fr); gap:8px; }
        @media (max-width:820px){ .org-arc-grid{ grid-template-columns:repeat(3,1fr); } }
        @media (max-width:480px){ .org-arc-grid{ grid-template-columns:repeat(2,1fr); } }

        .org-badge-row{ display:flex; flex-wrap:wrap; gap:8px; align-items:center; }
        .org-tabs{ display:flex; gap:4px; flex-wrap:wrap; align-items:center; }

        @media (max-width:560px){
          .org-hero-badges{ width:100%; }
        }
      `}</style>

      {/* ═══ HERO — shared <Hero>, matches AdminDashboard exactly ═══ */}
      <Hero borderHero={t.borderHero}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <button onClick={() => navigate(-1)} style={{
            display: "inline-flex", alignItems: "center", gap: 6,
            background: t.actBg, border: `1px solid ${t.actBorder}`,
            borderRadius: RADIUS.chip, padding: "8px 16px", fontSize: 13, fontWeight: FONT_WEIGHT.semibold,
            color: t.textSub, cursor: "pointer", fontFamily: FONT_FAMILY, lineHeight: 1,
          }}>
            <ArrowLeft size={14} style={{ flexShrink: 0 }} /> Back
          </button>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
              ...
              <span style={{
                fontSize: FONT_SIZE.eyebrow, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide,
                textTransform: "uppercase", color: t.textSub, fontFamily: FONT_FAMILY,
              }}>Admin Portal</span>
            </div>
            <h1 style={{
              fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.heroTitle, fontSize: FONT_SIZE.heroTitle,
              color: ACCENT_PURPLE.base, margin: "0 0 6px", lineHeight: LINE_HEIGHT.heroTitle, letterSpacing: LETTER_SPACING.heroTitle,
            }}>Trainer Reports</h1>
        ...
        <div className="hero-badges org-hero-badges">
          {!loading && (
            <div style={{
              display: "flex", alignItems: "center", gap: 12,
              background: t.actBg, border: `1px solid ${t.actBorder}`,
              borderRadius: RADIUS.chip, padding: "8px 16px", fontSize: 11,
              fontWeight: FONT_WEIGHT.semibold, fontFamily: FONT_FAMILY, color: t.textSub, flexWrap: "wrap",
            }}>
              <span>{trainers.length} trainers</span>
              <span style={{ width: 1, height: 14, background: t.actBorder }} />
              <span style={{ color: t.liveText }}>Enterprise · Real-time</span>
            </div>
          )}

          <div style={{
            display: "flex", alignItems: "center", gap: 7,
            background: `${ACCENT_PURPLE.base}14`, border: `1px solid ${ACCENT_PURPLE.base}4d`,
            borderRadius: RADIUS.pill, padding: "8px 18px", color: ACCENT_PURPLE.base,
            fontSize: 11, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, fontFamily: FONT_FAMILY,
          }}>
      ...
      {/* ═══ SUMMARY STAT — shared <StatCard>, via the shared .stat-grid class ═══ */}
      ...
      {loading && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {[1,2,3].map(i => <Skel key={i} height={80} t={t} />)}
        </div>
      )}
      ...
      {!loading && !error && !selected && (
        <>
          {trainers.length === 0 ? (
            <SectionCard t={t} style={{ textAlign: "center", padding: "3rem 1rem" }}>
              <EmptyBlock icon={Users} title="No trainers found" t={t} />
              <p style={{ fontSize: 13, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY }}>
                Add users with the{" "}
                <code style={{ background: t.recentItemBg, border: `1px solid ${t.recentItemBorder}`, padding: "1px 7px", borderRadius: 5, fontSize: 12 }}>ROLE_TRAINER</code>
                {" "}role in User Management.
              </p>
            </SectionCard>
          ) : (
            <SectionCard t={t}>
              <SectionHeader t={t} icon={Shield} color={ACCENT_PURPLE.base} title={`All Trainers (${trainers.length})`} />
              {trainers.map((trainer, i) => (
                <TrainerCard
                  key={trainer.id || trainer.email}
                  trainer={trainer}
