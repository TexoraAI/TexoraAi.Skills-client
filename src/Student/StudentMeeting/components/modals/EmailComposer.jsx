...
import { Bold, Italic, Underline, List, ListOrdered, Link2, Paperclip, X as XIcon } from "lucide-react";
  ...
  return (
    ...
    >
      <Field label="From" id="em-from">
        <input id="em-from" className="wm-input" value={currentUser.email} disabled />
      </Field>

      <Field label="To" required>
        <TagInput value={to} onChange={setTo} placeholder="Add recipient email" suggestions={contactEmails} />
      </Field>

      {!showCcBcc ? (
        <button type="button" className="wm-inline-link" onClick={() => setShowCcBcc(true)}>
          + Add Cc / Bcc
        </button>
      ) : (
        <Row cols={2}>
          <Field label="Cc">
            <TagInput value={cc} onChange={setCc} placeholder="Add Cc email" suggestions={contactEmails} />
          </Field>
          <Field label="Bcc">
            <TagInput value={bcc} onChange={setBcc} placeholder="Add Bcc email" suggestions={contactEmails} />
          </Field>
        </Row>
      )}
      ...
      <Field label="Message">
        <div className="wm-richtext">
          <div className="wm-richtext-toolbar">
            <button type="button" title="Bold" onClick={() => applyFormat("bold")}>
              <Bold size={14} />
            </button>
            <button type="button" title="Italic" onClick={() => applyFormat("italic")}>
              <Italic size={14} />
            </button>
            <button type="button" title="Underline" onClick={() => applyFormat("underline")}>
              <Underline size={14} />
            </button>
        ...
        {attachments.length > 0 && (
          <div className="wm-attachment-list">
            {attachments.map((name) => (
              <span className="wm-attachment-chip" key={name}>
                {name}
      ...
      {scheduleSend && (
        <Field label="Send at" id="em-schedule">
          <input id="em-schedule" type="datetime-local" className="wm-input" value={sendAt} onChange={(e) => setSendAt(e.target.value)} />
        </Field>
      )}
