      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
        <div>
          ...
          {batchId === "default_segment" && (
            <p style={{ margin: "5px 0 0", fontSize: 11, fontFamily: FF, color: c.accent }}>
              ℹ️ Not tied to any specific batch.
            </p>
          )}
        </div>
        <div>
...
const UploadVideoPanel = ({ t, isDark }) => {
  ...
  const [title,       setTitle]       = useState("");
  const [shortDesc,   setShortDesc]   = useState("");
  const [batchId,     setBatchId]     = useState("default_segment"); // ← default to Default Segment
  const [videoType,   setVideoType]   = useState("upload");
  const [videoUrl,    setVideoUrl]    = useState("");
  const [tags,        setTags]        = useState([]);
  const [audience,    setAudience]    = useState("not-kids");
  ...
  const handlePublish = async () => {
    ...
    const meta = { ...buildMeta(), status: "published" };
  ...
  const handleDraft = async () => {
    // Draft can be saved with minimal info — only a file or URL is required
    ...
    const resolvedBatch = resolveBatchId(batchId);
    const meta = { ...buildMeta(), status: "draft" };
    ...
    try {
      ...
      setRefreshKey((p) => p + 1);
      // Don't reset form — trainer may want to keep editing the draft
    } catch {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {/* ── Save as Draft (always visible, functional) ── */}
          <button
            onClick={handleDraft}
