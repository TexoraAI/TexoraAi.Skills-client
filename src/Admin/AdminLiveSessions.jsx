...
// import {
//   ArrowLeft, ChevronRight, Eye, GitBranch, GripVertical,
//   Layers, Pencil, Radio, Save, Trash2, Users, UserCheck, X,
//   CheckCircle2, Clock, PlayCircle, StopCircle,
// } from "lucide-react";
...
//   useEffect(() => {
//     setSessions([
//       { id:1, title:"React Live Class",       trainer:"John Doe",   batch:"Batch A", status:"Active"    },
//       { id:2, title:"NodeJS Live Session",    trainer:"Jane Smith", batch:"Batch B", status:"Completed" },
//       { id:3, title:"Python Masterclass",     trainer:"Ali Khan",   batch:"Batch C", status:"Scheduled" },
//       { id:4, title:"UI/UX Design Workshop",  trainer:"Sara Ahmed", batch:"Batch D", status:"Ended"     },
//     ]);
//   }, []);

//   const handleView = session => { setSelectedSession(session); setPanelMode("view"); setPanelOpen(true); };
//   const handleEdit = session => { setSelectedSession(session); setEditForm({ title:session.title, trainer:session.trainer, batch:session.batch, status:session.status }); setPanelMode("edit"); setPanelOpen(true); };
//   const handleSaveEdit = () => { setSessions(prev => prev.map(s => s.id === selectedSession.id ? { ...s, ...editForm } : s)); setSelectedSession(prev => ({ ...prev, ...editForm })); setPanelMode("view"); };
...
//   /* drag */
//   const { handlers: dragH, active: dActive, over: dOver } = useDragList(sessions, setSessions);
...
//             </div>
//             <div>
//               <h1 className="text-2xl font-bold tracking-tight text-white">Admin Live Sessions</h1>
//               <p className="mt-0.5 text-sm text-blue-100/80">Monitor and manage all live sessions conducted by trainers</p>
//             </div>
//           </div>
...
//               {sessions.length}<span className="ml-1 font-normal text-blue-100/80">Sessions</span>
//             </span>
//           </div>
//         </div>
//       </div>
...
//           <Input placeholder="Search sessions or trainers…" value={search} onChange={e => setSearch(e.target.value)}
...
//                       <th className="py-3 text-left text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">#</th>
//                       <th className="py-3 text-left text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">Session</th>
//                       <th className="py-3 text-left text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">Trainer</th>
//                       <th className="py-3 text-left text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">Batch</th>
//                       <th className="py-3 text-left text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">Status</th>
//                       <th className="pr-6 py-3 text-right text-[11px] uppercase tracking-wider font-semibold text-slate-500 dark:text-slate-400">Actions</th>
//                     </tr>
//                   </thead>
//                   <tbody>
//                     {filtered.length === 0 ? (
...
//                         const isSelected = selectedSession?.id === s.id && panelOpen;
...
//                               ${isOver     ? "bg-blue-50 dark:bg-blue-950/20 border-t-2 border-t-blue-400" : ""}
//                               ${isSelected ? "bg-blue-50/70 dark:bg-blue-950/20" : !isDragging && !isOver ? "hover:bg-blue-50/40 dark:hover:bg-slate-800/40" : ""}
//                             `}>

//                             {/* grip */}
//                             <td className="pl-4 py-3.5 w-8">
//                               <div className="cursor-grab active:cursor-grabbing opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
//                                 <GripVertical className="h-4 w-4 text-slate-400" />
//                               </div>
//                             </td>

//                             {/* # */}
//                             <td className="py-3.5 text-sm text-slate-400 font-medium w-10">
...
//                                 <div className={`h-9 w-9 rounded-xl bg-gradient-to-br ${grad(s.title)} flex items-center justify-center text-sm font-bold text-white shadow-sm flex-shrink-0`}>
...
//                                 <div className={`h-7 w-7 rounded-full bg-gradient-to-br ${grad(s.trainer)} flex items-center justify-center text-[11px] font-bold text-white`}>
//                                   {s.trainer.split(" ").map(n => n[0]).join("").slice(0,2)}
//                                 </div>
//                                 <span className="text-sm text-slate-700 dark:text-slate-300">{s.trainer}</span>
//                               </div>
//                             </td>
...
//                             {/* status */}
//                             <td className="py-3.5">
//                               <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold border ${st.text} ${st.bg} ${st.border}`}>
//                                 <span className={`h-1.5 w-1.5 rounded-full ${st.dot} ${s.status === "Active" ? "animate-pulse" : ""}`} />
//                                 {s.status}
//                               </span>
//                             </td>
...
//                                     isSelected && panelMode === "view"
//                                       ? "bg-blue-500 text-white"
//                                       : "bg-blue-50 dark:bg-blue-950/50 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900"
//                                   }`}>
...
//                                       : "bg-violet-50 dark:bg-violet-950/50 text-violet-600 hover:bg-violet-100 dark:hover:bg-violet-900"
//                                   }`}>
...
//                   <div className="flex items-center gap-2.5">
//                     <div className={`h-10 w-10 rounded-xl bg-gradient-to-br ${grad(selectedSession.title)} flex items-center justify-center text-base font-bold text-white shadow-md`}>
//                       {selectedSession.title.charAt(0).toUpperCase()}
//                     </div>
//                     <div>
//                       <h2 className="text-sm font-bold text-white leading-tight">
//                         {panelMode === "edit" ? "Edit Session" : selectedSession.title}
//                       </h2>
//                       <p className="text-[11px] text-white/70 mt-0.5">
//                         {panelMode === "edit" ? "Update session details" : "Session Details"}
//                       </p>
//                     </div>
//                   </div>
//                   <div className="flex items-center gap-1.5">
//                     {panelMode === "view" && (
//                       <button onClick={() => { setEditForm({ title:selectedSession.title, trainer:selectedSession.trainer, batch:selectedSession.batch, status:selectedSession.status }); setPanelMode("edit"); }}
//                         className="h-7 w-7 rounded-lg bg-white/15 hover:bg-white/30 flex items-center justify-center transition-colors" title="Edit">
...
//                   <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${panelMode === "edit" ? "bg-white/20 text-white" : "bg-white/20 text-white"}`}>
//                     {panelMode === "edit" ? <Pencil className="h-3 w-3"/> : <Eye className="h-3 w-3"/>}
//                     {panelMode === "edit" ? "Edit Mode" : "View Mode"}
//                   </span>
//                 </div>
//               </div>

//               {/* panel body */}
//               <div className="p-5 space-y-3 overflow-y-auto max-h-[calc(100vh-320px)]">

//                 {panelMode === "view" ? (
//                   <>
//                     {[
//                       { label:"Session Title", value:selectedSession.title,   icon:Radio        },
//                       { label:"Trainer",       value:selectedSession.trainer, icon:UserCheck    },
//                       { label:"Batch",         value:selectedSession.batch,   icon:Layers       },
//                       { label:"Status",        value:selectedSession.status,  icon:CheckCircle2 },
...
//                       </div>
//                     ))}
//                     <div className="rounded-xl border border-blue-100 dark:border-blue-900 bg-blue-50/60 dark:bg-blue-950/20 p-3 flex items-center gap-3">
//                       <div className={`h-10 w-10 rounded-full bg-gradient-to-br ${grad(selectedSession.trainer)} flex items-center justify-center text-sm font-bold text-white flex-shrink-0`}>
//                         {selectedSession.trainer.split(" ").map(n=>n[0]).join("").slice(0,2)}
//                       </div>
//                       <div>
//                         <p className="text-[10px] font-semibold text-blue-500 uppercase tracking-wide">Conducting Trainer</p>
//                         <p className="text-sm font-semibold text-slate-800 dark:text-slate-100 mt-0.5">{selectedSession.trainer}</p>
//                       </div>
//                     </div>
//                   </>
//                 ) : (
...
//                     {[
//                       { key:"title",   label:"Session Title", icon:Radio,     placeholder:"e.g. React Live Class",  type:"text"   },
//                       { key:"trainer", label:"Trainer Name",  icon:UserCheck, placeholder:"e.g. John Doe",          type:"text"   },
//                       { key:"batch",   label:"Batch",         icon:Layers,    placeholder:"e.g. Batch A",           type:"text"   },
//                     ].map(({ key, label, icon: Icon, placeholder, type }) => (
//                       <div key={key} className="space-y-1.5">
//                         <label className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
...
//                         className="w-full h-9 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 text-sm text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-500">
...
//                       className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-red-500 to-red-600 shadow hover:opacity-90 transition-all">
...
//                       className="flex items-center gap-1.5 px-5 py-2 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-violet-600 to-purple-500 shadow hover:opacity-90 hover:scale-105 transition-all">
...
// export default AdminLiveSessions; old2
...
const AdminLiveSessions = () => {
  ...
  /* ── RECORDED VIDEOS state (unchanged) ── */
  ...
  useEffect(() => {
    ...
    setVideos([
      { id:1, title:"React Hooks Explained", trainer:"John Doe",   batch:"Batch A", date:"2026-02-20" },
      { id:2, title:"MongoDB Basics",        trainer:"Jane Smith", batch:"Batch B", date:"2026-02-18" },
      { id:3, title:"Node.js REST APIs",     trainer:"Ali Khan",   batch:"Batch C", date:"2026-02-15" },
      { id:4, title:"Python for Beginners",  trainer:"Sara Ahmed", batch:"Batch D", date:"2026-02-10" },
    ]);
  }, []);

  /* ── live handlers (unchanged) ── */
  ...
  const handleSaveEdit   = () => { setSessions(p => p.map(s => s.id===selectedSession.id ? {...s,...editForm} : s)); setSelectedSession(p => ({...p,...editForm})); setPanelMode("view"); };
  const handleDelete     = id => { if (window.confirm("Delete this session?")) setSessions(p => p.filter(s => s.id!==id)); if (selectedSession?.id===id) closePanel(); };
  const closePanel       = () => { setPanelOpen(false); setSelectedSession(null); setPanelMode("view"); };
  ...
  const handleVideoSave     = () => { setVideos(p => p.map(v => v.id===selectedVideo.id ? {...v,...videoEditForm} : v)); setSelectedVideo(p => ({...p,...videoEditForm})); setVideoPanelMode("view"); };
  const handleVideoDelete   = id => { if (window.confirm("Delete this video?")) setVideos(p => p.filter(v => v.id!==id)); if (selectedVideo?.id===id) closeVideoPanel(); };
