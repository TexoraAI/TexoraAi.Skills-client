...
import {
  ArrowRight, CheckCircle2, Star, Users, Award, TrendingUp,
  ChevronDown, Globe, Clock, BookOpen,
  MapPin, GraduationCap, DollarSign, Calendar, Plane,
  FileText, ClipboardList, Phone, Mail, Shield, Zap,
  Building, Languages, Heart, Target, BarChart, ChevronRight,
  Flag, X, Send, User, MessageSquare,
...
// Pricing, Contact, FAQ, etc). This guarantees StudyAbroad gets the exact
// same AnnouncementBanner → Navbar → ... → Footer as the rest of the site,
// with identical alignment/spacing — no more custom, duplicate navbar.
...
/* ─────────────────────────────────────────────────────────────
   ✅ ALIGNMENT FIX: all rules are scoped under `.sa-page` instead
   ...
   shared navbar and footer — that's what caused misaligned nav.
   The old custom Navbar (with MegaMenu / mobile Sheet) has also
   been removed entirely — PublicLayout already renders the exact
   same navbar + footer used by Careers and every other public page.
───────────────────────────────────────────────────────────── */
const GLOBAL = `
  .sa-page, .sa-page *, .sa-page *::before, .sa-page *::after{box-sizing:border-box;}
  .sa-page h1,.sa-page h2,.sa-page h3,.sa-page h4,
  .sa-page p,.sa-page ul,.sa-page ol,
  .sa-page button,.sa-page span,.sa-page div,.sa-page a{margin:0;padding:0;}

  @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
  @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}
  .sa-page .flag-float{animation:float 3s ease-in-out infinite;}
  .sa-page .sa-card-hover:hover{transform:translateY(-4px);box-shadow:0 16px 40px rgba(0,0,0,.14)!important;}
  .sa-page .sa-card-hover{transition:all .25s ease;}
  .sa-page .exam-card:hover{transform:translateY(-2px);}
  .sa-page .exam-card{transition:all .2s;}
`;
...
/* ══════════════════ MAIN ══════════════════
   Rendered INSIDE PublicLayout, exactly like Careers.jsx, so this page
   gets the same shared navbar + footer as the rest of the public site.
   theme/toggleTheme now come in as props (owned by the app shell) instead
   of local state, matching how every other public page is wired up.
═══════════════════════════════════════════════════════════ */
export default function StudyAbroad({ theme, toggleTheme, setShowLoginModal, scrollToSection }) {
  ...
  return (
    ...
    >
      <style>{GLOBAL}</style>
      <div className="sa-page" style={{ fontFamily: F, background: BG, minHeight: "100vh", color: TP, transition: "background .3s,color .3s" }}>
