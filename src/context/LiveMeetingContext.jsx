...
export function LiveMeetingProvider({ children }) {
  ...
  const joinMeeting = useCallback(
    async ({ role, sessionId, roomName, token, title, joinedAt }) => {
      if (
        roomRef.current &&
        activeMeeting?.sessionId === sessionId &&
            ...
            if (msg.type === "reaction") {
              ...
              setFloaters((prev) => [
                ...prev,
                {
                  id,
                  emoji: msg.emoji,
                  name: participant?.name || participant?.identity || "Someone",
                },
              ]);
              setTimeout(() => {
            ...
            if (msg.type === "trainer_command") {
              ...
              switch (msg.command) {
                ...
                case "disableChat": {
                  ...
                  pushSystemMsg(
                    "Chat has been disabled for you by the trainer.",
                  );
                  break;
                }
                case "enableChat": {
                  ...
                  pushSystemMsg(
                    "Chat has been re-enabled for you by the trainer.",
                  );
                  break;
                }
                ...
                case "lowerHand": {
                  setRaisedHands((prev) =>
                    prev.you ? { ...prev, you: false } : prev,
                  );
                  pushSystemMsg("Your hand was lowered by the trainer.");
                  try {
                    const downPayload = new TextEncoder().encode(
                      JSON.stringify({ type: "raiseHand", raised: false }),
                    );
                    room?.localParticipant?.publishData(downPayload, {
                      reliable: true,
                    });
                  } catch (_) {}
                  break;
                ...
                case "screenShareBlocked": {
                  ...
                  if (msg.value) {
                    ...
                    setParticipants((prev) =>
                      prev.map((p) =>
                        p.isLocal ? { ...p, screenTrack: null } : p,
                      ),
                    );
                  ...
                  pushSystemMsg(
                    msg.value
                      ? "Screen sharing has been blocked by the trainer."
                      : "Screen sharing is allowed again.",
                  );
                  break;
                }
  ...
  const toggleMic = useCallback(async () => {
    ...
    if (micOn) {
      ...
      if (micLockedByTrainer) {
        pushSystemMsg(
          "You've been muted by the trainer and can't unmute right now.",
        );
        return;
      }
      await track.unmute();
  ...
  const toggleCam = useCallback(async () => {
    if (camOn) {
      ...
      if (cameraLockedByTrainer) {
        pushSystemMsg(
          "Your camera has been disabled by the trainer and can't be turned on right now.",
        );
        return;
      }
      ...
      if (!ok)
        pushSystemMsg(
          "Couldn't restart your camera — check your camera permissions.",
        );
    }
  }, [
    camOn,
    cameraLockedByTrainer,
    pushSystemMsg,
    syncParticipants,
    restartLocalCamera,
  ]);
  ...
  const toggleScreen = useCallback(async () => {
    ...
    try {
      if (screenOn) {
        ...
        setScreenOn(false);
        setParticipants((prev) =>
          prev.map((p) => (p.isLocal ? { ...p, screenTrack: null } : p)),
        );
...
export const useLiveMeeting = () => {
  ...
  if (!ctx)
    throw new Error("useLiveMeeting must be used within LiveMeetingProvider");
  return ctx;
};
