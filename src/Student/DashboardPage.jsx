    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-1">
          <button onClick={prevMonth} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 dark:text-zinc-500 transition">
            <ChevronLeft size={14} />
          </button>
          <button onClick={nextMonth} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 dark:text-zinc-500 transition">
      ...
      <div className="grid grid-cols-7 mb-1">
        {dayNames.map(d => (
          <div key={d} className="text-center text-[10px] font-semibold text-slate-400 dark:text-zinc-600 py-1">{d}</div>
        ))}
      </div>
      <div className="grid grid-cols-7 gap-y-0.5">
        {cells.map((d, i) => (
          <div key={i} className="flex items-center justify-center py-1">
            {d ? (
              <button className={`w-7 h-7 rounded-full text-xs font-medium flex items-center justify-center transition
                ${isToday(d)
...
const EmptyChart = () => (
  <div className="flex items-end justify-center gap-2 h-24 px-4">
    {[30,50,20,70,40,60,35,55,25,45,65,30].map((h,i) => (
      <div key={i} className="flex-1 rounded-t-sm bg-slate-100 dark:bg-white/5" style={{ height: `${h}%` }} />
    ))}
  </div>
);

/* ═══════════════════════════════════════════════
   NOTIFICATION BELL DROPDOWN (real API)
═══════════════════════════════════════════════ */
const NotificationBell = () => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        <Bell className={`w-5 h-5 transition-transform duration-300 ${animateBell ? "animate-bounce" : ""}`} />
        {unreadCount > 0 && (
        ...
        {animateBell && <span className="absolute inset-0 rounded-xl animate-ping bg-indigo-400 opacity-30" />}
      </button>

      {open && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl dark:shadow-black/40 z-[9999] overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-slate-800 flex-shrink-0">
            ...
            <div className="flex items-center gap-1">
              {unreadCount > 0 && (
                <button onClick={handleMarkAllRead} className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-colors">
                  <CheckCheck className="w-3 h-3" /> All read
                </button>
              )}
              <button onClick={() => setOpen(false)} className="p-1 rounded-lg text-gray-400 hover:text-gray-600 dark:text-slate-500 dark:hover:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
            ...
            {notifications.map(item => {
              ...
              return (
                <div key={item.id}
                  ...
                  className={`flex items-start gap-3 px-4 py-3 border-b border-gray-50 dark:border-slate-800/60 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer group ${!item.read ? "bg-indigo-50/50 dark:bg-indigo-950/10" : ""}`}
                >
                  <div className={`flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl text-base bg-gradient-to-br ${color} shadow-sm`}>{icon}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-gray-900 dark:text-slate-100 truncate">{item.title}</p>
                    <p className="text-xs text-gray-500 dark:text-slate-400 line-clamp-2 mt-0.5">{item.message}</p>
                    <p className="text-[10px] text-gray-400 dark:text-slate-600 mt-1">{formatTime(item.createdAt)}</p>
                  </div>
                  {!item.read && <Circle className="w-2 h-2 fill-indigo-500 text-indigo-500 flex-shrink-0 mt-1" />}
                </div>
              );
            })}
          </div>
...
const LatestMessagePanel = () => {
    ...
    return (
      <div className="bg-white dark:bg-[#111111] rounded-2xl border border-slate-100 dark:border-[#1a1a1a] shadow-sm p-5 flex flex-col">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setOpen => setExpanded(true)} className="text-slate-400 dark:text-zinc-600 hover:text-slate-600 dark:hover:text-zinc-400 transition">
          ...
          <div className="flex-1 space-y-2 mb-3 max-h-36 overflow-y-auto">
            {chat.slice(-3).map((msg, idx) => (
              <div key={idx} className={`flex ${msg.sender === "student" ? "justify-end" : "justify-start"}`}>
                ...
                }`} style={msg.sender === "student" ? { background: "linear-gradient(135deg,#7c3aed,#a855f7)" } : {}}>
                  {msg.text}
                </div>
              </div>
            ))}
        ...
        {batchId && trainerEmail && (
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="flex-1 text-xs px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-400/40"
            />
        ...
        <div className="mt-auto pt-3 border-t border-slate-100 dark:border-[#1a1a1a]">
          <button onClick={() => setExpanded(true)} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-violet-600 dark:text-violet-400 hover:bg-violet-50 dark:hover:bg-violet-500/10 transition">
            <Inbox size={14} />
            Go to Inbox
          </button>
        </div>
  ...
  // Expanded full-height chat
  return (
    <div className="bg-white dark:bg-[#111111] rounded-2xl border border-slate-100 dark:border-[#1a1a1a] shadow-sm flex flex-col" style={{ height: "380px" }}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-[#1a1a1a]">
        <div className="flex items-center gap-2">
          ...
          <div>
            <p className="text-xs font-semibold text-slate-800 dark:text-white">{trainerEmail?.split("@")[0] || "Trainer"}</p>
            <p className="text-[10px] text-slate-400">{trainerEmail || "Not assigned"}</p>
          </div>
        </div>
        <button onClick={() => setExpanded(false)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 text-slate-400 transition">
          <X size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {chat.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <p className="text-xs text-slate-400 dark:text-zinc-600">No messages yet. Say hello!</p>
          </div>
        ) : chat.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === "student" ? "justify-end" : "justify-start"}`}>
            {msg.sender === "teacher" && (
            ...
            <div className={`max-w-[75%] px-3 py-2 text-xs rounded-2xl ${
              msg.sender === "student"
                ? "text-white rounded-br-sm"
                : "bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-slate-100 rounded-bl-sm"
            }`} style={msg.sender === "student" ? { background: "linear-gradient(135deg,#7c3aed,#a855f7)" } : {}}>
              <p className="leading-relaxed">{msg.text}</p>
              <p className={`text-[9px] mt-0.5 text-right ${msg.sender === "student" ? "text-white/60" : "text-slate-400"}`}>{msg.time}</p>
            </div>
          </div>
        ))}
        <div ref={chatEndRef} />
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-t border-slate-100 dark:border-[#1a1a1a]">
        <input
          ...
          className="flex-1 text-xs px-3 py-2 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-400/40 disabled:opacity-50"
        />
        <button onClick={handleSend} disabled={!message.trim() || sending || !batchId}
...
/* ═══════════════════════════════════════════════
   NOTIFICATIONS PANEL (real API — sidebar widget)
═══════════════════════════════════════════════ */
const NotificationsPanel = () => {
  ...
  useEffect(() => {
    fetchMyNotifications()
      .then(data => setNotifications(Array.isArray(data) ? data.slice(0, 6) : []))
      .catch(console.error)
          ...
          {notifications.map(item => {
            ...
            return (
              <div key={item.id}
                ...
                <div className={`h-9 w-9 flex-shrink-0 rounded-xl flex items-center justify-center text-base bg-gradient-to-br ${color} shadow-sm`}>{icon}</div>
                <div className="flex-1 min-w-0">
                  <p className={`text-xs font-semibold truncate ${item.read ? "text-slate-500 dark:text-zinc-500" : "text-slate-800 dark:text-white"}`}>{item.title}</p>
                  <p className="text-[11px] text-slate-400 dark:text-zinc-600 line-clamp-1">{item.message}</p>
                  <p className="text-[10px] text-slate-300 dark:text-zinc-700 mt-0.5">{formatTime(item.createdAt)}</p>
                </div>
                {!item.read && <div className="w-2 h-2 rounded-full bg-violet-500 mt-1 flex-shrink-0 animate-pulse" />}
              </div>
            );
          })}
        </div>
    ...
    <div className="space-y-5">
        ...
        {stats.map(item => {
          ...
          return (
            <div key={item.label}
              ...
              <div className="flex items-center justify-between mb-3">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-zinc-500">{item.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#111111] rounded-2xl border border-slate-100 dark:border-[#1a1a1a] shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-zinc-500 mb-0.5">Total Users</p>
              <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1 bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
              {["Today","Weekly","Monthly","Annual"].map(p => (
                <button key={p} onClick={() => setChartPeriod(p)}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-all duration-150 ${chartPeriod === p
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {/* Live chat with trainer — real API */}
        <LatestMessagePanel />
      </div>
    </div>
  );
    ...
    <div className="min-h-full">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, #7c3aed 0%, #a855f7 30%, #ec4899 60%, #f97316 100%)" }}
        >
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <p className="text-white/60 text-xs font-semibold tracking-widest uppercase mb-1">Student Portal</p>
              <h1 className="text-3xl font-bold tracking-tight">Student Dashboard</h1>
              <p className="text-sm text-white/65 mt-1 font-normal">Your classroom & learning overview</p>
            </div>
              ...
              <div className="flex bg-black/20 backdrop-blur-sm border border-white/15 p-1 rounded-2xl gap-0.5">
                {["overview","courses","progress"].map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)}
                    className={`px-5 py-2 rounded-xl text-sm capitalize font-medium transition-all duration-200 ${activeTab === tab
