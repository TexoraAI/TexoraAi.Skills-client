...
// } from "lucide-react";
// import slide1 from "/fde-academy-assets/1.webp";
// import slide2 from "/fde-academy-assets/2.webp";
// import slide3 from "/fde-academy-assets/3.webp";
// import slide4 from "/fde-academy-assets/4.webp";
...
export default function FDEAcademy({
  ...
  // ── BUG FIX ──────────────────────────────────────────────────
  // Previously this page did `window.scrollTo({ top: 0 })` on every
  // tab change, which yanked the whole page back up to the hero
  // section even when the user had scrolled down to click a tab.
  // Instead, we scroll just the tab-bar/panel container into view,
  // and we skip this on the very first render so the page doesn't
  // jump on initial load either.
  const tabSectionRef = useRef(null);
  const isFirstRender = useRef(true);

  // Force the page to always open at the top (hero section), regardless
  // of the browser's scroll-restoration behavior, a leftover URL hash
  // (e.g. /fde-academy#curriculum from wherever this page is linked),
  // or a late reflow from images loading. Without this, navigating to
  // /fde-academy could land already scrolled down at the tab panel
  // instead of showing the hero first.
  useEffect(() => {
    ...
    // Strip any hash so the browser's native "scroll to #id" behavior
    // can't fight with us (this is the most common cause of landing
    ...
    window.scrollTo(0, 0);
    // Re-apply after paint/layout in case something (hash scroll,
    // image load reflow) tries to move it right after mount.
...
/>
      ...
      {/* ── TABS + PANEL ── */}
      <div ref={tabSectionRef} style={{ maxWidth: maxW, margin: "0 auto", padding: sectionPad, scrollMarginTop: "16px" }}>
        {/* Scrollable tab bar */}
        <div style={{ display: "flex", gap: "4px", background: surface, border: `1px solid ${border}`, borderRadius: "14px", padding: "5px", marginBottom: "14px", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          {TABS.map(({ id, label, Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)}
