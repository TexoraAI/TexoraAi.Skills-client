...
const inputCls =
  "w-full px-2.5 py-2 text-sm border border-gray-200 rounded-lg outline-none transition-all focus:ring-2 focus:ring-indigo-500 focus:border-transparent hover:border-gray-300 bg-white";
    ...
    <div className="space-y-4">
      <div className="pb-2 border-b border-gray-100">
        ...
        <p className="text-xs text-gray-500">
          Duration, pricing, and enrollment numbers
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {[
          ...
          { key: "assignmentsCount", label: "Assignments", placeholder: "10" },
          { key: "quizzesCount", label: "Quizzes", placeholder: "6" },
          {
            key: "studentsCount",
      ...
      {data.difficultyLevel && (
        <div className="flex items-center gap-2 text-xs">
          ...
          <span className="px-2 py-0.5 rounded-full font-semibold bg-indigo-50 text-indigo-600 border border-indigo-100">
            {data.difficultyLevel}
          </span>
          <span className="text-gray-300">· set in Basic Info tab</span>
        </div>
      )}
      ...
      <div className="pt-2 border-t border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <ToggleSwitch
            label="EMI Available"
            checked={!!data.emiAvailable}
            onChange={(val) => onChange("emiAvailable", val)}
          />
          <ToggleSwitch
            label="Free Trial"
            checked={!!data.freeTrial}
            onChange={(val) => onChange("freeTrial", val)}
          />
        </div>
      </div>
      ...
      {(data.price || data.offerText) && (
        <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-200 rounded-xl p-4">
          ...
          <div className="flex items-baseline gap-2">
            ...
            {data.originalPrice &&
              Number(data.originalPrice) > Number(data.price || 0) && (
                <span className="text-sm text-gray-400 line-through">
                  {Number(data.originalPrice).toLocaleString()}
                </span>
              )}
          ...
          {(data.emiAvailable || data.freeTrial) && (
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
            ...
            {data.projectsCount && (
              <span>🛠 {data.projectsCount} projects</span>
            )}
          </div>
        </div>
      )}
