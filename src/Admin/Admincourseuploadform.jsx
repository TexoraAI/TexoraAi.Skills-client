...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   IconBadge are page-local but token-driven. The page keeps its own
   orange (#f97316) domain accent for actions (Publish, panel numbers,
   toggles) the same way the Golden Reference keeps distinct per-feature
   accent colors (blue/green/amber/purple) inside a shared shell.
───────────────────────────────────────────────────────────────────────── */
...
/* ─── Panel / Section component ─── */
function Panel({ t, icon: Icon, number, title, children, defaultOpen = true, accent = ACCENT }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div style={{
      border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, overflow: "hidden",
      background: t.emptyBg, marginBottom: 10,
    }}>
      <button type="button" onClick={() => setOpen(!open)} style={{
        width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "11px 16px", background: t.recentItemBg,
        borderBottom: open ? `1px solid ${t.border}` : "none",
        cursor: "pointer", border: "none",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            background: `linear-gradient(135deg,${accent},${accent}cc)`,
            display: "flex", alignItems: "center", justifyContent: "center",
            color: "#fff", fontSize: 11, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY,
          }}>{number}</div>
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            <Icon size={13} color={accent} />
            <span style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY }}>{title}</span>
          </div>
        </div>
        {open
          ? <ChevronUp size={15} color={t.textMuted} />
...
function Field({ t, label, required, hint, children }) {
  return (
    <div>
      <label style={{ display: "block", fontSize: 10, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", color: t.textMuted, fontFamily: FONT_FAMILY, marginBottom: 5 }}>
...
const iStyle = (t) => ({
  ...
  color: t.text, fontSize: 12, fontFamily: FONT_FAMILY,
  outline: "none", boxSizing: "border-box",
});
...
function Toggle({ t, value, onChange }) {
  return (
    <button type="button" onClick={onChange} style={{
      ...
      background: value ? ACCENT : t.borderHov, position: "relative", flexShrink: 0, transition: "background 0.2s",
    }}>
      <span style={{
        position: "absolute", top: 3, width: 16, height: 16, borderRadius: "50%",
        background: "#fff", boxShadow: "0 1px 4px rgba(0,0,0,0.3)",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4, flexWrap: "wrap", gap: 10 }}>
        <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY, margin: 0 }}>
          Uploaded Courses <span style={{ color: t.textMuted, fontWeight: 400 }}>({courses.length})</span>
        </p>
        <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: RADIUS.chip, border: `1px solid ${t.border}`, background: t.actBg, color: t.textSub, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>
          <ArrowLeft size={12} /> Upload New
        </button>
      </div>

      {error && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.2)", borderRadius: RADIUS.chip, color: "#f43f5e", fontSize: 12, fontFamily: FONT_FAMILY }}>
          <AlertCircle size={14} />{error}
        </div>
      )}
      ...
      {loading && (
        <div style={{ display: "flex", justifyContent: "center", padding: "40px 0" }}>
          <Loader size={24} color={ACCENT} className="spin" />
        </div>
      )}
      ...
      {!loading && courses.length === 0 && !error && (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 0", gap: 10 }}>
          ...
          <p style={{ fontSize: 12, color: t.textMuted, fontFamily: FONT_FAMILY, margin: 0 }}>No courses uploaded yet</p>
        </div>
      )}
      ...
      {!loading && courses.map(course => {
        ...
        const isConfirming = confirmId === id;
        return (
          <div key={id} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", borderRadius: RADIUS.chip, border: `1px solid ${t.recentItemBorder}`, background: t.recentItemBg, transition: "background 0.15s" }}>
            <IconBadge icon={Film} color={ACCENT} size={36} iconSize={16} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: 0, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{course.title || "Untitled Course"}</p>
              <p style={{ fontSize: 10, color: t.textMuted, margin: "3px 0 0", fontFamily: FONT_FAMILY }}>{course.instructorName || "—"}{course.instructorRole ? ` • ${course.instructorRole}` : ""}</p>
              <div style={{ display: "flex", gap: 6, marginTop: 5, flexWrap: "wrap" }}>
                {course.learnersCount != null && (
                  <span style={{ fontSize: 9, background: "rgba(249,115,22,0.1)", color: ACCENT, padding: "2px 8px", borderRadius: RADIUS.pill, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY }}>{course.learnersCount}+ learners</span>
                )}
                {course.featuredTag && (
                  <span style={{ fontSize: 9, background: t.actBg, color: t.textSub, padding: "2px 8px", borderRadius: RADIUS.pill, fontFamily: FONT_FAMILY }}>{course.featuredTag}</span>
                )}
              </div>
            </div>
            <div style={{ flexShrink: 0, display: "flex", alignItems: "center", gap: 6 }}>
              {!isConfirming ? (
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY }}>Delete?</span>
                  <button onClick={() => handleDelete(id)} disabled={isDeleting} style={{ padding: "4px 10px", background: "#f43f5e", color: "#fff", borderRadius: 6, border: "none", fontSize: 10, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY, display: "flex", alignItems: "center", gap: 4 }}>
                    {isDeleting ? <Loader size={11} className="spin" /> : "Yes"}
                  </button>
                  <button onClick={() => setConfirmId(null)} style={{ padding: "4px 10px", background: t.actBg, color: t.textSub, borderRadius: 6, border: `1px solid ${t.border}`, fontSize: 10, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY }}>No</button>
                </div>
              )}
            </div>
          </div>
...
export default function AdminCourseUploadForm({ onSubmit, onClose }) {
  ...
  const globalStyles = (
    <style>{`
      ...
      @keyframes spin{to{transform:rotate(360deg)}}
      .spin{animation:spin 1s linear infinite}
      input:focus,textarea:focus,select:focus{border-color:${ACCENT} !important;box-shadow:0 0 0 3px rgba(249,115,22,0.1)}
      @media (max-width:900px){
        .upload-form-grid{grid-template-columns:1fr !important;}
      }
      @media (max-width:560px){
        .upload-hero-actions{width:100%;}
        .upload-hero-actions > *{width:100%;justify-content:center;}
      }
    `}</style>
  );
  ...
  if (view === "list") return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      {globalStyles}
      <SectionCard t={t}>
        <CoursesList t={t} onBack={() => setView("form")} />
      </SectionCard>
    </PageContainer>
  );
  ...
  if (status === "success") return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      <div style={{ minHeight: "70vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <SectionCard t={t} style={{ padding: "48px 40px", maxWidth: 420, width: "100%", textAlign: "center", boxShadow: t.shadowHov }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(52,211,153,0.12)", border: "1px solid rgba(52,211,153,0.3)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
            <CheckCircle size={30} color={t.liveColor} />
          </div>
          <h3 style={{ fontSize: 20, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: "0 0 8px", fontFamily: FONT_FAMILY }}>Course is Live! 🎉</h3>
          <p style={{ fontSize: 12, color: t.textSub, marginBottom: 24, fontFamily: FONT_FAMILY }}>"{videoTitle}" has been successfully published.</p>
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={reset} style={{ padding: "9px 20px", borderRadius: RADIUS.button, background: `linear-gradient(135deg,${ACCENT},#ea580c)`, border: "none", color: "#fff", fontSize: 12, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY }}>Create New</button>
            <button onClick={() => setView("list")} style={{ padding: "9px 20px", borderRadius: RADIUS.button, border: `1px solid ${t.border}`, background: t.actBg, color: t.textSub, fontSize: 12, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>View Courses</button>
          </div>
        </SectionCard>
      </div>
    </PageContainer>
  ...
  return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      <Hero borderHero={t.borderHero}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
            <button onClick={() => navigate(-1)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: RADIUS.chip, border: `1px solid ${t.borderHov}`, background: t.actBg, color: t.textSub, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>
              <ArrowLeft size={12} /> Back
            </button>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: ACCENT_PURPLE.base }} />
              <span style={{ fontSize: FONT_SIZE.eyebrow, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrowWide, textTransform: "uppercase", color: t.textSub, fontFamily: FONT_FAMILY }}>Admin Portal</span>
            </div>
          </div>
          <h1
            style={{
        ...
        <div className="hero-badges upload-hero-actions" style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <button
            type="button"
            onClick={() => navigate("/admin/featured-course-upload")}
            style={{
              display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: RADIUS.chip,
              background: `linear-gradient(135deg,${ACCENT},#ea580c)`, border: "none", color: "#fff",
              fontSize: 11, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY,
          ...
          <button type="button" onClick={() => setView("list")} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: RADIUS.chip, border: "1px solid rgba(249,115,22,0.4)", background: "rgba(249,115,22,0.08)", color: ACCENT, fontSize: 11, fontWeight: FONT_WEIGHT.bold, cursor: "pointer", fontFamily: FONT_FAMILY, whiteSpace: "nowrap" }}>
            <List size={13} /> My Courses
          </button>

          <div style={{ display: "flex", alignItems: "center", gap: 8, background: t.actBg, border: `1px solid ${t.actBorder}`, borderRadius: RADIUS.chip, padding: "7px 12px" }}>
            <Activity size={11} color={t.actIcon} />
            <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 12 }}>
              <span className="d1" style={{ width: 3, height: 8, borderRadius: 2, background: t.actBar, display: "block" }} />
              <span className="d2" style={{ width: 3, height: 12, borderRadius: 2, background: t.actBar, display: "block" }} />
          ...
          <div className="livebadge" style={{ display: "flex", alignItems: "center", gap: 6, background: dark ? "rgba(52,211,153,0.08)" : "rgba(22,163,74,0.08)", border: dark ? "1px solid rgba(52,211,153,0.3)" : "1px solid rgba(22,163,74,0.3)", borderRadius: RADIUS.pill, padding: "7px 14px", color: t.liveText, fontSize: 10, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, fontFamily: FONT_FAMILY }}>
            <span style={{ width: 5, height: 5, borderRadius: "50%", background: t.liveColor, display: "inline-block" }} /> LIVE
          </div>
        </div>
      </Hero>
      ...
      <SectionCard t={t}>
        {errorMsg && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.2)", borderRadius: RADIUS.chip, color: "#f43f5e", fontSize: 12, fontFamily: FONT_FAMILY, marginBottom: 14 }}>
            <AlertCircle size={14} style={{ flexShrink: 0 }} />{errorMsg}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          {/* ── Panel 1: Top Section ── */}
          <Panel t={t} icon={Tag} number="1" title="Top Section">
            <div className="upload-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field t={t} label="Platform Name" hint="optional">
                <input value={platformName} onChange={e => setPlatformName(e.target.value)} placeholder="TexoraAI.skills" style={inp} />
              </Field>
              <Field t={t} label="Featured Tag" hint="optional">
          ...
          {/* ── Panel 2: Video Card ── */}
          <Panel t={t} icon={Film} number="2" title="Main Video Card">
            <Field t={t} label="Video Title" required>
              <input value={videoTitle} onChange={e => setVideoTitle(e.target.value)} placeholder="e.g. System Design for Velocity Coders" style={inp} />
            </Field>
            ...
            <Field t={t} label="Video File" required hint="Upload file or paste URL">
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {!videoFile ? (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: RADIUS.chip, padding: "20px 16px", textAlign: "center", cursor: "pointer",
                      background: dragOver ? "rgba(249,115,22,0.06)" : t.pillBg, transition: "all 0.2s",
                    }}
                  >
                    <Upload size={20} color={dragOver ? ACCENT : t.textMuted} style={{ margin: "0 auto 6px" }} />
                    <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>Drag &amp; drop or click to upload</p>
                    <p style={{ fontSize: 10, color: t.textMuted, margin: "3px 0 0", fontFamily: FONT_FAMILY }}>MP4, WebM, MOV • Max 500MB</p>
                  </div>
                ) : (
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.25)", borderRadius: RADIUS.chip }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{videoFile.name}</p>
                      <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>{formatSize(videoFile.size)}</p>
                    </div>
                    <button type="button" onClick={() => setVideoFile(null)} style={{ background: "none", border: "none", cursor: "pointer", color: t.textMuted, padding: 2 }}><X size={14} /></button>
                  </div>
                )}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ flex: 1, height: 1, background: t.border }} />
                  <span style={{ fontSize: 10, color: t.textMuted, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY }}>OR</span>
                  <div style={{ flex: 1, height: 1, background: t.border }} />
                </div>
                <input value={videoUrl} onChange={e => { setVideoUrl(e.target.value); if (e.target.value) setVideoFile(null); }} placeholder="Paste video URL (https://...)" style={{ ...inp, opacity: videoFile ? 0.4 : 1 }} disabled={!!videoFile} />
              </div>
            ...
            <Field t={t} label="Thumbnail" hint="optional">
              <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
                {thumbnailPreview ? (
                  ...
                  <div onClick={() => thumbInputRef.current?.click()} style={{ width: 100, height: 60, border: `1.5px dashed ${t.border}`, borderRadius: 8, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", background: t.pillBg, flexShrink: 0, transition: "all 0.2s" }}>
                    <Image size={16} color={t.textMuted} />
                    <span style={{ fontSize: 9, color: t.textMuted, marginTop: 3, fontFamily: FONT_FAMILY }}>Add Image</span>
                  </div>
                )}
                <p style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY, lineHeight: 1.6 }}>JPG, PNG or WebP<br />Recommended: 16:9</p>
              </div>
              <input ref={thumbInputRef} type="file" accept="image/*" style={{ display: "none" }} onChange={handleThumbnailChange} />
            </Field>

            {/* Instructor Live Badge toggle */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: t.recentItemBg, borderRadius: RADIUS.chip, border: `1px solid ${t.border}`, gap: 10, flexWrap: "wrap" }}>
              <div>
                <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>Instructor Live Badge</p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>Show badge at top-right of video player</p>
              </div>
              <Toggle t={t} value={showInstructorLive} onChange={() => setShowInstructorLive(!showInstructorLive)} />
            </div>
          ...
          {/* ── Panel 3: Instructor + Description + Learn Points combined ── */}
          <Panel t={t} icon={User} number="3" title="Instructor Card">
            <div className="upload-form-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field t={t} label="Instructor Name" required>
                <input value={instructorName} onChange={e => setInstructorName(e.target.value)} placeholder="e.g. Arjay McCandless" style={inp} />
              </Field>
          ...
          <Panel t={t} icon={BookOpen} number="4" title="Description Section" defaultOpen={false}>
            <Field t={t} label="Full Description">
              <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Write about the course..." rows={4} style={{ ...inp, resize: "none" }} />
            </Field>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", background: t.recentItemBg, borderRadius: RADIUS.chip, border: `1px solid ${t.border}`, gap: 10, flexWrap: "wrap" }}>
              <div>
                <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>"Show More" Toggle</p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>Show a "Show more" button below description</p>
              </div>
              <Toggle t={t} value={showMoreEnabled} onChange={() => setShowMoreEnabled(!showMoreEnabled)} />
            </div>
          </Panel>

          <Panel t={t} icon={BookOpen} number="5" title="What You'll Learn" defaultOpen={false}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {learnPoints.map((point, idx) => (
                <div key={point.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "10px 12px", background: t.recentItemBg, borderRadius: RADIUS.chip, border: `1px solid ${t.border}` }}>
                  <div style={{ width: 22, height: 22, borderRadius: 6, background: `linear-gradient(135deg,${ACCENT},#ea580c)`, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: 10, fontWeight: FONT_WEIGHT.bold, fontFamily: FONT_FAMILY, flexShrink: 0, marginTop: 1 }}>{idx + 1}</div>
                  <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
                    <input value={point.title} onChange={e => updateLearnPoint(point.id, "title", e.target.value)} placeholder={`Point ${idx + 1} Title...`} style={{ ...inp, fontWeight: FONT_WEIGHT.semibold }} />
                    <input value={point.desc} onChange={e => updateLearnPoint(point.id, "desc", e.target.value)} placeholder="Short description (optional)..." style={inp} />
                  </div>
                  {learnPoints.length > 1 && (
                    <button type="button" onClick={() => removeLearnPoint(point.id)} style={{ background: "none", border: "none", cursor: "pointer", color: t.textMuted, padding: 2, marginTop: 2 }}><Trash2 size={13} /></button>
            ...
            <button type="button" onClick={addLearnPoint} style={{ width: "100%", padding: "9px", border: `1.5px dashed ${t.border}`, borderRadius: RADIUS.chip, background: "transparent", color: t.textMuted, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all 0.15s" }}>
              <Plus size={13} /> Add Another Point
            </button>
          </Panel>
          ...
          {/* Progress bar */}
          {status === "uploading" && (
            <div style={{ background: t.recentItemBg, borderRadius: RADIUS.chip, padding: "14px 16px", border: "1px solid rgba(249,115,22,0.2)", marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.text, fontFamily: FONT_FAMILY, marginBottom: 8 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <Loader size={13} color={ACCENT} className="spin" /> Uploading...
                </span>
                <span>{uploadProgress}%</span>
              </div>
              <div style={{ height: 6, background: t.border, borderRadius: RADIUS.pill, overflow: "hidden" }}>
                <div style={{ height: "100%", background: `linear-gradient(90deg,${ACCENT},#fb923c)`, borderRadius: RADIUS.pill, width: `${uploadProgress}%`, transition: "width 0.3s" }} />
              </div>
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4, flexWrap: "wrap" }}>
            <button type="submit" disabled={status === "uploading"} style={{
              flex: 1, minWidth: 160, padding: "12px 20px", borderRadius: RADIUS.button,
              background: status === "uploading" ? t.actBg : `linear-gradient(135deg,${ACCENT},#ea580c)`,
              border: status === "uploading" ? `1px solid ${t.border}` : "none",
              color: status === "uploading" ? t.textMuted : "#fff",
              fontSize: 13, fontWeight: FONT_WEIGHT.bold, cursor: status === "uploading" ? "not-allowed" : "pointer",
              fontFamily: FONT_FAMILY,
              boxShadow: status !== "uploading" ? "0 4px 14px rgba(249,115,22,0.4)" : "none",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all 0.2s",
            }}>
            ...
            {onClose && status !== "uploading" && (
              <button type="button" onClick={onClose} style={{ padding: "12px 20px", borderRadius: RADIUS.button, border: `1px solid ${t.border}`, background: t.actBg, color: t.textSub, fontSize: 13, fontWeight: FONT_WEIGHT.semibold, cursor: "pointer", fontFamily: FONT_FAMILY }}>
                Cancel
              </button>
            )}
          </div>
