...
import AuthModals from "./pages/Landing/components/AuthModals";
...
const SuperAdminAttendance = lazyLoad(
  () => import("./SuperAdmin/admin-control/SuperAdminAttendance"),
);

const SuperAdminAssessmentSystem = lazyLoad(
  () => import("./SuperAdmin/admin-control/SuperAdminAssessmentSystem"),
);
...
/>
      ...
      {/*
        Mounted once, here, so it's available on every route. Pages
        like About/Careers/Pricing/etc. only ever receive the
        `setShowLoginModal` setter and call it — until now nothing
        was listening to `showLoginModal` outside of LMSHomepage
        (which manages its own separate, local copy of this modal).
      */}
      <AuthModals
