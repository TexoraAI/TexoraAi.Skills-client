...
// /* ═══════════════════════════════════════════════
//    STAT CARD (solid gradient + watermark icon)
// ═══════════════════════════════════════════════ */
...
//           fontFamily: "'Poppins',sans-serif", color: "#fff", margin: 0,
//         }}>{value}</p>
//         <p style={{
//           fontSize: 10, fontWeight: 700, letterSpacing: "0.1em",
//           textTransform: "uppercase", color: "rgba(255,255,255,0.85)",
//           fontFamily: "'Poppins',sans-serif", margin: "8px 0 0",
//         }}>{label}</p>
//       </div>
//     </div>
//   );
...
//           {/* ═══ STAT CARDS (solid gradient + watermark icon) ═══ */}
//           <div style={{
//             display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(185px,1fr))",
//             gap: 14, marginBottom: 20,
//           }}>
//             <StatCard icon={FileText}     value={totalAttempts}                  label="Total Attempts" gradient="linear-gradient(135deg,#3b82f6,#2563eb)" />
//             <StatCard icon={CheckCircle2} value={passedAttempts}                 label="Passed"         gradient="linear-gradient(135deg,#22c55e,#16a34a)" />
//             <StatCard icon={XCircle}      value={totalAttempts - passedAttempts} label="Failed"         gradient="linear-gradient(135deg,#f87171,#dc2626)" />
//             <StatCard icon={TrendingUp}   value={`${averageScore.toFixed(1)}%`}  label="Avg Score"      gradient="linear-gradient(135deg,#8b5cf6,#7c3aed)" />
//           </div>
...
// ── Pulled from the Golden Reference (DashboardPage) instead of a local
// duplicate. T / STAT_COLORS / StatCard / PageContainer are the single
// source of truth for every page in the LMS — this page no longer keeps
// its own copy of the theme map or the stat-card component.
...
export default function MyQuizHistory() {
  ...
  /* ================= THEME (same pattern as DashboardPage) ================= */
  const [isDark, setIsDark] = useState(
    () =>
      typeof document !== "undefined" &&
      (document.documentElement.classList.contains("dark") ||
  ...
  // Stat cards now feed the shared StatCard from the design system (same
  // `stat` object shape DashboardPage uses), instead of the page's own
  // gradient-card component.
  const stats = [
    {
      label: "Total Attempts",
      numericValue: totalAttempts,
      change: `${totalAttempts} recorded`,
      trend: "up",
      icon: FileText,
      colorKey: "blue",
    },
    {
      label: "Passed",
      numericValue: passedAttempts,
      change: `${passedAttempts} of ${totalAttempts}`,
      trend: passedAttempts > 0 ? "up" : "down",
      icon: CheckCircle2,
      colorKey: "green",
    },
    {
      label: "Failed",
      numericValue: failedAttempts,
      change: failedAttempts > 0 ? "Needs review" : "None failed",
      trend: failedAttempts > 0 ? "down" : "up",
      icon: XCircle,
      colorKey: "red",
    },
    {
      label: "Avg Score",
      numericValue: Math.round(averageScore),
      isPercent: true,
      change: averageScore >= 50 ? "Passing average" : "Below passing",
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ color: isDark ? "rgba(255,255,255,0.3)" : "#94a3b8", fontSize: FONT_SIZE.bodySmall, fontFamily: FONT_FAMILY }}>
            Loading quiz history...
          </p>
  ...
  // Fonts, keyframes (dfade / d1-d3 / pulse-ring / drag-handle hover) and
  // the responsive grid classes all now come from PageContainer, same as
  // DashboardPage — nothing page-specific re-declared here.
  return (
    <PageContainer mode={isDark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      <div className="dfade" style={{
        ...
        <div className="hero-flex">
          ...
          <div>
            ...
            <p style={{
              fontSize: FONT_SIZE.bodySmall, color: t.textSub, marginTop: 7,
              fontWeight: FONT_WEIGHT.medium, fontFamily: FONT_FAMILY,
            }}>Track your assessment performance and review quiz attempts</p>
          </div>
          ...
          <div className="hero-badges">
            <div style={{
              ...
              fontFamily: FONT_FAMILY, color: t.textSub,
            }}>
              <span>{totalAttempts} attempts</span>
              <span style={{ width: 1, height: 14, background: t.actBorder }} />
              <span style={{ color: "#34d399", fontWeight: FONT_WEIGHT.bold }}>{passedAttempts} passed</span>
              <span style={{ width: 1, height: 14, background: t.actBorder }} />
              <span style={{ color: "#7c3aed", fontWeight: FONT_WEIGHT.bold }}>{averageScore.toFixed(1)}% avg</span>
            </div>
            <div style={{
              display: "flex", alignItems: "center", gap: 8,
              background: t.actBg, border: `1px solid ${t.actBorder}`,
            ...
            <div className="livebadge" style={{
              ...
              border: "1px solid rgba(124,58,237,0.3)",
              borderRadius: 999, padding: "8px 18px",
              color: "#7c3aed", fontSize: 11, fontWeight: FONT_WEIGHT.bold,
              letterSpacing: LETTER_SPACING.eyebrowWide, fontFamily: FONT_FAMILY,
            }}>
              <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#7c3aed", display: "inline-block" }} />
              LIVE
            </div>
      ...
      {/* ═══ STAT CARDS — shared StatCard, same as DashboardPage ═══ */}
      ...
      {attempts.length === 0 ? (
        <div className="dfade" style={{
          ...
          <div style={{ textAlign: "center" }}>
            <p style={{ fontSize: 14, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>No Quiz Attempts Yet</p>
            <p style={{ fontSize: 11, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY }}>
              Your quiz attempts will appear here once you start taking assessments
            </p>
          </div>
        </div>
      ) : (
        /* ===== SPLIT PANEL (unchanged layout/logic, tokens from `t`) ===== */
        <div
          ref={containerRef}
          className="dfade"
          style={{
        ...
        >
          ...
          >
            ...
            <div style={{
              ...
              <div>
                <span style={{ fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.bold, fontSize: 13, color: t.text, display: "block" }}>All Attempts</span>
                <span style={{ fontFamily: FONT_FAMILY, fontSize: 10, color: t.textMuted }}>Drag the handle to resize panels</span>
              </div>
            </div>
            ...
            <div style={{
              ...
              color: t.textMuted, fontFamily: FONT_FAMILY,
            }}>
              <span>Quiz</span>
              <span style={{ minWidth: 80 }}>Score</span>
              <span style={{ minWidth: 70 }}>Date</span>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
                      ...
                      <span style={{
                        fontSize: FONT_SIZE.bodySmall, fontWeight: FONT_WEIGHT.bold, color: t.text,
                        fontFamily: FONT_FAMILY,
                        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                      }}>
                        {a.quizTitle || a.quiz?.title || "Quiz"}
                    ...
                    {/* Score: "2/3" + percentage + mini bar */}
                    <div style={{ minWidth: 80 }}>
                      <p style={{ margin: 0, fontSize: FONT_SIZE.bodySmall, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY }}>
                        {scoreDisplay}
                        <span style={{ marginLeft: 4, fontSize: 10, fontWeight: FONT_WEIGHT.medium, color: t.textMuted }}>correct</span>
                      </p>
                      <p style={{ margin: "1px 0 0", fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY }}>{percent}%</p>
                      <div style={{ marginTop: 4, height: 4, borderRadius: 99, background: t.barBg, overflow: "hidden", width: 56 }}>
                        <div style={{
                          height: "100%", borderRadius: 99,
                          width: `${percent}%`,
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 70 }}>
                      <Calendar size={11} color={t.textMuted} />
                      <span style={{ fontSize: 10, color: t.textMuted, fontFamily: FONT_FAMILY }}>
                        {a.submittedAt
                          ? new Date(a.submittedAt).toLocaleDateString()
                          : "—"}
                      </span>
                    ...
                    {/* Result badge */}
                    <span style={{
                      fontSize: FONT_SIZE.eyebrow, fontWeight: FONT_WEIGHT.bold, letterSpacing: "0.08em",
                      textTransform: "uppercase", padding: "4px 10px", borderRadius: 999,
                      background: passed ? "rgba(52,211,153,0.12)" : "rgba(248,113,113,0.12)",
                      color: accentColor,
                      border: `1px solid ${passed ? "rgba(52,211,153,0.3)" : "rgba(248,113,113,0.3)"}`,
          ...
          <div style={{ display: "flex", flexDirection: "column", overflowY: "auto", flex: 1, minWidth: "20%" }}>
            ...
            <div style={{
              ...
              <div>
                <span style={{ fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.bold, fontSize: 13, color: t.text, display: "block" }}>Performance</span>
                <span style={{ fontFamily: FONT_FAMILY, fontSize: 10, color: t.textMuted }}>Your summary</span>
              </div>
            </div>
            ...
            <div style={{ padding: "20px 22px", display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div style={{
                ...
                <p style={{ fontSize: FONT_SIZE.eyebrow, fontWeight: FONT_WEIGHT.bold, letterSpacing: "0.15em", textTransform: "uppercase", color: t.textMuted, margin: "0 0 10px", fontFamily: FONT_FAMILY }}>
                  Average Score
                </p>
                <p style={{ fontSize: 42, fontWeight: FONT_WEIGHT.extrabold, color: "#a78bfa", margin: 0, fontFamily: FONT_FAMILY, lineHeight: 1 }}>
                  {averageScore.toFixed(1)}%
                </p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: "6px 0 12px", fontFamily: FONT_FAMILY }}>
                  across {totalAttempts} attempt{totalAttempts !== 1 ? "s" : ""}
                </p>
                <div style={{ height: 6, borderRadius: 99, background: t.barBg, overflow: "hidden" }}>
                  <div style={{
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {[
                    { label: "Passed", value: passedAttempts, color: "#34d399" },
                    { label: "Failed", value: failedAttempts, color: "#f87171" },
                    { label: "Total",  value: totalAttempts,  color: "#22d3ee" },
                  ].map((s, i) => (
                    <div key={i} style={{
                      display: "flex", alignItems: "center", justifyContent: "space-between",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ width: 8, height: 8, borderRadius: "50%", background: s.color, display: "inline-block" }} />
                        <span style={{ fontSize: FONT_SIZE.bodySmall, color: t.text, fontFamily: FONT_FAMILY, fontWeight: FONT_WEIGHT.medium }}>{s.label}</span>
                      </div>
                      <span style={{ fontSize: 13, fontWeight: FONT_WEIGHT.extrabold, color: s.color, fontFamily: FONT_FAMILY }}>{s.value}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                return (
                  <div>
                    ...
                    <div style={{
                      ...
                      <div style={{ minWidth: 0 }}>
                        <p style={{ margin: 0, fontSize: FONT_SIZE.bodySmall, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {best.quizTitle || best.quiz?.title || "Quiz"}
                        </p>
