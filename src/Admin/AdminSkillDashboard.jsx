  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: FONT_WEIGHT.bold, color: t.text, fontFamily: FONT_FAMILY }}>{title}</span>
      </div>
      {right}
    </div>
  );
...
const AdminSkillDashboard = () => {
  ...
  const card = (extra = {}) => ({
    background: t.cardBg, border: `1px solid ${t.border}`,
    borderRadius: RADIUS.standardCard, padding: CARD_PADDING.standardCard, boxShadow: t.shadow, ...extra,
  });

  const CustomTooltip = ({ active, payload, label }) => {
    if (!active || !payload?.length) return null;
    return (
      <div style={{ background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, padding: "8px 14px", boxShadow: t.shadow, fontFamily: FONT_FAMILY }}>
        <p style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: 0 }}>{label ?? payload[0]?.payload?.subject}</p>
        {payload.map((p, i) => (
          <p key={i} style={{ fontSize: 11, color: p.color ?? ACCENT_PURPLE.base, margin: "2px 0 0" }}>{p.name}: <strong>{p.value}%</strong></p>
        ))}
      </div>
    );
  };
  ...
  // visuals instead of the page's old bespoke tiles.
  const stats = [
    { label: "Total Students", numericValue: totalStudents, icon: Users, colorKey: "blue", change: "Across all batches" },
    { label: "Org Avg Score (%)", numericValue: orgAvg, icon: BarChart2, colorKey: "purple", change: "Organization-wide average" },
    { label: "Strong Learners", numericValue: totalStrong, icon: Trophy, colorKey: "green", change: "Scoring 70% or above" },
    { label: "Need Attention", numericValue: totalWeak, icon: AlertTriangle, colorKey: "red", change: "Scoring below 50%" },
    { label: "Active Batches", numericValue: BATCH_SUMMARY.length, icon: BookOpen, colorKey: "amber", change: "Currently running" },
  ];

  const tabs = ["overview", "by batch", "trends"];

  return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      <style>{`
        @media (max-width:900px){
          .skill-overview-grid{grid-template-columns:1fr !important;}
        }
        @media (max-width:560px){
          .skill-hero-badges{width:100%;}
          .skill-tabs{width:100%;}
      ...
      <Hero borderHero={t.borderHero}>
        <div>
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, fontWeight: FONT_WEIGHT.semibold, color: t.textMuted, background: "transparent", border: "none", cursor: "pointer", marginBottom: 12, padding: 0, fontFamily: FONT_FAMILY }}
          >
            <ArrowLeft size={13} /> Back to Dashboard
          </button>
          ...
          <p style={{ fontSize: FONT_SIZE.bodySmall, color: t.textSub, margin: 0, fontWeight: FONT_WEIGHT.medium, fontFamily: FONT_FAMILY }}>
            Organisation-wide skill intelligence &amp; performance overview
          </p>

          <div className="skill-tabs" style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
            {tabs.map(tab => (
              <button
                key={tab}
        ...
        <div className="hero-badges skill-hero-badges">
          <div
            style={{
              display: "flex",
              alignItems: "center",
      ...
      {/* ═══ SUMMARY STATS — shared <StatCard>, via the shared .stat-grid class ═══ */}
      ...
      {activeTab === "overview" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div className="skill-overview-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            {/* Radar */}
            <SectionCard t={t}>
              <SectionHeader t={t} icon={Brain} color={ACCENT_PURPLE.base} title="Org Skill Distribution" />
              <ResponsiveContainer width="100%" height={240}>
                <RadarChart data={RADAR_DATA}>
                  <PolarGrid stroke={t.border} />
                  <PolarAngleAxis dataKey="subject" tick={{ fill: t.textMuted, fontSize: 10, fontFamily: FONT_FAMILY, fontWeight: 600 }} />
                  <PolarRadiusAxis angle={30} domain={[0, 100]} tick={{ fill: t.textMuted, fontSize: 9 }} tickCount={4} />
                  <Radar name="Org Avg" dataKey="score" stroke="#22d3ee" fill="#22d3ee" fillOpacity={0.15} strokeWidth={2} dot={{ r: 4, fill: "#22d3ee", strokeWidth: 0 }} />
                  <Tooltip content={<CustomTooltip />} />
                </RadarChart>
            ...
            {/* Skill avg bars */}
            <SectionCard t={t}>
              <SectionHeader t={t} icon={BarChart2} color="#22d3ee" title="Org Skill Averages" />
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {RADAR_DATA.map(item => (
                  <div key={item.subject} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ width: 90, fontSize: 12, fontWeight: FONT_WEIGHT.semibold, color: t.text, flexShrink: 0, fontFamily: FONT_FAMILY }}>{item.subject}</span>
                    <div style={{ flex: 1, height: 6, background: t.barBg, borderRadius: RADIUS.pill, overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: RADIUS.pill, background: getProgressColor(item.score), width: `${item.score}%`, transition: "width 0.8s ease" }} />
                    </div>
                    <span style={{ fontSize: 12, fontWeight: FONT_WEIGHT.bold, color: getProgressColor(item.score), width: 36, textAlign: "right", flexShrink: 0, fontFamily: FONT_FAMILY }}>{item.score}%</span>
                    <span style={{ fontSize: 9, fontWeight: FONT_WEIGHT.bold, padding: "3px 8px", borderRadius: RADIUS.pill, background: `${getProgressColor(item.score)}15`, color: getProgressColor(item.score), flexShrink: 0, fontFamily: FONT_FAMILY }}>{getLevel(item.score)}</span>
                  </div>
                ))}
              </div>
            </SectionCard>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 14 }}>
            {BATCH_SUMMARY.map(b => (
              <SectionCard t={t} key={b.batch}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: FONT_WEIGHT.bold, color: t.text, margin: 0, fontFamily: FONT_FAMILY }}>{b.batch}</p>
                    <p style={{ fontSize: 10, color: t.textMuted, margin: "2px 0 0", fontFamily: FONT_FAMILY }}>Trainer: {b.trainer}</p>
                  </div>
                  <span style={{ fontSize: 22, fontWeight: FONT_WEIGHT.bold, color: getProgressColor(b.avgScore), fontFamily: FONT_FAMILY }}>{b.avgScore}%</span>
                </div>
                <div style={{ height: 6, background: t.barBg, borderRadius: RADIUS.pill, overflow: "hidden", marginBottom: 12 }}>
                  <div style={{ height: "100%", borderRadius: RADIUS.pill, background: getProgressColor(b.avgScore), width: `${b.avgScore}%`, transition: "width 0.8s ease" }} />
                </div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, padding: "4px 10px", borderRadius: RADIUS.chip, background: "rgba(34,211,238,0.1)", color: "#22d3ee", fontFamily: FONT_FAMILY }}>{b.students} students</span>
                  <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, padding: "4px 10px", borderRadius: RADIUS.chip, background: "rgba(52,211,153,0.1)", color: "#34d399", fontFamily: FONT_FAMILY }}>✓ {b.strong} strong</span>
                  {b.weak > 0 && <span style={{ fontSize: 11, fontWeight: FONT_WEIGHT.semibold, padding: "4px 10px", borderRadius: RADIUS.chip, background: "rgba(248,113,113,0.1)", color: "#f87171", fontFamily: FONT_FAMILY }}>⚠ {b.weak} weak</span>}
                </div>
              </SectionCard>
            ))}
          </div>
      ...
      {activeTab === "by batch" && (
        <SectionCard t={t}>
          <SectionHeader t={t} icon={BarChart2} color="#22d3ee" title="Skill Comparison — All Batches" />
          <ResponsiveContainer width="100%" height={320}>
            <BarChart data={ORG_SKILL_DATA} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={t.barBg} />
              <XAxis dataKey="skill" tick={{ fill: t.textMuted, fontSize: 10, fontFamily: FONT_FAMILY }} />
              <YAxis domain={[0, 100]} tick={{ fill: t.textMuted, fontSize: 9 }} />
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11, fontFamily: FONT_FAMILY, color: t.textMuted }} />
              <Bar dataKey="batchA" name="Batch A" fill="#a78bfa" radius={[4, 4, 0, 0]} />
              <Bar dataKey="batchB" name="Batch B" fill="#22d3ee" radius={[4, 4, 0, 0]} />
              <Bar dataKey="batchC" name="Batch C" fill="#34d399" radius={[4, 4, 0, 0]} />
            </BarChart>
      ...
      {activeTab === "trends" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <SectionCard t={t}>
            <SectionHeader t={t} icon={TrendingUp} color="#34d399" title="Org Average Score Trend (6 months)" />
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={TREND_DATA} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke={t.barBg} />
                <XAxis dataKey="month" tick={{ fill: t.textMuted, fontSize: 10, fontFamily: FONT_FAMILY }} />
                <YAxis domain={[40, 100]} tick={{ fill: t.textMuted, fontSize: 9 }} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="avg" name="Avg Score" stroke="#34d399" strokeWidth={2.5} dot={{ r: 5, fill: "#34d399", strokeWidth: 0 }} activeDot={{ r: 7 }} />
              </LineChart>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 14 }}>
            {[
              { label: "Score improvement", value: "+22%", sub: "Nov → Apr", color: "#34d399" },
              { label: "Best performing skill", value: "CSS", sub: "Avg 75%", color: ACCENT_PURPLE.base },
              { label: "Skill needing focus", value: "Python", sub: "Avg 57%", color: "#f87171" },
              { label: "Best batch", value: "Batch A", sub: "Avg 65%", color: "#22d3ee" },
            ].map((item, i) => (
              <SectionCard t={t} key={i}>
                <p style={{ fontSize: 10, fontWeight: FONT_WEIGHT.bold, letterSpacing: LETTER_SPACING.eyebrow, textTransform: "uppercase", color: t.textMuted, margin: "0 0 8px", fontFamily: FONT_FAMILY }}>{item.label}</p>
                <p style={{ fontSize: 24, fontWeight: FONT_WEIGHT.bold, color: item.color, margin: 0, fontFamily: FONT_FAMILY }}>{item.value}</p>
                <p style={{ fontSize: 10, color: t.textMuted, margin: "4px 0 0", fontFamily: FONT_FAMILY }}>{item.sub}</p>
              </SectionCard>
            ))}
          </div>
        </div>
