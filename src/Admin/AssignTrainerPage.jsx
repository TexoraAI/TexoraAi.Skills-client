...
const AssignTrainerPage = () => {
  ...
  const load = async () => {
    try {
      ...
      // Bug 2 guard — if this batch already has a trainer, this page is wrong; redirect to overview
      if (map && Object.keys(map).length > 0) {
        navigate(`/admin/batches/${batchId}/trainers`, { replace: true });
        return;
      }
          ...
          <div className="hidden md:flex items-center gap-2">
            <div className="flex items-center gap-2 rounded-2xl bg-white/15 px-4 py-2 backdrop-blur-sm">
              ...
              <span className="text-sm font-semibold text-white">
                {assignedCount}
                <span className="ml-1 font-normal text-blue-100/80">
                  Assigned
                </span>
              </span>
            </div>
        ...
        <CardContent className="p-4 space-y-3">
          ...
          {!loading && filtered.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 gap-3">
              <div
                className="h-14 w-14 rounded-2xl bg-slate-100 dark:bg-slate-800
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3.5">
                    <div
                      className={`h-11 w-11 rounded-xl bg-gradient-to-br ${grad(name)}
