  ...
  return (
    ...
    >
      ...
      <div className="ud-note-toolbar" style={{ borderBottomColor: c.divider }}>
        ...
        <ToolBtn
          active={activeFormats.normal}
          onClick={() => toggleFormat("normal")}
          title="Normal text"
        >
        ...
        <div style={{ position: "relative" }} ref={colorPickerRef}>
          <button
            ...
            title="Text color"
            onClick={() =>
              setShowColorPicker((p) => (p === "text" ? false : "text"))
            }
              ...
              >
                {TEXT_COLORS.map((col) => (
                  <div
                    ...
                    title={col.label}
                    onClick={() => {
                      setSelectedTextColor(col.value);
                      setShowColorPicker(false);
                    }}
                    style={{
...
const FilePreviewModal = ({ doc, onClose, isDark, c }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const meta = await fileService.getViewMeta(doc.id);
        // pdf/image/txt just need the presigned URL directly — no CORS
        // involved since it's not a JS fetch. Office files still need
        // this same URL for the download button and Google viewer link.
        setBlobUrl(meta.url);
      } catch {
                ...
                >
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    >
                      ...
                      previewed in the browser. Download and open in Microsoft
...
const DocCard = ({
  ...
  const handleDownload = async () => {
    try {
      const meta = await fileService.getDownloadMeta(doc.id);
      const a = document.createElement("a");
      a.href = meta.url;
      a.download = meta.originalName || doc.originalName || "file";
      a.target = "_blank";
...
const UploadDocuments = () => {
  ...
  const fetchQuota = async () => {
    try {
      const res = await fileService.getUploadQuota();
      setQuota(res.data);
    } catch (err) {
        ...
        {quota && (
          ...
          >
            <UsageBadge
              ...
              label={`${quota.tier?.toUpperCase() || ""} · ${quota.fileCount}/${quota.maxFileCount} files`}
              c={c}
            />
            <div style={{ fontSize: 11, color: c.textSub, fontFamily: FF }}>
              Max size per file upload:{" "}
              <strong style={{ color: c.textPrimary }}>
                {quota.maxFileSizeBytes
                  ? `${(quota.maxFileSizeBytes / 1024 / 1024).toFixed(0)} MB`
                  : "—"}
              </strong>
            </div>
          </div>
