...
const selectStyle = (t) => ({
  ...inputStyle(t), cursor: "pointer", appearance: "none",
});
const onFocusInput = (e, color) => { e.target.style.borderColor = color; e.target.style.boxShadow = `0 0 0 2px ${color}28`; };
const onBlurInput = (e, t) => { e.target.style.borderColor = t.border; e.target.style.boxShadow = "none"; };
...
const RichDescriptionEditor = ({ value, onChange, placeholder, t }) => {
  ...
  return (
    <div className="rde-wrap" style={{ border: `1px solid ${t.border}`, background: t.pillBg, borderRadius: RADIUS.button, overflow: "hidden" }}>
      <div className="rde-toolbar" style={{ borderBottom: `1px solid ${t.border}`, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 1, padding: "5px 8px" }}>
        <button type="button" className={`rde-btn${fmt.h1 ? " active" : ""}`} onClick={() => toggle("h1")} style={{ color: fmt.h1 ? ACCENT_PURPLE.base : t.textMuted }}>
          <span style={{ fontSize: 11, fontWeight: 800 }}>H1</span>
        </button>
        <button type="button" className={`rde-btn${fmt.h2 ? " active" : ""}`} onClick={() => toggle("h2")} style={{ color: fmt.h2 ? ACCENT_PURPLE.base : t.textMuted }}>
          <span style={{ fontSize: 11, fontWeight: 800 }}>H2</span>
        </button>
        <button type="button" className="rde-btn" onClick={() => setFmt({ bold: false, italic: false, underline: false, strike: false, h1: false, h2: false })} style={{ color: t.textMuted }}>
          <span style={{ fontSize: 11 }}>Aa</span>
        </button>
        <div className="rde-sep" style={{ background: t.border }} />
        <Btn k="bold" title="Bold"><Bold size={12} strokeWidth={fmt.bold ? 3 : 2} /></Btn>
        ...
        <div className="rde-sep" style={{ background: t.border }} />
        <div style={{ position: "relative" }} ref={cpRef}>
          <button type="button" className="rde-btn" onClick={() => setCpOpen(p => !p)} title="Text color" style={{ color: t.textMuted }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
              <span style={{ fontSize: 12, fontWeight: 800, color: txCol !== "default" ? txCol : t.text, lineHeight: 1 }}>A</span>
              <div style={{ width: 14, height: 3, borderRadius: 2, background: txCol !== "default" ? txCol : t.textMuted }} />
            </div>
          </button>
          {cpOpen && (
            <div style={{ position: "absolute", top: "110%", left: 0, zIndex: 9999, background: t.cardBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, padding: "10px 12px", boxShadow: t.shadow, minWidth: 220 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: t.textMuted, letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 8, fontFamily: FONT_FAMILY }}>Color</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {TX_COLORS.map(c => (
                  <div key={c.v} className={`rde-swatch${txCol === c.v ? " sel" : ""}`}
                    style={{ background: c.bg, border: c.bd ? `2px solid ${c.bd}` : txCol === c.v ? `2px solid ${ACCENT_PURPLE.base}` : "2px solid transparent" }}
                    onClick={() => { setTxCol(c.v); setCpOpen(false); }}
                    title={c.label}
                  />
                ))}
        ...
        <div style={{ marginLeft: "auto" }}>
          <button type="button" className="rde-btn" title="Pin" style={{ color: t.textMuted }}><Pin size={11} /></button>
        </div>
      </div>
      <textarea
        rows={3}
        ...
        style={{
          width: "100%", padding: "9px 12px", border: "none", outline: "none",
          background: t.pillBg, fontFamily: FONT_FAMILY, boxSizing: "border-box",
          resize: "vertical",
          fontSize: fmt.h1 ? 18 : fmt.h2 ? 14 : 12.5,
          fontWeight: fmt.bold ? 700 : 400,
          fontStyle: fmt.italic ? "italic" : "normal",
...
const CreateAssignments = () => {
  ...
  const panelMeta = {
    basic: { num: 1, label: "Basic Information", sub: "Title & description", color: ACCENT_PURPLE.base, nextLabel: "Details →" },
    details: { num: 2, label: "Assignment Details", sub: "Batch, deadline & marks", color: "#2563eb", nextLabel: "Attachments →" },
    attachments: { num: 3, label: "Attachments & Submit", sub: `${formData.attachments.length} file${formData.attachments.length !== 1 ? "s" : ""} selected`, color: "#059669", nextLabel: null },
  };
  ...
  return (
    <PageContainer mode={dark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      {showSuccess && (
        <div style={{
          display: "flex", alignItems: "center", gap: 10,
          padding: "11px 16px", borderRadius: RADIUS.chip, marginBottom: 16,
          background: "rgba(5,150,105,0.08)", border: "1px solid rgba(5,150,105,0.25)",
        }}>
          <div style={{ width: 24, height: 24, borderRadius: "50%", background: "rgba(5,150,105,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <CheckCircle size={13} color="#059669" />
          </div>
          <span style={{ fontSize: 12.5, fontWeight: 600, color: "#059669", fontFamily: FONT_FAMILY }}>
            Assignment Created Successfully!
          </span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ borderRadius: RADIUS.standardCard, overflow: "hidden", border: `1px solid ${t.border}`, boxShadow: t.shadow, background: t.cardBg }}>
          {PANELS.map((key, idx) => {
            const meta = panelMeta[key];
            const isOpen = open === key;
            const isDone = completedPanels[key];
            ...
            return (
              <div key={key} style={{ borderBottom: !isLast ? `1px solid ${t.border}` : "none" }}>
                <button
                  ...
                  className="ca-header"
                  onClick={() => toggle(key)}
                  style={{ background: isOpen ? `${meta.color}08` : t.cardBg, borderBottom: isOpen ? `1px solid ${t.border}` : "none" }}
                >
                  <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 3, background: stripColor, borderRadius: "0 2px 2px 0" }} />

                  <div className="ca-step-circle" style={{
                    marginLeft: 8,
                    background: isDone ? "rgba(5,150,105,0.12)" : isOpen ? `${meta.color}18` : t.pillBg,
                  ...
                  <div style={{ width: 32, height: 32, borderRadius: RADIUS.chip, flexShrink: 0, background: isOpen ? `${meta.color}12` : t.pillBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <PanelIcon size={14} color={isOpen ? meta.color : t.textMuted} />
                  </div>

                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: isOpen ? t.text : t.textSub, fontFamily: FONT_FAMILY, lineHeight: 1.2 }}>
                      {meta.label}
                    </div>
                    <div style={{ fontSize: 10.5, color: t.textMuted, fontFamily: FONT_FAMILY, marginTop: 2 }}>
                      {meta.sub}
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 4, alignItems: "center", marginRight: 6 }}>
                    {PANELS.map((p, pi) => (
                      <span key={p} style={{
                        width: p === key ? 16 : 5, height: 4, borderRadius: 999,
                        background: p === key ? meta.color : pi < PANELS.indexOf(key) ? t.textMuted : t.border,
                      }} />
                    ))}
                  </div>

                  <ChevronRight size={14} color={t.textMuted} style={{ transform: isOpen ? "rotate(90deg)" : "none", transition: "transform 0.22s", flexShrink: 0 }} />
                </button>
                  ...
                  <div className="ca-body" style={{ background: t.cardBg, padding: "16px 20px 20px 28px" }}>
                    <div style={{ display: "flex", gap: 16 }}>
                      ...
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 13 }}>
                        ...
                        {key === "details" && (
                          <>
                            <div className="ca-details-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                              <div>
                                <FieldLabel t={t}>Select Batch</FieldLabel>
                                <select style={selectStyle(t)} value={formData.batch} onChange={e => setFormData({ ...formData, batch: e.target.value })}
                                  onFocus={(e) => onFocusInput(e, meta.color)} onBlur={(e) => onBlurInput(e, t)} required>
                                  <option value="">Select Batch</option>
                                  {batches.map(b => <option key={b.id} value={b.id}>Batch {b.id}</option>)}
                                </select>
                              </div>
                              <div>
                                <FieldLabel t={t}>Deadline</FieldLabel>
                                <input style={inputStyle(t)} type="datetime-local" value={formData.deadline} onChange={e => setFormData({ ...formData, deadline: e.target.value })}
                                  onFocus={(e) => onFocusInput(e, meta.color)} onBlur={(e) => onBlurInput(e, t)} required />
                              </div>
                              <div>
                                <FieldLabel t={t}>Max Marks</FieldLabel>
                                <input style={inputStyle(t)} type="number" placeholder="e.g. 100" value={formData.maxMarks} onChange={e => setFormData({ ...formData, maxMarks: e.target.value })}
                                  onFocus={(e) => onFocusInput(e, meta.color)} onBlur={(e) => onBlurInput(e, t)} required />
                              </div>
                              <div>
                                <FieldLabel t={t}>Duration <span style={{ fontWeight: 400, opacity: 0.6 }}>(opt.)</span></FieldLabel>
                                <input style={inputStyle(t)} placeholder="e.g. 2 hours" value={formData.duration} onChange={e => setFormData({ ...formData, duration: e.target.value })}
                                  onFocus={(e) => onFocusInput(e, meta.color)} onBlur={(e) => onBlurInput(e, t)} />
                              </div>
                            </div>
                            <div style={{ display: "flex", justifyContent: "flex-end" }}>
                              <button type="button" className="ca-continue"
                          ...
                          <>
                            <label style={{ cursor: "pointer", display: "block" }}>
                              <div style={{
                                ...
                                borderRadius: RADIUS.chip, padding: "20px 16px",
                                textAlign: "center", background: t.pillBg,
                              }}>
                                <div style={{
                                  width: 36, height: 36, borderRadius: 9, margin: "0 auto 9px",
                                  background: `${meta.color}12`, border: `1px solid ${meta.color}28`,
                                ...
                                <div style={{ fontSize: 12, fontWeight: 600, color: t.textSub, fontFamily: FONT_FAMILY, marginBottom: 3 }}>
                                  Click to upload files
                                </div>
                                <div style={{ fontSize: 10.5, color: t.textMuted, fontFamily: FONT_FAMILY }}>
                                  PDF · DOC · DOCX · ZIP · TXT
                                </div>
                                <input type="file" multiple onChange={handleFileChange} hidden accept=".pdf,.doc,.docx,.zip,.txt" />
                              </div>
                            ...
                            {formData.attachments.length > 0 && (
                              <div style={{ borderRadius: RADIUS.chip, border: `1px solid ${t.border}`, overflow: "hidden" }}>
                                {formData.attachments.map((file, index) => (
                                  <div key={index} className="ca-file-row"
                                    style={{ borderBottom: index < formData.attachments.length - 1 ? `1px solid ${t.border}` : "none" }}>
                                    <div style={{ width: 24, height: 24, borderRadius: 6, background: `${ACCENT_PURPLE.base}18`, border: `1px solid ${ACCENT_PURPLE.base}30`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                                      <FileText size={11} color={ACCENT_PURPLE.base} />
                                    </div>
                                    <span style={{ fontSize: 11.5, color: t.text, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: FONT_FAMILY }}>
                                      {file.name}
                                    </span>
                                    <button type="button" onClick={() => removeFile(index)}
                                      style={{ width: 20, height: 20, borderRadius: "50%", border: `1px solid ${t.border}`, background: t.pillBg, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
                                      <X size={9} color={t.textMuted} />
                                    </button>
                                  </div>
                                ))}
                            ...
                            <div className="ca-summary-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                              {[
                                { label: "Title", val: formData.title || "—", color: ACCENT_PURPLE.base },
                                { label: "Batch", val: formData.batch ? `Batch ${formData.batch}` : "—", color: "#2563eb" },
                                { label: "Max Marks", val: formData.maxMarks || "—", color: "#059669" },
                              ].map(({ label, val, color }) => (
                                <div key={label} style={{ background: t.actBg, border: `1px solid ${t.border}`, borderTop: `2px solid ${color}50`, borderRadius: 9, padding: "9px 12px" }}>
                                  <div style={{ fontSize: 9, fontWeight: 700, color: t.textMuted, textTransform: "uppercase", letterSpacing: "0.07em", fontFamily: FONT_FAMILY, marginBottom: 4 }}>
                                    {label}
                                  </div>
                                  <div style={{ fontSize: 12.5, fontWeight: 700, color: t.text, fontFamily: FONT_FAMILY, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                    {val}
                                  </div>
                                </div>
                              ))}
                            ...
                            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", paddingTop: 2 }}>
                              <button type="button" onClick={() => setOpen("details")}
                                style={{ display: "flex", alignItems: "center", gap: 4, fontSize: 11.5, color: t.textMuted, background: "none", border: "none", cursor: "pointer", fontFamily: FONT_FAMILY, fontWeight: 500 }}>
                                <ChevronRight size={11} style={{ transform: "rotate(180deg)" }} /> Back
                              </button>
                              <button type="submit" className="ca-submit" disabled={isSubmitting}>
                                {isSubmitting ? (
