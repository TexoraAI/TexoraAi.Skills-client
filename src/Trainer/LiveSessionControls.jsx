...
/**
 * LiveSessionControls.jsx — TRAINER SIDE
 * UI updated to match Dashboard design system (Poppins, same color tokens, card style)
...
import { FaPhoneSlash, FaTimes, FaDotCircle, FaUsers, FaPaperPlane, FaMicrophone, FaMicrophoneSlash, FaVideo, FaVideoSlash, FaDesktop } from "react-icons/fa";
...
const LiveSessionControls = () => {
  ...
  const pushSystem = useCallback((text) => {
    setMessages((prev) => [...prev, { id: Date.now(), user: "System", text, time: getTime(), system: true }]);
  }, []);

  const refreshParticipants = useCallback(() => {
    const room = roomRef.current;
    if (!room) return;
    const list = [{ id: room.localParticipant.identity, name: room.localParticipant.name || "You (Trainer)", self: true, isHost: true }];
    room.remoteParticipants.forEach((p) => list.push({ id: p.identity, name: p.name || p.identity, self: false, isHost: false }));
    setParticipants(list);
  }, []);
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      } catch (err) { console.error("startLiveSession failed:", err); return; }
      ...
      try {
        ...
        setConnected(true);
        refreshParticipants();
      } catch (err) { console.error("LiveKit connect failed:", err); return; }
      ...
      } catch (err) { console.error("createLocalTracks failed:", err); }

      room.remoteParticipants.forEach((participant) => {
        participant.trackPublications.forEach((pub) => { if (pub.isSubscribed && pub.track) attachTrack(pub.track, remoteGridRef.current); });
      });

      room.on(RoomEvent.TrackSubscribed, (track) => attachTrack(track, remoteGridRef.current));
      room.on(RoomEvent.TrackUnsubscribed, (track) => track.detach().forEach((el) => el.remove()));
      room.on(RoomEvent.ParticipantConnected, (p) => { refreshParticipants(); pushSystem(`${p.name || p.identity} joined`); });
      room.on(RoomEvent.ParticipantDisconnected, (p) => { refreshParticipants(); pushSystem(`${p.name || p.identity} left`); });
      room.on(RoomEvent.DataReceived, (payload, participant) => {
        try {
          const decoded = new TextDecoder().decode(payload);
          const msg = JSON.parse(decoded);
          if (msg.text) setMessages((prev) => [...prev, { id: Date.now(), user: participant?.name || participant?.identity || "Student", text: msg.text, time: getTime(), self: false }]);
        } catch (_) {}
      });
      room.on(RoomEvent.Disconnected, () => setConnected(false));
    };
  ...
  useEffect(() => { chatEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, sidebarTab, sidebarOpen]);
  ...
  const toggleCam = useCallback(async () => {
    const track = localVideoTrackRef.current;
    if (!track) return;
    if (camOn) { await track.mute(); if (localPreviewRef.current) localPreviewRef.current.style.visibility = "hidden"; }
    else { await track.unmute(); if (localPreviewRef.current) localPreviewRef.current.style.visibility = "visible"; }
    setCamOn((v) => !v);
  }, [camOn]);
  ...
  const toggleScreen = useCallback(async () => {
    ...
    if (screenOn) {
      ...
      try {
        ...
        setScreenOn(true);
        const mediaTrack = screenTrack?.mediaStreamTrack ?? pub.track?.mediaStreamTrack ?? null;
        if (mediaTrack) {
      ...
      } catch (err) { if (err?.name !== "NotAllowedError") console.warn("Screen share failed:", err); }
    }
  }, [screenOn, restoreCamPip]);
  ...
  const sendMessage = useCallback(() => {
    const text = input.trim();
    if (!text) return;
    setMessages((prev) => [...prev, { id: Date.now(), user: "You (Trainer)", text, time: getTime(), self: true }]);
    setInput("");
    try {
    ...
    <div style={S.root}>
      ...
      <div style={S.topBar}>
        ...
        <div style={S.topRight}>
          ...
          <button style={{ ...S.recBtn, ...(recording ? S.recBtnOn : S.recBtnOff) }} onClick={() => setRecording((r) => !r)}>
      ...
      <div style={S.body}>
        ...
        <div style={S.videoArea}>
          {connected ? (
            <>
              <div ref={remoteGridRef} style={S.remoteGrid} />
              <div style={S.pip}>
                <div ref={localPreviewRef} style={{ width: "100%", height: "100%", visibility: camOn || screenOn ? "visible" : "hidden" }} />
                {!camOn && !screenOn && (
                  <div style={S.pipOff}>
                    <FaVideoSlash size={18} color="#64748b" />
                    <span style={S.pipOffTxt}>Cam Off</span>
              ...
              <div style={S.ctrlBar}>
                <CtrlBtn icon={micOn ? <FaMicrophone /> : <FaMicrophoneSlash />} label={micOn ? "Mute" : "Unmute"} danger={!micOn} onClick={toggleMic} />
                <CtrlBtn icon={camOn ? <FaVideo /> : <FaVideoSlash />} label={camOn ? "Stop Cam" : "Start Cam"} danger={!camOn} onClick={toggleCam} />
                <CtrlBtn icon={<FaDesktop />} label={screenOn ? "Stop Share" : "Share Screen"} active={screenOn} onClick={toggleScreen} />
                <CtrlBtn icon={<span style={{ fontSize: 16 }}>💬</span>} label="Chat" active={sidebarOpen && sidebarTab === "chat"} onClick={() => { if (sidebarOpen && sidebarTab === "chat") setSidebarOpen(false); else { setSidebarTab("chat"); setSidebarOpen(true); } }} />
                <CtrlBtn icon={<FaUsers />} label="People" active={sidebarOpen && sidebarTab === "participants"} onClick={() => { if (sidebarOpen && sidebarTab === "participants") setSidebarOpen(false); else { setSidebarTab("participants"); setSidebarOpen(true); } }} />
              </div>
            </>
          ) : (
            <div style={S.loadingBox}>
        ...
        <div style={S.handle} onClick={() => setSidebarOpen((o) => !o)} title="Toggle panel">
          <div style={S.handlePill}>
            {sidebarOpen ? (<><Chevron dir="right" /><div style={S.handleLine} /><Chevron dir="left" /></>) : (<><Chevron dir="left" /><div style={S.handleLine} /><Chevron dir="right" /></>)}
          </div>
        </div>
            ...
            <>
              ...
              {sidebarTab === "chat" && (
                <>
                      ...
                      <div key={m.id} style={m.system ? S.sysRow : m.self ? { ...S.msgBlock, alignItems: "flex-end" } : S.msgBlock}>
                        {m.system ? (
                          ...
                          <>
                            ...
                            <div style={{ ...S.msgBubble, ...(m.self ? S.bubbleSelf : S.bubbleOther) }}>{m.text}</div>
                          </>
                        )}
                      </div>
                    ))}
                  ...
                  <div style={S.inputRow}>
                    <input style={S.chatInput} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder="Message students…" />
                    <button style={S.sendBtn} onClick={sendMessage}><FaPaperPlane size={13} /></button>
                  </div>
                </>
              )}
              ...
              {sidebarTab === "participants" && (
                <div style={S.peopleList}>
                  {participants.length === 0 && (
                    <div style={S.emptyPeople}>
                      <FaUsers size={28} style={{ opacity: 0.3 }} />
                      <p style={{ fontSize: 12, color: "#475569", marginTop: 8, fontFamily: "'Poppins',sans-serif" }}>No participants yet</p>
                    </div>
                  )}
                  {participants.map((p) => <PersonRow key={p.id} name={p.name} isHost={p.isHost} self={p.self} />)}
                </div>
              )}
            </>
          )}
...
const Chevron = ({ dir }) => (
  <svg width="5" height="10" viewBox="0 0 6 12" fill="none">
    {dir === "right" ? <path d="M1 1L6 6L1 11" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round" /> : <path d="M5 1L0 6L5 11" stroke="#64748b" strokeWidth="1.5" strokeLinecap="round" />}
  </svg>
);
...
const TabBtn = ({ active, label, onClick }) => (
  <button onClick={onClick} style={{
    ...
    transition: "all .15s", ...(active ? { background: "rgba(34,211,238,0.06)" } : {}),
  }}>
    {label}
  </button>
);
...
const PersonRow = ({ name, isHost, self }) => (
  <div style={S.pRow}>
    <div style={{ ...S.pAv, background: self ? "linear-gradient(135deg,#0ea5e9,#6366f1)" : "linear-gradient(135deg,#8b5cf6,#ec4899)" }}>
      {(name || "?")[0].toUpperCase()}
    </div>
    <span style={S.pName}>{name}</span>
    {isHost && <span style={S.hostTag}>Host</span>}
...
const CtrlBtn = ({ icon, label, active, danger, onClick }) => {
  const [hov, setHov] = useState(false);
  const bg = danger ? (hov ? "#991b1b" : "#7f1d1d") : active ? (hov ? "rgba(34,211,238,.3)" : "rgba(34,211,238,.16)") : (hov ? "rgba(255,255,255,.14)" : "rgba(255,255,255,.07)");
  const col = danger ? "#fca5a5" : active ? "#67e8f9" : "#cbd5e1";
  return (
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{
      display: "flex", flexDirection: "column", alignItems: "center", gap: 5,
      background: bg, color: col,
      border: danger ? "1px solid rgba(239,68,68,.3)" : active ? "1px solid rgba(34,211,238,.3)" : "1px solid transparent",
      borderRadius: 14, padding: "10px 20px", cursor: "pointer", fontSize: 11, fontWeight: 600,
      fontFamily: "'Poppins',sans-serif", letterSpacing: 0.3, transition: "all .18s", minWidth: 68,
    }}>
      <span style={{ fontSize: 18 }}>{icon}</span>
      <span>{label}</span>
    </button>
...
/* ─────────────────────────────────────────────────────────────
   STYLES — updated to Poppins + Dashboard color tokens
───────────────────────────────────────────────────────────── */
const S = {
  root: {
    position: "fixed", inset: 0, zIndex: 9999, display: "flex", flexDirection: "column",
    background: "#07090f", color: "#e2e8f0", fontFamily: "'Poppins','DM Sans','Segoe UI',sans-serif", overflow: "hidden",
  },
  topBar: {
    flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between",
    padding: "8px 20px", background: "#0d1117", borderBottom: "1px solid rgba(255,255,255,.07)", zIndex: 10,
  },
  topLeft: { display: "flex", alignItems: "center", gap: 10 },
  topRight: { display: "flex", alignItems: "center", gap: 8 },
  liveBadge: {
    ...
    fontSize: 9, fontWeight: 800, letterSpacing: "0.15em", color: "#f43f5e", fontFamily: "'Poppins',sans-serif",
  },
  liveDot: {
  ...
  sessionTitle: { fontSize: 13, fontWeight: 700, color: "#f1f5f9", fontFamily: "'Poppins',sans-serif" },
  recBadge: {
    ...
    fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", color: "#fca5a5", fontFamily: "'Poppins',sans-serif",
  },
  pCountBadge: {
    display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", borderRadius: 10,
    background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.09)", fontSize: 11, color: "#94a3b8",
    fontFamily: "'Poppins',sans-serif",
  },
  recBtn: {
    display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 10,
    fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'Poppins',sans-serif",
    letterSpacing: "0.05em", transition: "all .15s",
  },
  recBtnOn: { background: "rgba(244,63,94,.15)", border: "1px solid rgba(244,63,94,.3)", color: "#f87171" },
  ...
  endBtn: {
    ...
    background: "linear-gradient(135deg,#dc2626,#f43f5e)", color: "#fff", fontSize: 12, fontWeight: 700,
    cursor: "pointer", fontFamily: "'Poppins',sans-serif", letterSpacing: "0.03em",
  },
  closeBtn: {
  ...
  remoteGrid: {
    ...
    gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))",
    gap: 4, padding: 4, alignContent: "center", paddingBottom: 80,
  },
  pip: {
    position: "absolute", bottom: 90, right: 16, width: 176, height: 118, borderRadius: 14,
    overflow: "hidden", border: "2px solid rgba(255,255,255,.12)", background: "#0d1117",
    boxShadow: "0 8px 32px rgba(0,0,0,.7)", zIndex: 10,
  },
  ...
  pipOffTxt: { fontSize: 10, color: "#475569", fontFamily: "'Poppins',sans-serif" },
  pipLabel: { position: "absolute", bottom: 6, left: 8, fontSize: 9, color: "#fff", background: "rgba(0,0,0,.6)", padding: "2px 8px", borderRadius: 6, pointerEvents: "none", fontFamily: "'Poppins',sans-serif", fontWeight: 600, letterSpacing: "0.04em" },
  ctrlBar: {
    ...
    borderTop: "1px solid rgba(255,255,255,.07)", zIndex: 20,
  },
  loadingBox: { position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14 },
  spinner: { width: 40, height: 40, border: "3px solid rgba(34,211,238,.2)", borderTop: "3px solid #22d3ee", borderRadius: "50%", animation: "spin .8s linear infinite" },
  loadingText: { fontSize: 12, color: "#475569", fontFamily: "'Poppins',sans-serif", fontWeight: 600 },
  handle: {
    flexShrink: 0, width: 14, display: "flex", alignItems: "center", justifyContent: "center",
    background: "#0d1117", borderLeft: "1px solid rgba(255,255,255,.05)", borderRight: "1px solid rgba(255,255,255,.05)",
    cursor: "pointer", zIndex: 5, transition: "background .2s",
  },
  handlePill: { display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "10px 3px", borderRadius: 8, background: "#1e293b", border: "1px solid rgba(255,255,255,.1)" },
  handleLine: { width: 1, height: 12, background: "rgba(255,255,255,.15)" },
  sidebar: { flexShrink: 0, background: "#0d1117", borderLeft: "1px solid rgba(255,255,255,.07)", display: "flex", flexDirection: "column", overflow: "hidden", transition: "width .25s ease" },
  tabRow: { flexShrink: 0, display: "flex", borderBottom: "1px solid rgba(255,255,255,.07)" },
  msgList: { flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 10, minHeight: 0 },
  sysRow: { display: "flex", justifyContent: "center" },
  sysBubble: { fontSize: 10, color: "#475569", fontStyle: "italic", background: "rgba(255,255,255,.04)", borderRadius: 8, padding: "3px 10px", fontFamily: "'Poppins',sans-serif" },
  msgBlock: { display: "flex", flexDirection: "column", gap: 2 },
  msgUser: { fontSize: 9, color: "#64748b", fontWeight: 700, fontFamily: "'Poppins',sans-serif", letterSpacing: "0.04em" },
  msgBubble: { maxWidth: "88%", padding: "7px 11px", borderRadius: 12, fontSize: 12, color: "#f1f5f9", lineHeight: 1.45, fontFamily: "'Poppins',sans-serif" },
  bubbleSelf: { background: "linear-gradient(135deg,#0e7490,#22d3ee)", alignSelf: "flex-end", borderBottomRightRadius: 2 },
  bubbleOther: { background: "#1e293b", borderBottomLeftRadius: 2 },
  inputRow: { flexShrink: 0, display: "flex", gap: 8, padding: "10px 12px", borderTop: "1px solid rgba(255,255,255,.07)" },
  chatInput: { flex: 1, background: "#1e293b", border: "1px solid rgba(255,255,255,.08)", borderRadius: 10, padding: "8px 12px", color: "#e2e8f0", fontSize: 12, fontFamily: "'Poppins',sans-serif", fontWeight: 500, outline: "none" },
  sendBtn: { flexShrink: 0, background: "linear-gradient(135deg,#0e7490,#22d3ee)", border: "none", borderRadius: 10, padding: "0 14px", color: "#0f172a", cursor: "pointer", display: "flex", alignItems: "center" },
  peopleList: { flex: 1, overflowY: "auto", padding: "10px 12px", display: "flex", flexDirection: "column", gap: 4, minHeight: 0 },
  emptyPeople: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 },
  pRow: { display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 10, background: "rgba(255,255,255,.03)" },
  pAv: { width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, flexShrink: 0, fontFamily: "'Poppins',sans-serif" },
  pName: { flex: 1, fontSize: 12, color: "#cbd5e1", fontFamily: "'Poppins',sans-serif", fontWeight: 500 },
  hostTag: { fontSize: 9, background: "rgba(34,211,238,.12)", color: "#67e8f9", padding: "2px 8px", borderRadius: 6, fontWeight: 700, fontFamily: "'Poppins',sans-serif", letterSpacing: "0.06em" },
  youTag: { fontSize: 9, background: "rgba(52,211,153,.12)", color: "#6ee7b7", padding: "2px 8px", borderRadius: 6, fontWeight: 700, fontFamily: "'Poppins',sans-serif", letterSpacing: "0.06em" },
};
