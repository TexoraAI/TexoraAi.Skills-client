...
const TexoraLogin = () => {
  ...
  const handleGoogleSuccess = async res => {
    try {
      ...
      if (resp?.isNewUser === true) {
        // USER DOES NOT EXIST → show full-page popup, NO dashboard access
        newUserRef.current = { name: dec.name, email: dec.email };
        setShowPopup(true);
      } else {
      ...
      console.error(err);
      // On backend error (user not found / not approved) → show popup instead of ugly alert
      try {
        const dec = jwtDecode(res.credential);
        newUserRef.current = { name: dec.name, email: dec.email };
      } catch (_) {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* ═══════════════════════════════════════════════════════
           FULL-PAGE POPUP — covers the entire browser viewport
        ═══════════════════════════════════════════════════════ */
        .pu-page {
      ...
      {/* ═══════════════════════════════════════════════════════════════════
          FULL-PAGE POPUP — takes over entire browser screen for new users
      ═══════════════════════════════════════════════════════════════════ */}
      {showPopup && (
        <div className="pu-page">
          {/* same warm background blobs as login page */}
