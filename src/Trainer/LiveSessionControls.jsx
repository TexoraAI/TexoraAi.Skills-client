...
//   // Bug 1: the sidebar used to show Trainer Controls, Screen Sharing,
//   // Join Requests and Participants all stacked together. This state
//   // just tracks which of the two new tabs is active; no other layout,
//   // styling, or logic changes.
//   const [sidebarTab, setSidebarTab] = useState("trainer");
...
//       pip.document.body.style.background = "#0F1117";
...
//   // Shape sortedLiveParticipants + the local trainer tile into the flat
//   // participant objects <LiveParticipantsPanel> expects. Read-only view
//   // model built from the same data the old inline peopleList used
//   // (dbMatch/displayLabel/raisedHands/screenTiles) — no new state.
...
//         presenting: screenTiles.some((st) => st.identity === t.identity),
//       };
//     }),
//   ];
...
//   // Lowers a single student's raised hand, mirroring the "Lower All
//   // Hands" button but targeted at one identity via the `identity`
//   // field on the trainer_command payload. Optimistically clears the
//   // local raisedHands entry (same pattern "Lower All Hands" uses with
//   // setRaisedHands({})); the student's own raiseHand:false echo will
//   // reconcile it either way.
//   const lowerStudentHand = useCallback(
//     (identity, name) => {
//       broadcastTrainerCommand("lowerHand", true, { identity });
//       setRaisedHands((prev) => {
...
//         return next;
//       });
//       pushSystem(`Lowered ${name || "a student"}'s hand.`);
//     },
//     [broadcastTrainerCommand, pushSystem],
//   );

//   // Generic dispatcher for the simple, non-toggle per-user commands
//   // (muteUser, unmuteUser, disableCamera, enableCamera, disableChat,
//   // enableChat). Each sends { type: "trainer_command", command,
//   // identity, value: true } over the same LiveKit data channel the
//   // global controls already use — the student side keys off
//   // `command` + `identity` alone, so `value` is just a constant
//   // truthy marker here, same shape the spec calls for.
//   const sendUserCommand = useCallback(
//     (command, identity, name, label) => {
//       broadcastTrainerCommand(command, true, { identity });
//       pushSystem(`${label}: ${name || identity}`);
//     },
//     [broadcastTrainerCommand, pushSystem],
//   );
...
//   // Single dispatcher fed to <LiveTrainerControlsGrid onCommand={...}/>.
//   // Mirrors exactly what each old inline TCBtn onClick used to do
//   // (same setTrainerFlags/broadcastTrainerCommand/pushSystem calls,
//   // same system-log copy) — UI wiring only, no new behavior.
//   const trainerCommandLabels = {
//     allMuted: ["All participants muted.", "Requested all participants to unmute."],
//     camerasDisabled: ["All cameras disabled.", "Requested all participants to enable cameras."],
//     chatDisabled: ["Chat disabled for participants.", "Chat enabled for participants."],
//     locked: ["Meeting locked — no new participants can join.", "Meeting unlocked."],
//     micRequestsAllowed: ["Mic requests are now allowed.", "Mic requests are now blocked."],
//     screenShareBlocked: ["Screen sharing blocked for participants.", "Screen sharing allowed for participants."],
//   };
//   const handleTrainerCommand = useCallback(
//     (key, value) => {
//       if (key === "attendance") {
...
//       }
//       if (key === "handsLowered") {
//         pushSystem("All raised hands lowered.");
//         broadcastTrainerCommand("handsLowered", true);
//         setRaisedHands({});
//         return;
//       }
//       setTrainerFlags((prev) => ({ ...prev, [key]: value }));
//       broadcastTrainerCommand(key, value);
//       if (key === "allMuted" && value === false) {
//         broadcastTrainerCommand("requestUnmuteAll", true);
//       }
//       const labels = trainerCommandLabels[key];
//       if (labels) pushSystem(value ? labels[0] : labels[1]);
//     },
//     [pushSystem, broadcastTrainerCommand, handleEndSession],
//   );

//   // Fed to <LiveParticipantsPanel onAction={...}/> → mirrors exactly
//   // what the old inline per-row <ParticipantMenu onAction={...}/> did.
//   const handleParticipantAction = useCallback(
//     (participant, item) => {
//       const { identity, name } = participant;
//       if (item.command === "stopScreenShare") {
//         stopStudentScreenShare(identity, name);
//       } else if (item.command === "lowerHand") {
//         lowerStudentHand(identity, name);
//       } else if (item.command) {
//         sendUserCommand(item.command, identity, name, item.label);
//       } else {
//         pushSystem(`${item.label}: ${name}`);
//       }
//     },
//     [stopStudentScreenShare, lowerStudentHand, sendUserCommand, pushSystem],
//   );
...
//         @keyframes speakBar{0%,100%{height:4px}50%{height:14px}}
//         @keyframes speakGlow{0%,100%{box-shadow:0 0 0 3px rgba(59,130,246,.32),0 16px 36px rgba(0,0,0,.4)}50%{box-shadow:0 0 0 5px rgba(59,130,246,.5),0 16px 36px rgba(0,0,0,.4)}}
//         .ilm-speaking{animation:speakGlow 1.4s ease-in-out infinite}
...
//         .ilm-hoverscale:active{transform:scale(.96)}
//         .ilm-card{transition:transform .2s ease,box-shadow .2s ease,border-color .2s ease}
//         .ilm-card:hover{transform:scale(1.015);box-shadow:0 22px 44px rgba(0,0,0,.5)}
...
//         .ilm-tcbtn:active{transform:translateY(0) scale(.96);filter:brightness(.95)}
//         .ilm-prow{transition:background .15s ease,transform .15s ease}
//         .ilm-prow:hover{background:rgba(255,255,255,.055) !important;transform:translateX(1px)}
//         .ilm-ctrlbtn:active{transform:translateY(0) scale(.94) !important}
...
//                       <div
//                         key={t.id}
//                         className={`ilm-card${t.isSpeaking ? " ilm-speaking" : ""}`}
//                         style={{
//                           ...S.gridTile,
//                           ...(t.isLocal && !t.isSpeaking
//                             ? {
//                                 border: "1px solid #3B82F6",
//                                 boxShadow:
//                                   "0 0 0 3px rgba(59,130,246,.28), 0 16px 36px rgba(0,0,0,.4)",
//                               }
//                             : null),
//                         }}
//                       >
...
//                         />
//                         <button
//                           style={{ ...S.tileMoreBtn, pointerEvents: "auto" }}
//                           onClick={() =>
//                             setOpenParticipantMenuId((cur) => (cur === t.id ? null : t.id))
//                           }
//                           title="More"
//                         >
...
//                           <div style={{ position: "absolute", top: 40, right: 10, zIndex: 20 }}>
//                             <ParticipantMenu
//                               onClose={() => setOpenParticipantMenuId(null)}
//                               isPresenting={screenTiles.some((st) => st.identity === t.identity)}
//                               handRaised={!!raisedHands[t.identity]}
//                               onAction={(item) => {
//                                 if (item.command === "stopScreenShare") {
//                                   stopStudentScreenShare(t.identity, t.name);
//                                 } else if (item.command === "lowerHand") {
//                                   lowerStudentHand(t.identity, t.name);
//                                 } else if (item.command) {
//                                   sendUserCommand(item.command, t.identity, t.name, item.label);
//                                 } else {
//                                   pushSystem(`${item.label}: ${t.name}`);
//                                 }
...
//               {/* Bug 1: two independent tabs replace the single stacked
//                   sidebar. Only the active tab's panels render below —
//                   no layout, color, spacing, or other styling changes. */}
//               <div style={S.sidebarTabRow}>
//                 <button
//                   className="ilm-hoverscale"
//                   style={{ ...S.sidebarTabBtn, ...(sidebarTab === "trainer" ? S.sidebarTabBtnActive : null) }}
//                   onClick={() => setSidebarTab("trainer")}
//                 >
//                   Trainer Controls
//                 </button>
//                 <button
//                   className="ilm-hoverscale"
//                   style={{ ...S.sidebarTabBtn, ...(sidebarTab === "participants" ? S.sidebarTabBtnActive : null) }}
...
//                 <LiveTrainerControlsGrid flags={trainerFlags} onCommand={handleTrainerCommand} />
//               </div>
...
//                             border: isFocused ? "1px solid rgba(96,165,250,.35)" : "1px solid rgba(255,255,255,0.08)",
//                           }}
//                         >
...
//                             display: "flex", alignItems: "center", justifyContent: "center",
//                             width: 30, height: 30, borderRadius: 8, cursor: "pointer",
//                             border: "1px solid rgba(34,197,94,.4)", background: "rgba(34,197,94,.14)", color: "#22C55E",
//                           }}
//                         >
//                           <UserCheck size={15} />
//                         </button>
...
//                   <div style={{ ...S.pAv, background: "linear-gradient(135deg,#3B82F6,#60A5FA)" }}>T</div>
//                   <div style={{ flex: 1 }}>
...
//             />
//           </div>
//           {!isCompactDevice && <div style={S.dockDivider} />}
...
// const VideoTile = ({ tile, small, large, device = "desktop", handRaised, tileFloaters, zoom, hideNameTag, overlay, isSpeaking }) => {
...
//   // `isSpeaking` is purely additive: it only lights up if a caller ever
//   // passes `true`. Nothing in the current data model sets this yet, so
//   // this has no effect until real active-speaker detection is wired up —
//   // it's here so the Meet-style glow is ready to switch on later without
//   // any further UI changes.
//   const speaking = !!isSpeaking && !tile?.isScreen;
...
//       {overlay && !tile?.isScreen && <div style={S.tileGradientTop} aria-hidden="true" />}
//       {overlay && !tile?.isScreen && <div style={S.tileGradientBottom} aria-hidden="true" />}
...
//       {overlay && !tile?.isScreen && (
//         <div style={S.tileOverlayBar}>
//           <div style={S.tileOverlayLeft}>
//             <span style={S.tileOverlayName}>{tile?.name}</span>
//             {tile?.isHost && <span style={S.tileOverlayHostTag}>Host</span>}
//           </div>
//           <div style={S.tileOverlayRight}>
//             <span style={{ ...S.tileOverlayIcon, ...(tile?.micMuted ? S.tileOverlayIconMuted : null) }}>
//               {tile?.micMuted ? <MicOff size={13} strokeWidth={2.2} /> : <Mic size={13} strokeWidth={2.2} />}
//             </span>
//             <span style={{ ...S.tileOverlayIcon, ...(tile?.videoMuted ? S.tileOverlayIconMuted : null) }}>
//               {tile?.videoMuted ? <VideoOff size={13} strokeWidth={2.2} /> : <Video size={13} strokeWidth={2.2} />}
//             </span>
//           </div>
//         </div>
//       )}
...
//       {speaking && (
//         <span style={S.tileSpeakingDot} title="Speaking">
//           <span style={S.tileSpeakingBar} />
//           <span style={{ ...S.tileSpeakingBar, animationDelay: ".12s" }} />
//           <span style={{ ...S.tileSpeakingBar, animationDelay: ".24s" }} />
//         </span>
//       )}
...
//   borderRadius: 12,
//   border: "none",
//   background: danger ? "rgba(239,68,68,.18)" : "rgba(255,255,255,0.08)",
...
//   <div style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column", background: "#0F1117", fontFamily: "'Inter',sans-serif" }}>
...
//       </div>
//     </div>
//     <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", gap: 8, padding: 10, background: "#181A20", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
...
//   green: { bg: "rgba(34,197,94,.12)", border: "rgba(34,197,94,.28)", color: "#22C55E" },
//   amber: { bg: "rgba(251,191,36,.12)", border: "rgba(251,191,36,.28)", color: "#FACC15" },
//   purple: { bg: "rgba(139,92,246,.14)", border: "rgba(139,92,246,.3)", color: "#a78bfa" },
//   blue: { bg: "rgba(59,130,246,.14)", border: "rgba(59,130,246,.3)", color: "#60A5FA" },
// };
...
// const ParticipantMenu = ({ onClose, onAction, isPresenting, handRaised }) => {
//   const ref = useRef(null);
//   useEffect(() => {
//     const handler = (e) => {
...
//   const items = [
//     { label: "Mute", icon: <MicOff size={13} />, command: "muteUser" },
//     { label: "Unmute", icon: <Mic size={13} />, command: "unmuteUser" },
...
//     // Only offered when this participant is actually presenting /
//     // has a hand raised — keeps the menu uncluttered the rest of the
//     // time, same spirit as the spec's "(if user is sharing)".
//     ...(isPresenting
...
//       : []),
//     { label: "Spotlight", icon: <Pin size={13} />, command: null },
//     { label: "Remove Participant", icon: <Trash2 size={13} />, danger: true, command: null },
//   ];
...
//       : active
//         ? hov ? "rgba(59,130,246,.32)" : "rgba(59,130,246,.2)"
//         : hov ? "rgba(255,255,255,.12)" : "rgba(255,255,255,.05)";
//   const col = danger ? "#fca5a5" : gold && active ? "#FACC15" : active ? "#93C5FD" : "#E5E7EB";
//   return (
//     <div style={{ position: "relative" }}>
//       <button
//         className="ilm-ctrlbtn"
//         onClick={onClick}
...
//           background: bg,
//           color: col,
//           border: gold && active ? "1px solid rgba(250,204,21,.4)" : "1px solid transparent",
//           borderRadius: 16,
...
//     flexWrap: "wrap",
//   },
//   topBarCompact: { padding: "10px 12px", gap: 8 },
//   topLeft: { display: "flex", alignItems: "center", gap: 10, minWidth: 0, flexWrap: "wrap" },
...
//     overflow: "hidden",
//     minWidth: 0,
//     background: "radial-gradient(circle at 50% 0%, #161922 0%, #0F1117 60%)",
//   },
//   stageWrap: { position: "absolute", inset: 0, display: "flex", flexDirection: "column", gap: 16, padding: "22px" },
//   stageWrapCompact: { padding: "10px", gap: 10 },
//   gridWrap: { position: "absolute", inset: 0, padding: "20px", overflowY: "auto" },
//   gridWrapCompact: { padding: "10px" },
//   videoGrid: {
//     display: "grid",
//     gap: 16,
...
//     borderRadius: 20,
//     overflow: "hidden",
//     background: "linear-gradient(165deg,rgba(38,42,54,.9),rgba(11,13,18,.98))",
//     backdropFilter: "blur(6px)",
//     WebkitBackdropFilter: "blur(6px)",
//     border: "1px solid rgba(255,255,255,0.1)",
//     boxShadow: "0 16px 36px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.04)",
//     aspectRatio: "16 / 10",
...
//     borderRadius: 24,
//     overflow: "hidden",
//     background: "linear-gradient(160deg,#1c202b 0%,#12141b 55%,#0a0b0f 100%)",
//     border: "1px solid rgba(255,255,255,.12)",
//     boxShadow:
//       "0 8px 16px rgba(0,0,0,.3), 0 32px 72px rgba(0,0,0,.55), 0 0 0 1px rgba(255,255,255,.05), inset 0 0 0 1px rgba(255,255,255,.04)",
//     animation: "presentIn .28s ease",
...
//   },
//   spotlightBtnOn: { color: "#93C5FD", border: "1px solid rgba(96,165,250,.4)", background: "rgba(59,130,246,.22)" },
//   presentingChip: {
//     position: "absolute",
//     top: 14,
//     left: "50%",
...
//     fontWeight: 700,
//   },
//   filmstripVideoWrap: { width: "100%", height: "100%", position: "relative", background: "#181A20", borderRadius: 12, overflow: "hidden" },
...
//     whiteSpace: "nowrap",
//   },
//   /* ── Meet-style tile overlay: subtle top/bottom gradients so the
//      name, badges, and mic/camera chips stay legible over any video,
//      plus the bottom info bar itself (name+host left, mic/cam right). */
//   tileGradientTop: {
//     position: "absolute",
//     top: 0,
//     left: 0,
...
//     borderRadius: 999,
//   },
//   tileOverlayRight: { display: "flex", alignItems: "center", gap: 6, flexShrink: 0 },
//   tileOverlayIcon: {
//     display: "flex",
//     alignItems: "center",
//     justifyContent: "center",
...
//     color: "#E5E7EB",
//   },
//   tileOverlayIconMuted: { background: "rgba(239,68,68,.28)", color: "#fca5a5" },
//   tileMoreBtn: {
//     position: "absolute",
//     top: 10,
//     right: 10,
...
//     cursor: "pointer",
//   },
//   /* Speaking indicator: three small animated bars, Meet-style, shown
//      only when a tile is explicitly marked as the active speaker. */
//   tileSpeakingDot: {
//     position: "absolute",
//     bottom: 10,
//     right: 10,
...
//     borderRadius: 2,
//     background: "#22C55E",
//     animation: "speakBar .9s ease-in-out infinite",
//   },
//   floatingChat: {
//     position: "absolute",
//     bottom: 16,
...
//   chatOnline: { marginLeft: "auto", fontSize: 11, color: "#22C55E", fontWeight: 600 },
...
//   bubbleSelf: { background: "#3B82F6", alignSelf: "flex-end", borderBottomRightRadius: 4 },
//   bubbleOther: { background: "#181A20", border: "1px solid rgba(255,255,255,0.08)", borderBottomLeftRadius: 4 },
//   inputRow: { flexShrink: 0, display: "flex", gap: 8, padding: "10px 12px", borderTop: "1px solid rgba(255,255,255,0.08)" },
//   chatInput: {
//     flex: 1,
//     background: "#181A20",
//     border: "1px solid rgba(255,255,255,0.08)",
...
//   spinner: { width: 40, height: 40, border: "3px solid rgba(59,130,246,.2)", borderTop: "3px solid #3B82F6", borderRadius: "50%", animation: "spin .8s linear infinite" },
...
//   },
//   sidebarInner: { flex: 1, display: "flex", flexDirection: "column", minHeight: 0, overflowY: "auto", gap: 12, padding: 14 },
//   sidebarTabRow: {
//     flexShrink: 0,
//     display: "flex",
//     gap: 6,
...
//     borderBottom: "1px solid rgba(255,255,255,0.08)",
//   },
//   panelTitle: { fontSize: 14, fontWeight: 700, color: "#fff", letterSpacing: "0.01em" },
//   sidebarCloseBtn: { display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, borderRadius: 10, border: "none", background: "rgba(255,255,255,.05)", color: "#B4B9C7", cursor: "pointer" },
//   tcGrid: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, padding: 14 },
//   searchRow: {
//     flexShrink: 0,
//     display: "flex",
//     alignItems: "center",
...
//   peopleList: { flex: 1, overflowY: "auto", padding: "0 12px 12px", display: "flex", flexDirection: "column", gap: 8, minHeight: 0 },
...
//   },
//   pAv: { width: 32, height: 32, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800, flexShrink: 0, color: "#fff", boxShadow: "0 4px 12px rgba(0,0,0,.3)" },
//   pName: { flex: 1, fontSize: 12.5, color: "#E5E7EB", fontWeight: 500 },
//   pJoinedText: { fontSize: 10, color: "#6B7280", marginTop: 1 },
//   hostTag: { fontSize: 9.5, background: "rgba(59,130,246,.15)", color: "#93C5FD", padding: "3px 9px", borderRadius: 999, fontWeight: 700, letterSpacing: "0.04em" },
//   pHandBadge: {
//     display: "flex",
//     alignItems: "center",
//     justifyContent: "center",
...
//     overflowX: "auto",
//   },
//   ctrlBarCompact: { padding: "10px 8px 14px" },
//   /* Floating glass dock — the pill that actually holds the controls,
//      centered and hugging its content instead of stretching edge to edge. */
//   ctrlDock: {
//     display: "flex",
//     alignItems: "center",
//     gap: 14,
...
//     WebkitBackdropFilter: "blur(20px)",
//     border: "1px solid rgba(255,255,255,0.08)",
//     boxShadow: "0 20px 48px rgba(0,0,0,.45), inset 0 0 0 1px rgba(255,255,255,.03)",
//     maxWidth: "100%",
//   },
//   dockLeft: { display: "flex", alignItems: "center", gap: 6, flexWrap: "nowrap", overflowX: "auto" },
//   dockDivider: { width: 1, height: 28, background: "rgba(255,255,255,.1)", flexShrink: 0 },
//   ctrlBadge: {
//     position: "absolute",
//     top: -4,
//     right: -4,
...
const LiveSessionControls = () => {
  ...
  // Opens the sidebar on a given tab; if that tab is already open, collapses
  // the sidebar instead (toggle behaviour), matching how the bottom toolbar
  // buttons already worked before Chat became a sidebar tab.
          ...
          {sidebarOpen && (
            <div style={{ ...S.sidebarInner, animation: "panelIn .2s ease" }}>
              ...
              <div style={S.sidebarTabRow}>
                ...
                <button
                  className="ilm-hoverscale"
                  style={{ ...S.sidebarTabBtn, ...(sidebarTab === "chat" ? S.sidebarTabBtnActive : null) }}
              ...
              {sidebarTab === "chat" && (
                <div style={{ ...S.panelBlock, flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
                  <div style={S.panelHead}>
                    <span style={S.panelTitle}>Live Chat</span>
                    <span style={S.chatOnline}>{totalLiveParticipants} online</span>
                  </div>
                  <div style={S.msgList}>
                    {messages.map((m) => (
        ...
        <div style={S.ctrlDock}>
          <div style={S.dockLeft}>
            ...
            <CtrlBtn
              icon={<Crown size={18} strokeWidth={2.1} />}
              label="Trainer"
              active={sidebarOpen && sidebarTab === "trainer"}
              gold
              onClick={() => openSidebarTab("trainer")}
              compact={deviceCfg.ctrlCompact}
            />
          </div>
          {!isCompactDevice && <div style={S.dockDivider} />}
...
const S = {
  ...
  chatOnline: { marginLeft: "auto", fontSize: 11, color: "#22C55E", fontWeight: 600 },
  chatCloseBtn: { background: "none", border: "none", color: "#8b90a0", cursor: "pointer", display: "flex", padding: 2 },
  msgList: { flex: 1, overflowY: "auto", padding: "12px 14px", display: "flex", flexDirection: "column", gap: 12, minHeight: 80 },
  sysRow: { display: "flex", justifyContent: "center" },
  sysBubble: {
    fontSize: 10.5,
    color: "#8b90a0",
