...
if (
  ...
  !document.getElementById("doc-viewer-styles")
) {
...
const DocxViewer = ({ arrayBuffer }) => {
  ...
  return (
    ...
    >
      <div
        className="docprev-docx-content"
        style={{
          maxWidth: 860,
          margin: "0 auto",
      ...
      {sheetNames.length > 1 && (
        ...
        >
          {sheetNames.map((name) => (
            <button
              key={name}
              className={`docprev-xlsx-tab${activeSheet === name ? " active" : ""}`}
            ...
            slide.map((text, i) => (
              <p
                ...
                style={{
                  margin: 0,
                  fontSize: i === 0 ? 20 : 14,
                  fontWeight: i === 0 ? FONT_WEIGHT.bold : FONT_WEIGHT.regular,
...
const Documents = () => {
  ...
  const openPreview = async (file) => {
    try {
      ...
      if (["docx", "xlsx", "pptx"].includes(mode)) {
        // These need raw bytes for mammoth/XLSX.js/JSZip — requires S3
        // CORS to allow this origin for GET.
        const arrayBuffer = await fileService.fetchViewBytes(meta.url);
        setPreview({ mode, arrayBuffer, name: file.originalName });
        return;
      }

      // pdf / image / text — hand the presigned S3 URL straight to the
      // <iframe>/<img>, no blob fetch, no CORS involved.
      setPreview({ mode, url: meta.url, name: file.originalName });
    } catch (err) {
  ...
  const stats = [
    ...
    {
      label: "Previewed",
      numericValue: downloadedFileIds.length,
      change: `${Math.max(docs.length - downloadedFileIds.length, 0)} remaining`,
      trend: "up",
      icon: CheckCircle,
      colorKey: "green",
    },
    {
      label: "Progress",
      numericValue: downloadPercentage,
      isPercent: true,
      change: downloadPercentage >= 100 ? "All done!" : "Keep going",
      trend: downloadPercentage >= 50 ? "up" : "down",
      icon: TrendingUp,
      colorKey: "purple",
    },
  ];
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {lockedCount > 0 && (
          ...
          >
            ...
            >
              🔒 {lockedCount} more document{lockedCount > 1 ? "s" : ""} locked
            </p>
            <p
              style={{
                fontSize: 11.5,
            ...
            >
              Upgrade your plan to unlock the full document library.
            </p>
            <button
            ...
            {preview.mode === "none" && (
              ...
              >
                ...
                >
                  ...
                  <p style={{ margin: 0, fontSize: FONT_SIZE.bodySmall }}>
