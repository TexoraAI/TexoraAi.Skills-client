...
// /* ═══════════════════════════════════════════════
//    HERO COLOR HELPER — generates a lighter shade for gradient
// ═══════════════════════════════════════════════ */
// const hexToRgb = (hex) => {
...
// /* Returns "r,g,b" tuple string for use inside rgba() */
// const hexToRgbStr = (hex) => {
//   try { return hexToRgb(hex).join(","); } catch { return "124,58,237"; }
// };

// /* ─── Predefined hero color swatches ─── */
// const HERO_SWATCHES = [
...
//   /* ── NEW: Hero editor state ── */
//   const [heroColor, setHeroColor]     = useState("#7c3aed");
//   const [heroSubtitle, setHeroSubtitle] = useState("Student Portal");
//   const [heroTitle, setHeroTitle]     = useState("Student Dashboard");
//   const [heroTagline, setHeroTagline] = useState("Your classroom & learning overview");
...
//   /* ── Derived hero gradient — uses rgba so it's visible in both light & dark ── */
//   const heroRgb = hexToRgbStr(heroColor);
//   /* Strong enough to be visible on white (light) and dark backgrounds */
//   const heroBg = isDark
//     ? `linear-gradient(135deg, rgba(${heroRgb},0.30) 0%, rgba(${heroRgb},0.10) 100%)`
//     : `linear-gradient(135deg, rgba(${heroRgb},0.22) 0%, rgba(${heroRgb},0.08) 100%), #ffffff`;
//   const heroBorderColor = `rgba(${heroRgb},0.45)`;

//   /* ── Shared input style for editor ── */
//   const editorInput = {
...
//         .livebadge{animation:pulse-ring 2.2s ease-out infinite}
//         @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}
//         .hero-editor-input:focus { border-color: var(--hero-accent) !important; }
//       `}</style>

//       <div style={{ minHeight: "100vh", background: t.pageBg, color: t.text, fontFamily: "'Poppins',sans-serif", transition: "background 0.3s,color 0.3s" }}>
//         <div style={{ position: "relative", zIndex: 1, padding: 24, maxWidth: 1300, margin: "0 auto", paddingBottom: 52 }}>

//           {/* ═══ HERO — background + text driven by state ═══ */}
//           <div className="dfade" style={{
//             borderRadius: 24,
//             padding: "28px 32px",
//             background: heroBg,
...
//               <div>
//                 <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
//                   <div style={{ width: 6, height: 6, borderRadius: "50%", background: heroColor }} className="d1" />
//                   <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: t.textSub, fontFamily: "'Poppins',sans-serif" }}>
//                     {heroSubtitle}
//                   </span>
//                 </div>
...
//                     <button key={tab} onClick={() => setActiveTab(tab)} style={{
//                       padding: "6px 16px", borderRadius: 10,
//                       border: `1px solid ${activeTab === tab ? `rgba(${heroRgb},0.50)` : t.borderHov}`,
//                       background: activeTab === tab ? `rgba(${heroRgb},0.12)` : t.actBg,
//                       color: activeTab === tab ? heroColor : t.textSub,
//                       fontSize: 11, fontWeight: 600, cursor: "pointer",
//                       fontFamily: "'Poppins',sans-serif", textTransform: "capitalize", transition: "all 0.2s",
//                     }}>
//                       {tab.charAt(0).toUpperCase() + tab.slice(1)}
...
//                     background: showEditor ? `rgba(${heroRgb},0.13)` : t.actBg,
//                     border: `1px solid ${showEditor ? `rgba(${heroRgb},0.40)` : t.actBorder}`,
//                     color: showEditor ? heroColor : t.actIcon,
...
//                 <div className="livebadge" style={{ display: "flex", alignItems: "center", gap: 7, background: `rgba(${heroRgb},0.10)`, border: `1px solid rgba(${heroRgb},0.35)`, borderRadius: 999, padding: "8px 18px", color: heroColor, fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", fontFamily: "'Poppins',sans-serif" }}>
//                   <span style={{ width: 6, height: 6, borderRadius: "50%", background: heroColor, display: "inline-block" }} />LIVE
//                 </div>
//               </div>
//             </div>

//             {/* ═══════════════════════════════════════════
//                 HERO EDITOR PANEL — inline, below hero row
//             ═══════════════════════════════════════════ */}
//             {showEditor && (
//               <div style={{
//                 marginTop: 20,
//                 paddingTop: 18,
//                 borderTop: `1px solid rgba(${heroRgb},0.20)`,
//                 display: "flex", flexWrap: "wrap", gap: 20, alignItems: "flex-start",
//               }}>

//                 {/* Color swatches */}
//                 <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
//                   <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: t.textLabel, fontFamily: "'Poppins',sans-serif" }}>Theme Color</span>
//                   <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
//                     {HERO_SWATCHES.map((sw) => (
//                       <button
//                         key={sw.value}
//                         title={sw.label}
//                         onClick={() => setHeroColor(sw.value)}
//                         style={{
//                           width: 26, height: 26, borderRadius: 8, border: `2px solid ${heroColor === sw.value ? sw.value : "transparent"}`,
//                           background: sw.value, cursor: "pointer", outline: "none",
//                           boxShadow: heroColor === sw.value ? `0 0 0 3px ${sw.value}40` : "none",
...
//                       border: `2px solid ${t.actBorder}`, cursor: "pointer",
//                       display: "flex", alignItems: "center", justifyContent: "center",
//                       background: t.actBg, position: "relative",
//                     }}>
...
//                   {[
//                     { label: "Subtitle", value: heroSubtitle, setter: setHeroSubtitle, placeholder: "e.g. Student Portal" },
//                     { label: "Main Title", value: heroTitle, setter: setHeroTitle, placeholder: "e.g. Student Dashboard" },
//                     { label: "Tagline", value: heroTagline, setter: setHeroTagline, placeholder: "e.g. Your classroom overview" },
//                   ].map((field) => (
//                     <div key={field.label} style={{ display: "flex", flexDirection: "column", gap: 5, flex: "1 1 160px", minWidth: 140 }}>
//                       <label style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: t.textLabel, fontFamily: "'Poppins',sans-serif" }}>
//                         {field.label}
//                       </label>
//                       <input
//                         className="hero-editor-input"
//                         type="text"
//                         value={field.value}
//                         onChange={(e) => field.setter(e.target.value)}
//                         placeholder={field.placeholder}
//                         style={{ ...editorInput, "--hero-accent": heroColor }}
//                       />
//                     </div>
//                   ))}
//                 </div>
...
//                 {/* Reset button */}
//                 <button
//                   onClick={() => { setHeroColor("#7c3aed"); setHeroSubtitle("Student Portal"); setHeroTitle("Student Dashboard"); setHeroTagline("Your classroom & learning overview"); }}
//                   style={{ alignSelf: "flex-end", padding: "6px 14px", borderRadius: 8, background: "transparent", border: `1px solid ${t.border}`, color: t.textMuted, fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "'Poppins',sans-serif", transition: "all 0.2s", whiteSpace: "nowrap" }}
//                 >
//                   Reset
//                 </button>
//               </div>
...
//           {activeTab === "overview" && <OverviewPage />}
//           {activeTab === "courses" && (
//             <CoursesTab courses={courses} progressMap={progressMap} loading={courseProgressLoading} t={t} />
//           )}
//           {activeTab === "progress" && (
//             <ProgressTab
//               courses={courses} progressMap={progressMap} loading={courseProgressLoading}
//               videoProgress={videoProgress} fileProgress={fileProgress}
//               assignmentProgress={assignmentProgress} quizProgress={quizProgress}
//               t={t}
//             />
//           )}
//         </div>
