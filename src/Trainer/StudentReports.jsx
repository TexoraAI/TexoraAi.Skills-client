...
import { useState, useEffect } from "react";
import { Users, Search, BookOpen, Video, FileText, ClipboardList, TrendingUp, ChevronRight } from "lucide-react";
import { progressService } from "../services/progressService";
import { getTrainerBatches } from "../services/batchService";
...
function fmt(v) { return typeof v === "number" ? Math.round(v) : 0; }
function initials(email = "") {
  const name = email.split("@")[0];
  const parts = name.split(/[._-]/);
  return parts.length > 1 ? (parts[0][0] + parts[1][0]).toUpperCase() : name.slice(0, 2).toUpperCase();
}
function progColor(v) { return v >= 75 ? METRIC_COLORS.overall : v >= 50 ? METRIC_COLORS.assignment : "#ef4444"; }
...
function Loader({ t, text }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, color: t.textMuted, fontSize: 13, fontFamily: FONT_FAMILY }}>
      <span style={{ width: 20, height: 20, borderRadius: "50%", border: `2px solid ${ACCENT_PURPLE.base}33`, borderTopColor: ACCENT_PURPLE.base, display: "inline-block", animation: "srspin .8s linear infinite" }} />
      <span>{text}</span>
    </div>
  );
}
...
function ArcMeter({ value, color, size = 80 }) {
  ...
  return (
    <svg viewBox="0 0 88 52" style={{ width: size, height: size * 0.6 }}>
      <path d={`M ${cx - r} ${cy} A ${r} ${r} 0 0 1 ${cx + r} ${cy}`} fill="none" stroke="rgba(128,128,128,0.18)" strokeWidth="5" strokeLinecap="round" />
      ...
      <text x={cx} y={cy - 3} textAnchor="middle" fontSize="12" fontWeight="700" fill={color} fontFamily={FONT_FAMILY}>{fmt(value)}%</text>
    </svg>
  );
}
...
function RadarChart({ data, isDark }) {
  ...
  return (
    <svg viewBox="0 0 260 240" style={{ width: "100%", maxWidth: 220 }}>
      ...
      {axes.map((_, i) => { const [x2, y2] = pt(i, 100); return <line key={i} x1={cx} y1={cy} x2={x2} y2={y2} stroke={ringStroke} strokeWidth="1" />; })}
      <polygon points={poly} fill={radarFill} stroke={radarStroke} strokeWidth="1.5" />
      {axes.map((a, i) => { const [x, y] = pt(i, a.val); return <circle key={i} cx={x} cy={y} r={3} fill={radarStroke} />; })}
      {axes.map((a, i) => {
        const [x, y] = pt(i, 115);
        return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="10" fill={labelColor} fontFamily={FONT_FAMILY}>{a.label}</text>;
      })}
    </svg>
  );
}
  ...
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
      ...
      <circle cx={cx} cy={cy} r={r} fill="none" stroke={color} strokeWidth={strokeW} strokeLinecap="round"
        strokeDasharray={`${progress} ${circ}`} style={{ transition: "stroke-dasharray 1.2s cubic-bezier(0.4,0,0.2,1)" }} />
    </svg>
  );
}
...
export default function StudentReports() {
  ...
  useEffect(() => {
    ...
    setStudentDetail(null);
    progressService.getBatchProgressReport(selectedBatchId)
      .then((res) => { setStudents(res.data?.studentReports || []); setStudentsLoading(false); })
      .catch((err) => { setStudentsError(err.response?.data?.message || err.message); setStudentsLoading(false); });
  }, [selectedBatchId]);

  useEffect(() => {
    if (!selectedStudent) return;
    setDetailLoading(true);
    progressService.getStudentProgressInBatch(selectedStudent.batchId || selectedBatchId, selectedStudent.studentEmail)
      .then((res) => { setStudentDetail(res.data); setDetailLoading(false); })
      .catch(() => { setStudentDetail(selectedStudent); setDetailLoading(false); });
  }, [selectedStudent]);

  const filtered = students.filter((s) => s.studentEmail.toLowerCase().includes(search.toLowerCase()));
  const detail = studentDetail || selectedStudent;
  ...
  return (
    <PageContainer mode={isDark ? "dark" : "light"} pageBg={t.pageBg} textColor={t.text}>
      ...
      {/* ═══ MAIN GRID ═══ */}
      <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 16, alignItems: "start" }} className="sr-main-grid">

        {/* LEFT: Student list */}
        <SectionCard t={t}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 20px 12px", borderBottom: `1px solid ${t.border}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: RADIUS.chip, display: "flex", alignItems: "center", justifyContent: "center", background: `${ACCENT_PURPLE.base}18`, border: `1px solid ${ACCENT_PURPLE.base}30` }}>
                <Users size={15} color={ACCENT_PURPLE.base} />
              </div>
              <span style={{ fontWeight: 700, fontSize: 13, color: t.text, fontFamily: FONT_FAMILY }}>Students</span>
            </div>
            <span style={{ fontSize: 11, fontWeight: 700, background: `${ACCENT_PURPLE.base}18`, color: ACCENT_PURPLE.base, border: `1px solid ${ACCENT_PURPLE.base}30`, borderRadius: RADIUS.pill, padding: "2px 10px", fontFamily: FONT_FAMILY }}>
              {filtered.length}
            </span>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 16px", borderBottom: `1px solid ${t.border}` }}>
            <Search size={13} color={t.textMuted} />
            <input
              style={{ flex: 1, background: "none", border: "none", outline: "none", fontSize: 12, color: t.text, fontFamily: FONT_FAMILY }}
              placeholder="Search by email…"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          ...
          <div style={{ maxHeight: 580, overflowY: "auto" }} className="sr-scroll">
            {studentsLoading && (
              <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} style={{ height: 64, borderRadius: RADIUS.chip, background: t.barBg, animation: "srpulse 1.4s ease-in-out infinite" }} />
                ))}
              </div>
            )}
            {studentsError && (
              <div style={{ padding: 20, textAlign: "center", fontSize: 12, color: "#ef4444", fontFamily: FONT_FAMILY }}>{studentsError}</div>
            )}
            {!studentsLoading && filtered.length === 0 && (
              <EmptyBlock t={t} icon={Users} title="No students found" />
            )}
            <div style={{ padding: 10 }}>
              {filtered.map((s, i) => {
                const pal = AV_PALETTES[i % 5];
                ...
                const oc = progColor(s.overallProgressPercentage);
                return (
                  <StudentCard key={s.studentEmail} s={s} pal={pal} isActive={isActive} t={t} oc={oc} onClick={() => setSelectedStudent(s)} />
                );
              })}
            </div>
          </div>
        ...
        {/* RIGHT: Detail panel */}
        <SectionCard t={t} style={{ minHeight: 500 }}>
          {!selectedStudent && (
            <EmptyBlock t={t} icon={Users} title="No student selected" sub="Select a student to view their detailed progress" />
          )}
          {selectedStudent && detailLoading && (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: 400 }}>
              <Loader t={t} text="Loading report…" />
...
function StudentCard({ s, pal, isActive, t, oc, onClick }) {
  return (
    <div onClick={onClick} style={{
      display: "flex", alignItems: "center", gap: 10,
      padding: "11px 12px", borderRadius: RADIUS.chip, cursor: "pointer",
      marginBottom: 6,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[
            ...
            <div key={mi} style={{ flex: 1, height: 3, background: "rgba(128,128,128,0.15)", borderRadius: 2, overflow: "hidden" }}>
              <div style={{ height: 3, width: `${Math.min(m.val || 0, 100)}%`, background: m.color, borderRadius: 2, transition: "width .6s ease" }} />
            </div>
          ))}
        </div>
      </div>
      <span style={{ fontSize: 11, fontWeight: 700, border: `1px solid ${oc}44`, borderRadius: RADIUS.pill, padding: "3px 10px", color: oc, flexShrink: 0, fontFamily: FONT_FAMILY }}>
        {fmt(s.overallProgressPercentage)}%
      </span>
      {isActive && <ChevronRight size={14} color={ACCENT_PURPLE.base} style={{ flexShrink: 0 }} />}
    </div>
  );
}
  ...
  return (
    <div style={{ padding: "clamp(14px,3vw,24px)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22, paddingBottom: 18, borderBottom: `1px solid ${t.border}`, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h2 style={{ fontFamily: FONT_FAMILY, fontSize: "clamp(13px,2vw,16px)", fontWeight: 700, margin: "0 0 4px", color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {detail.studentEmail}
          </h2>
          <span style={{ fontSize: 11, background: t.pillBg, border: `1px solid ${t.pillBorder}`, padding: "2px 10px", borderRadius: RADIUS.pill, color: t.textMuted, fontFamily: FONT_FAMILY }}>
            Batch {detail.batchId || selectedBatchId}
          </span>
        </div>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, marginLeft: "auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8, marginBottom: 20 }} className="sr-arcs-grid">
        {metrics.map((m) => <MetricCard key={m.label} m={m} t={t} />)}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 16, alignItems: "start" }} className="sr-bottom-grid">
        <div style={{ background: t.actBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.standardCard, padding: 18 }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: t.textMuted, margin: "0 0 14px", fontFamily: FONT_FAMILY }}>
            Progress Breakdown
          </p>
          {metrics.map((m) => <ProgressRow key={m.label} label={m.label} val={m.val} color={m.color} t={t} />)}
        </div>
        <div style={{ background: t.actBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.standardCard, padding: 18, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: t.textMuted, margin: "0 0 10px", alignSelf: "flex-start", fontFamily: FONT_FAMILY }}>
            Skill Radar
          </p>
          <RadarChart data={detail} isDark={isDark} />
        </div>
      </div>
    </div>
  );
...
function MetricCard({ m, t }) {
  const Icon = m.icon;
  return (
    <div style={{ background: t.actBg, border: `1px solid ${t.border}`, borderRadius: RADIUS.chip, padding: "12px 8px 10px", textAlign: "center" }}>
      <ArcMeter value={m.val || 0} color={m.color} size={80} />
      <p style={{ fontSize: 10, fontWeight: 700, color: t.textMuted, margin: "4px 0 2px", textTransform: "uppercase", letterSpacing: "0.05em", fontFamily: FONT_FAMILY }}>{m.label}</p>
...
function ProgressRow({ label, val, color, t }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 11 }}>
      <span style={{ fontSize: 11, color: t.textSub, width: 105, flexShrink: 0, fontFamily: FONT_FAMILY }}>{label}</span>
      <div style={{ flex: 1, height: 5, background: t.barBg, borderRadius: 4, overflow: "hidden" }}>
        <div style={{ height: 5, width: `${Math.min(val || 0, 100)}%`, background: color, borderRadius: 4, transition: "width .8s cubic-bezier(0.4,0,0.2,1)" }} />
      </div>
      <span style={{ fontSize: 12, fontWeight: 700, color, width: 40, textAlign: "right", fontFamily: FONT_FAMILY }}>{fmt(val)}%</span>
    </div>
  );
}
...
function styles() {
  return `
    ...
    * { box-sizing: border-box; }
    .sr-scroll::-webkit-scrollbar { width: 3px; }
    .sr-scroll::-webkit-scrollbar-track { background: transparent; }
    .sr-scroll::-webkit-scrollbar-thumb { background: rgba(128,128,128,0.3); border-radius: 3px; }
    ...
    @media(max-width:420px){ .sr-arcs-grid { grid-template-columns: repeat(2,1fr) !important; } }
