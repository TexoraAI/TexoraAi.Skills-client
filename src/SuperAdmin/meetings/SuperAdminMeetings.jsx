...
// /* Swap this for your real useTheme() import from the Super Admin theme context. */
...
//     org: "—", // no organization name field wired up yet
...
//               placeholder="e.g. Platform-wide policy briefing"
//             />
//           </label>
...
//   const orgsLive = new Set(
//     sessions.filter((s) => s.status === "live").map((s) => s.org),
//   ).size;

//   //   const handleStartInstant = async () => {
//   //     try {
//   //       const res = await createInstantMeeting({
//   //         title: "Instant platform session",
...
//   return (
//     <div className={`ilmora-superadmin-meetings ${dark ? "dark" : ""}`}>
//       <style>{STYLES}</style>
...
//               </div>
//               <div className="subtitle">Super Admin workspace</div>
//             </div>
//           </div>
...
//             </div>
//             <h1>The whole platform, one place to run it.</h1>
//             <p>
//               Manage organizations, monitor every meeting in the system,
...
//               12 organizations · 340 trainers · All clear ✓
//             </div>
//           </div>
//         </section>
...
//             <h2>All sessions, every organization</h2>
...
// const STYLES = `
// .ilmora-superadmin-meetings {
//   --brand: #7c3aed; --brand-2: #a78bfa; --live: #14b8a6; --accent: #f59e0b; --accent-2: #ec4899;
//   --bg: #f8f7fc; --card: #ffffff; --text: #1a1523; --muted: #6b6478; --border: rgba(124,58,237,.12);
...
// }
// .ilmora-superadmin-meetings.dark { --bg: #0d0d14; --card: #17151f; --text: #f1eefc; --muted: #a49bc0; --border: rgba(255,255,255,.08); }

// .mesh-bg { position: absolute; inset: 0; overflow: hidden; z-index: 0; pointer-events: none; }
// .mesh-blob { position: absolute; border-radius: 9999px; filter: blur(90px); opacity: .34; }
// .dark .mesh-blob { opacity: .18; }
...
// .mesh-blob--3 { width: 460px; height: 460px; bottom: -200px; left: 30%; background: radial-gradient(circle, var(--live), transparent 70%); }

// .page-inner { position: relative; z-index: 1; max-width: 1180px; margin: 0 auto; padding: 28px 32px 80px; }
// .page-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 28px; }
// .brand-lockup { display: flex; align-items: center; gap: 12px; }
// .logo-mark { width: 38px; height: 38px; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #fff; background: linear-gradient(135deg, var(--brand), var(--brand-2)); box-shadow: 0 6px 16px rgba(124,58,237,.35); }
...
// .dark .live-pill { background: rgba(20,184,166,.18); color: #5eead4; }
...
// .icon-btn:hover { transform: translateY(-1px); background: rgba(124,58,237,.1); }
...
// .btn-primary { display: inline-flex; align-items: center; gap: 6px; background: linear-gradient(135deg, var(--brand), var(--brand-2)); color: #fff; border: none; padding: 8px 14px; border-radius: 9px; font-size: 13px; font-weight: 700; cursor: pointer; box-shadow: 0 6px 16px rgba(124,58,237,.3); }
...
// .split-btn-main { display: flex; align-items: center; gap: 6px; background: linear-gradient(135deg, var(--brand), var(--brand-2)); color: #fff; border: none; padding: 10px 16px; border-radius: 9px 0 0 9px; font-size: 13.5px; font-weight: 700; cursor: pointer; box-shadow: 0 6px 16px rgba(124,58,237,.3); }
...
// .split-menu { position: absolute; top: calc(100% + 6px); left: 0; min-width: 210px; background: var(--card); border: 1px solid var(--border); border-radius: 12px; box-shadow: 0 12px 30px rgba(23,15,40,.2); padding: 6px; z-index: 20; }
...
// .split-menu button:hover { background: rgba(124,58,237,.12); }
...
// .carousel-stage { position: relative; flex: 1; height: 360px; border-radius: 24px; overflow: hidden; background: linear-gradient(135deg, rgba(124,58,237,.08), rgba(236,72,153,.08)); border: 1px solid var(--border); }
...
// .carousel-arrow:hover { background: rgba(124,58,237,.12); }
// .carousel-dots { position: absolute; bottom: 16px; left: 50%; transform: translateX(-50%); display: flex; gap: 6px; z-index: 2; }
// .carousel-dot { width: 7px; height: 7px; border-radius: 999px; border: none; background: rgba(124,58,237,.28); cursor: pointer; }
// .carousel-dot.is-active { background: var(--brand); width: 20px; border-radius: 999px; transition: width .2s ease; }

// .illo { position: relative; width: 300px; height: 260px; flex-shrink: 0; border-radius: 20px; background: linear-gradient(160deg, rgba(124,58,237,.16), rgba(20,184,166,.10)); display: flex; align-items: center; justify-content: center; }
// .illo--verified { background: linear-gradient(160deg, rgba(20,184,166,.16), rgba(124,58,237,.08)); }
// .illo--attendance { background: linear-gradient(160deg, rgba(236,72,153,.16), rgba(124,58,237,.08)); }
// .illo-grid { display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 0.6fr; gap: 8px; width: 220px; height: 180px; }
// .illo-tile { border-radius: 12px; background: rgba(255,255,255,.75); border: 1px solid rgba(124,58,237,.18); padding: 10px; display: flex; flex-direction: column; gap: 6px; justify-content: center; }
// .dark .illo-tile { background: rgba(255,255,255,.06); }
...
// .illo-bar { display: block; height: 6px; border-radius: 999px; background: rgba(124,58,237,.32); }
...
// .illo-badge { position: absolute; bottom: -14px; right: 26px; width: 44px; height: 44px; border-radius: 14px; background: linear-gradient(135deg, var(--brand), var(--brand-2)); color: #fff; display: flex; align-items: center; justify-content: center; box-shadow: 0 8px 18px rgba(124,58,237,.38); }
// .illo--verified .illo-badge { background: linear-gradient(135deg, var(--live), #0ea5e9); box-shadow: 0 8px 18px rgba(20,184,166,.35); }
// .illo--attendance .illo-badge { background: linear-gradient(135deg, var(--accent-2), var(--brand-2)); box-shadow: 0 8px 18px rgba(236,72,153,.32); }

// .stat-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 32px; }
// .stat-card { position: relative; background: var(--card); border: 1px solid var(--border); border-radius: 20px; padding: 18px; box-shadow: 0 1px 2px rgba(23,15,40,.06); }
// .stat-icon { width: 34px; height: 34px; border-radius: 11px; display: flex; align-items: center; justify-content: center; margin-bottom: 14px; background: rgba(124,58,237,.14); color: var(--brand); }
...
// .tone-pink .stat-icon { background: rgba(236,72,153,.16); color: #be185d; }
...
// .tone-pink .stat-underline { background: linear-gradient(90deg, var(--accent-2), var(--brand-2)); }
...
// .session-row:hover { background: rgba(124,58,237,.06); }
// .session-dot { width: 8px; height: 8px; border-radius: 999px; background: #d8d2e8; flex-shrink: 0; }
...
// .btn-join:disabled { background: rgba(107,100,120,.25); color: var(--muted); cursor: not-allowed; }
// .btn-join.is-completed { background: #2563eb; color: #fff; opacity: .55; cursor: not-allowed; pointer-events: none; }

// .modal-overlay { position: fixed; inset: 0; background: rgba(13,13,20,.5); display: flex; align-items: center; justify-content: center; z-index: 50; }
// .modal-card { width: 420px; background: var(--card); border-radius: 18px; padding: 18px; box-shadow: 0 20px 50px rgba(0,0,0,.3); }
...
.eyebrow { display: flex; align-items: center; gap: 8px; font-size: 11.5px; font-weight: 700; letter-spacing: .08em; text-transform: uppercase; color: var(--brand); margin-bottom: 10px; }
.eyebrow-dot { width: 6px; height: 6px; border-radius: 999px; background: var(--brand); }
.hero h1 { font-size: clamp(22px, 2.6vw, 32px); font-weight: 800; letter-spacing: -0.02em; margin: 0 0 8px; line-height: 1.15; white-space: nowrap; }
...
.carousel-stage { position: relative; flex: 1; min-width: 0; height: clamp(260px, 32vw, 360px); border-radius: 24px; overflow: hidden; background: var(--card); border: 1px solid var(--border); }
.carousel-slide { position: absolute; inset: 0; display: flex; align-items: center; gap: 32px; padding: 32px 44px; opacity: 0; transition: opacity .6s ease; pointer-events: none; }
.carousel-slide.is-active { opacity: 1; pointer-events: auto; }
.carousel-copy { max-width: 340px; }
.carousel-copy h3 { font-size: 22px; font-weight: 800; margin: 0 0 10px; letter-spacing: -0.01em; }
...
.illo { position: relative; width: 300px; height: 260px; flex-shrink: 0; border-radius: 20px; background: var(--bg); display: flex; align-items: center; justify-content: center; border: 1px solid var(--border); }
.illo-grid { display: grid; grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 0.6fr; gap: 8px; width: 220px; height: 180px; }
.illo-tile { border-radius: 12px; background: rgba(255,255,255,.75); border: 1px solid rgba(124,58,237,.18); padding: 10px; display: flex; flex-direction: column; gap: 6px; justify-content: center; }
.dark .illo-tile { background: rgba(255,255,255,.06); }
.illo-tile--main { grid-column: 1 / span 2; align-items: flex-start; }
...
@media (max-width: 600px) {
  ...
  .corner-logo { height: 26px; max-width: 76px; }
