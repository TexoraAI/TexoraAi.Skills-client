...
import { getSessionHistory } from "@/services/liveSessionService"; // ✅ FIXED: use service
...
const LiveSessionHistory = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        // ✅ FIXED: use getSessionHistory() from service (JWT auth, hits /api/live-sessions/history)
        // was: raw fetch("/api/trainer/live-sessions/history") — wrong endpoint, no JWT
  ...
  const filtered = sessions.filter((s) => {
    ...
    // ✅ FIXED: backend returns "ENDED" not "completed"
    const matchStatus =
      statusFilter === "all" ||
      (statusFilter === "completed" && s.status === "ENDED") ||
        ...
        >
          ...
          >
            ...
            >
              ...
              <option value="completed">Completed</option> {/* maps to ENDED */}
...
function HistoryRow({ session: s, t, navigate }) {
  ...
  // ✅ FIXED: backend uses "ENDED" not "completed"
  const isCompleted = s.status === "ENDED" || s.status === "completed";
  ...
  // ✅ FIXED: map backend field names (scheduledDate/scheduledTime) to display
  const displayDate = s.scheduledDate ?? s.date ?? "—";
  const displayTime = s.scheduledTime ?? s.time ?? "—";
