...
export const blankForm = () => ({
  ...
  // ── Details: Course Statistics ──
  assignmentsCount: "",
  quizzesCount: "",
  reviewsCount: "",

  // ── About: Instructor Bio / Career Information ──
  instructorBio: "",
  jobRoles: [],
  salaryRange: "",
  hiringCompanies: [],
...
function buildRequestDTO(fd, categories = []) {
  ...
  return {
    ...
    // ✅ NEW — Basic: Course Media
    bannerUrl: fd.bannerUrl || "",

    // ✅ NEW — Basic: Instructor
    instructorPhotoUrl: fd.instructorPhotoUrl || "",
    instructorLinkedIn: fd.instructorLinkedIn || "",

    // ✅ NEW — Details: Pricing
    originalPrice: Number(fd.originalPrice) || 0,
    discountPercent: Number(fd.discountPercent) || 0,
    currency: fd.currency || "INR",
    emiAvailable: !!fd.emiAvailable,
    freeTrial: !!fd.freeTrial,

    // ✅ NEW — Details: Course Statistics
    assignmentsCount: Number(fd.assignmentsCount) || 0,
    quizzesCount: Number(fd.quizzesCount) || 0,
    reviewsCount: Number(fd.reviewsCount) || 0,

    // ✅ NEW — About: Instructor Bio / Career Information
    instructorBio: fd.instructorBio || "",
    jobRoles: Array.isArray(fd.jobRoles) ? fd.jobRoles : [],
    salaryRange: fd.salaryRange || "",
    hiringCompanies: Array.isArray(fd.hiringCompanies)
      ? fd.hiringCompanies
      : [],
    placementSupport: !!fd.placementSupport,
    careerAssistance: fd.careerAssistance || "",

    // ✅ NEW — Outcomes: Skills
    skills: Array.isArray(fd.skills) ? fd.skills : [],

    // ✅ NEW — Outcomes: Projects (repeater). Named "projectsList" to avoid
    // colliding with the existing numeric "projects" (Projects Count) field above.
    projectsList: (Array.isArray(fd.projects) ? fd.projects : []).map((p) => ({
      title: p?.title || "",
    ...
    // ✅ NEW — Highlights: Certificate
    certificateTitle: fd.certificateTitle || "",
    certificateImageUrl: fd.certificateImageUrl || "",
    certificateVerificationUrl: fd.certificateVerificationUrl || "",

    // ✅ NEW — FAQs: SEO
    metaTitle: fd.metaTitle || "",
    metaDescription: fd.metaDescription || "",
    metaKeywords: Array.isArray(fd.metaKeywords) ? fd.metaKeywords : [],
    ogImageUrl: fd.ogImageUrl || "",

    // ✅ NEW — Syllabus: Display Settings
    showOnHomepage: !!fd.showOnHomepage,
    isFeatured: !!fd.isFeatured,
    isTrending: !!fd.isTrending,
    isBestseller: !!fd.isBestseller,
    isPopular: !!fd.isPopular,
    isRecommended: !!fd.isRecommended,
    isComingSoon: !!fd.isComingSoon,
    // ✅ NEW — Draft / Publish workflow
    publishStatus: fd.publishStatus || "Draft",
    // AFTER
...
function loadFromDTO(dto) {
  return {
    ...
    // ✅ NEW — Basic: Instructor
    instructorPhotoUrl: dto.instructorPhotoUrl || "",
    instructorLinkedIn: dto.instructorLinkedIn || "",
    ...
    freeTrial: !!dto.freeTrial,

    // ✅ NEW — Details: Course Statistics
    assignmentsCount: dto.assignmentsCount ?? "",
    quizzesCount: dto.quizzesCount ?? "",
    reviewsCount: dto.reviewsCount ?? "",

    // ✅ NEW — About: Instructor Bio / Career Information
    instructorBio: dto.instructorBio || "",
    jobRoles: Array.isArray(dto.jobRoles) ? dto.jobRoles : [],
    salaryRange: dto.salaryRange || "",
    hiringCompanies: Array.isArray(dto.hiringCompanies)
      ? dto.hiringCompanies
      : [],
    placementSupport: !!dto.placementSupport,
    careerAssistance: dto.careerAssistance || "",

    // ✅ NEW — Outcomes: Skills / Projects
    skills: Array.isArray(dto.skills) ? dto.skills : [],
    projects: (Array.isArray(dto.projectsList) ? dto.projectsList : []).map(
      (p, i) => ({
        id: p.id ?? i,
        title: p.title || "",
        description: p.description || "",
    ...
    certificateVerificationUrl: dto.certificateVerificationUrl || "",

    // ✅ NEW — FAQs: SEO
    metaTitle: dto.metaTitle || "",
    metaDescription: dto.metaDescription || "",
    metaKeywords: Array.isArray(dto.metaKeywords) ? dto.metaKeywords : [],
...
export default function AddEditProgram({
  ...
  const handleSave = async (publishAfter = false) => {
    ...
    setSaving(true);
    try {
      // "Save as Draft" always forces Draft; "Publish" keeps whatever
      // publishStatus is currently on formData (e.g. already Published on
      // an edit), then promotes it via the dedicated endpoint below.
      const dataToSave = publishAfter
        ? formData
          ...
          {!isView && (
            <>
              ...
              <button
                ...
                disabled={saving}
                className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/90 hover:bg-emerald-500 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-60"
              >
                {saving ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
            ...
            <div className="p-4">
              ...
              {activeTab === "about" && (
                <ProgramAbout
                  data={{
                    ...
                    hiringCompanies: formData.hiringCompanies,
                    placementSupport: formData.placementSupport,
                    careerAssistance: formData.careerAssistance,
                  }}
                  onChange={mergeData}
                />
              )}
              ...
              {activeTab === "highlights" && (
                <ProgramHighlights
                  data={{
                    ...
                    certificateVerificationUrl:
                      formData.certificateVerificationUrl,
                  }}
                  onChange={mergeData}
                />
              )}
              ...
              {activeTab === "syllabus" && (
                <SyllabusManager
                  ...
                  onDisplaySettingsChange={mergeData}
                />
              )}
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 bg-gray-50 px-4 py-3 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => handleSave(false)}
                disabled={saving}
                className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 rounded-lg transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {saving ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
              ...
              <button
                onClick={() => handleSave(true)}
                disabled={saving}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-all disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {saving ? (
                  <Loader2 size={14} className="animate-spin" />
                ) : (
