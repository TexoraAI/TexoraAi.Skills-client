...
export default function TexoraAISkills() {
  ...
  const [settingsMenuPos, setSettingsMenuPos] = useState({ bottom: 56, right: 8, width: 280 });
  ...
  // ── Share URL ──────────────────────────────────────────────────
  // FIX: previously built from window.location.pathname (which already
  // contains /watch-demo/{id}) + appending videoId again, producing
  // /watch-demo/1/1. Now always builds a clean /watch-demo/{id} URL.
  const shareUrl = videoId
    ? `${window.location.origin}/watch-demo/${videoId}`
  ...
  // ── iOS Safari native video fullscreen listeners ───────────────
  // iPhone Safari fires webkitbeginfullscreen/webkitendfullscreen on the
  // <video> element itself (not document fullscreenchange), so without
  // this the UI state never resets correctly when exiting fullscreen.
  useEffect(() => {
  ...
  const toggleFullscreen = async () => {
    ...
    try {
      if (!isFullscreen) {
        ...
        // FIX: iPhone Safari does not support the Fullscreen API on
        // arbitrary container elements — only on the <video> element
        // itself via the non-standard webkitEnterFullscreen() method.
          ...
          <div
            ...
            className={`relative bg-black overflow-hidden shadow-2xl ${
              isFullscreen
                ...
                : "rounded-2xl border-4 border-[#F97316]/20"
            }`}
          >
            <div className={`relative group ${isFullscreen ? "w-full h-full" : "aspect-video"}`}>
              <video
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/95 to-transparent p-2 sm:p-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                ...
                <div className="flex items-center gap-1.5 sm:gap-3 flex-wrap">
                  ...
                  <div className="relative">
                    <button
                      ...
                      onClick={() => {
                        if (!showSettingsMenu && settingsBtnRef.current) {
                          ...
                          const menuWidth = window.innerWidth < 640 ? 220 : 280;
                          setSettingsMenuPos({
                    ...
                    {showSettingsMenu && (
                      <div
                        className="fixed bg-gray-900/95 backdrop-blur-sm rounded-lg shadow-2xl border border-gray-700 py-2 overflow-y-auto z-[60]"
                        style={{
                          bottom: settingsMenuPos.bottom,
                          right: settingsMenuPos.right,
                          width: settingsMenuPos.width,
                          maxWidth: "90vw",
                          maxHeight: "60vh",
                        }}
                      >
