...
import { getQuizCreateUsage } from "../services/assessmentService";
import {
  ChevronDown,
  Loader2,
  PlusCircle,
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 13,
                      fontFamily: FONT_FAMILY,
                      color: sel ? ACCENT_PURPLE.base : t.text,
...
const CreateQuiz = () => {
  ...
  const addNewQuestion = () =>
    setQuestions([...questions, getDefaultQuestion(quizType)]);
  ...
  const handleFileChosen = async (file) => {
    ...
    try {
      ...
      setUploadMsg(
        `✅ ${data.questions?.length || 0} question${(data.questions?.length || 0) !== 1 ? "s" : ""} loaded from "${file.name}"`,
      );
      setOpen("questions");
    } catch (err) {
  ...
  const handleSaveQuiz = async () => {
    for (const q of questions) {
      ...
      if (quizType === "Fill in the Blank" || quizType === "Short Answer") {
        if (!q.answer?.trim()) {
          alert("Please enter the answer for all questions");
          return;
        }
      } else if (quizType === "True / False") {
        if (!q.correctOption) {
          alert("Please select True or False for all questions");
          return;
        }
      } else {
        if (!q.correctOption) {
  ...
  const panelMeta = {
    details: {
      label: "Quiz Details",
      sub: "Title, type, difficulty & settings",
      color: "#22d3ee",
      num: 1,
      nextLabel: "Next: Add Questions →",
    },
    questions: {
      label: "Questions",
      ...
      {usage && (
        ...
        >
          {usage.limit === "unlimited"
            ...
            : `${usage.used}/${usage.limit} quizzes created this month`}
        </div>
      )}
                  ...
                  {key === "questions" && (
                    ...
                    >
                      <div
                        ...
                        onDrop={onDrop}
                        style={{
                          border: `2px dashed ${dragOver ? "#22d3ee" : uploadStatus === "success" ? "#34d399" : uploadStatus === "error" ? "#f87171" : `${ACCENT_PURPLE.base}55`}`,
                          borderRadius: RADIUS.chip,
                          padding: "20px 16px",
                          background: dragOver
                      ...
                      >
                        ...
                        {uploading ? (
                          ...
                          >
                            ...
                            >
                              <Upload size={18} color={ACCENT_PURPLE.base} />
                            </div>
                            <div>
                              <p
                                style={{
                            ...
                            <button
                              ...
                              style={{
                                ...
                                padding: "8px 16px",
                                borderRadius: RADIUS.chip,
                                border: `1px solid ${ACCENT_PURPLE.base}55`,
                                background: `${ACCENT_PURPLE.base}12`,
                                color: ACCENT_PURPLE.base,
                                fontSize: 12,
                                fontWeight: 600,
                                cursor: "pointer",
                              ...
                              {["A", "B", "C", "D"].map((optKey) => (
                                <label
                                  ...
                                  style={{
                                    ...
                                    padding: "10px 12px",
                                    cursor: "pointer",
                                    border: `1px solid ${q.correctOption === optKey ? "#34d399" : t.border}`,
                                    background:
                      ...
                      >
                        <button
                          ...
                          style={{
                            ...
                            padding: "9px 16px",
                            borderRadius: RADIUS.chip,
                            border: `2px dashed ${ACCENT_PURPLE.base}55`,
                            background: "transparent",
                            color: ACCENT_PURPLE.base,
                            fontSize: 12,
                            fontWeight: 600,
