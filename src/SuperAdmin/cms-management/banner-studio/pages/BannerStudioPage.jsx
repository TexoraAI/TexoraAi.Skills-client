...
// Last-line-of-defense guard. Even though bannerApi already normalizes the
// response, this ensures every child NEVER receives a non-array,
// regardless of what the API layer does now or in the future.
...
export default function BannerStudioPage() {
  ...
  const loadBanners = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const data = await bannerApi.getAll(filter, searchTerm);
      // Defensive: guarantee an array reaches state no matter what
      // bannerApi returned (network hiccup, backend change, etc.)
  ...
  const handleSave = async (formData) => {
    try {
      if (editingBanner) {
        ...
        showToast(formData.status === 'active' ? 'Banner published live 🎉' : 'Banner saved as draft');
      }
      setPanelOpen(false);
      loadBanners();
    } catch (err) {
  ...
  const handleUseAiBanner = async (draft) => {
    try {
      await bannerApi.create(draft);
  ...
  const handleSchedule = async (id, date, time) => {
    try {
      await bannerApi.update(id, { status: 'scheduled', startDate: date, startTime: time });
    ...
    <div className="banner-studio-page">
      ...
      <div className="bs-split-shell">
        <div className="bs-workspace">
          ...
          {!loading && (
            <PublishCard banner={latestDraft} onPublish={handlePublishNow} onSchedule={handleSchedule} />
          )}
          {!loading && <CalendarSection banners={safeBanners} onEdit={handleEdit} />}
        </div>
