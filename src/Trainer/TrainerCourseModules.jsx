...
//   Upload, List, Pencil, X, Check, AlertCircle, Loader2,
// } from "lucide-react";
...
// /* ─────────────────── EDIT MODAL ─────────────────── */
// const EditModal = ({ module, onClose, onSaved, type }) => {
...
//     if (!f) return;
//     setEditFile(f);
//     // auto-fill title only if user hasn't manually changed it from original
//     if (editTitle === module.title || editTitle === "") {
//       setEditTitle(extractTitle(f.name));
//     }
//   };

//   const handleSave = async () => {
//     if (!editTitle.trim()) { setError("Lesson title is required"); return; }
//     try {
//       setLoading(true);
//       setError("");
//       let fileUrl = module.url;

//       if (editFile) {
//         const formData = new FormData();
//         formData.append("file", editFile);
//         formData.append("courseId", module.courseId || 0);
//         formData.append("moduleId", 0);
//         formData.append("batchId", 0);

//         const endpoint = module.contentType === "VIDEO"
//           ? `${API}/course-videos/upload`
//           : `${API}/course-files/upload`;
...
//       await axios.put(`${API}/content/${module.id}`, {
...
//       setTimeout(() => {
//         onSaved({ ...module, title: editTitle.trim(), url: fileUrl });
//         onClose();
//       }, 800);
//     } catch (err) {
//       console.error(err);
...
//             <span className="text-sm font-bold text-white tracking-wide">Edit Module</span>
//           </div>
//           <button
//             onClick={onClose}
...
//           <div className="space-y-1.5">
//             <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
//               Replace File <span className="text-slate-300 dark:text-slate-600">(optional)</span>
//             </label>
//             <label className="flex flex-col items-center justify-center gap-2 px-4 py-4 rounded-xl
...
//                 {editFile
//                   ? <span className="text-blue-600 dark:text-blue-400 font-medium">{editFile.name}</span>
...
//                 type="file"
//                 className="hidden"
//                 accept={module.contentType === "VIDEO" ? "video/*" : ".pdf,.doc,.docx,.ppt,.pptx"}
...
//     if (!title.trim()) {
//       setTitle(extractTitle(f.name));
//     }
//   };

//   /* ── Upload / Add module ── */
//   const uploadAsset = async () => {
//     if (!title.trim()) { setAddError("Lesson title is required"); return; }
//     if (!file)         { setAddError(`Please select a ${type === "VIDEO" ? "video" : "PDF"} file`); return; }
...
//         headers: { ...authHeader(), "Content-Type": "multipart/form-data" },
//       });
//       const fileName = uploadRes.data?.url;
...
//     try {
//       await axios.delete(`${API}/content/${id}`, { headers: authHeader() });
...
//                   <button
//                     key={t}
//                     onClick={() => { setType(t); setFile(null); setTitle(""); setAddError(""); }}
