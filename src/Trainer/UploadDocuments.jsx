...
// import { CheckCircle, ChevronDown, Download, Eye, Folder, Loader2, Plus, Sparkles, Trash2, UploadCloud } from "lucide-react";
...
//    – fully custom div-based, no native <select>
//    – works in light + dark theme
//    – "+ Add" input lives INSIDE the dropdown at the bottom
// ═══════════════════════════════════════════════════════════ */
...
//                     background: sel ? selectedBg : "transparent",
//                     cursor: "pointer",
//                     display: "flex", alignItems: "center", justifyContent: "space-between",
...
//             {/* input stacked above full-width Add button */}
//             <input
//               ref={inputRef}
//               value={newCat}
...
//                 padding: "0 10px",
//                 borderRadius: 8,
//                 border: `1px solid ${error ? "#f87171" : addBorder}`,
...
//     try {
//       const res = await videoService.getTrainerCourses?.();
//       setCourses(res?.data || []);
//     } catch (err) { console.error("Failed to load courses", err); }
//   };

//   // ── ZIP ke saath updated allowed MIME types ──
//   const allowedTypes = [
//     "application/pdf",
//     "application/msword",
...
//   // Some browsers/OS report .zip as octet-stream — catch by extension too
//   const isZipByExtension = (fileName) =>
//     fileName?.toLowerCase().endsWith(".zip");

//   const validateAndSetFile = (selectedFile) => {
//     if (!selectedFile) return;
//     const isAllowedMime = allowedTypes.includes(selectedFile.type);
//     const isZip = isZipByExtension(selectedFile.name);
//     if (!isAllowedMime && !isZip) {
//       setMessage("Only PDF, Word, PPT, or ZIP files are allowed");
//       setMessageType("error"); setFile(null); return;
//     }
...
//     if (!file || !batchId) {
//       setMessage("Select batch and file"); setMessageType("error"); return;
//     }
//     try {
//       setLoading(true);
//       await fileService.uploadFile(file, batchId, title, description, courseId, category);
...
//       setMessage("File deleted"); setMessageType("success");
//     } catch {
//       setMessage("Delete failed"); setMessageType("error");
//     }
//   };
...
//                 Drag & drop document here — PDF, Word, PPT, ZIP (max 100MB)
//               </p>
//               <label style={{
...
//                     {fileItem.batchId && (
//                       <span style={{
//                         fontSize: 10, color: "#2563eb", background: "rgba(37,99,235,0.10)",
//                         padding: "2px 8px", borderRadius: 6,
//                         fontFamily: "'Poppins',sans-serif", fontWeight: 600,
//                       }}>Batch: {fileItem.batchId}</span>
//                     )}
//                     {fileItem.category && (
//                       <span style={{
//                         fontSize: 10, color: "#7c3aed", background: "rgba(124,58,237,0.10)",
//                         padding: "2px 8px", borderRadius: 6,
//                         fontFamily: "'Poppins',sans-serif", fontWeight: 600,
//                       }}>{fileItem.category}</span>
//                     )}
//                   </div>
//                 </div>
//                 <div style={{ display: "flex", gap: 8 }}>
//                   {[
//                     { icon: Eye,      color: "#2563eb", onClick: () => handlePreview(fileItem) },
//                     { icon: Download, color: "#34d399", onClick: () => {
//                         const a = document.createElement("a");
//                         a.href = fileItem.fileUrl;
//                         a.download = fileItem.fileName || "document";
//                         a.click();
//                       }
//                     },
//                     { icon: Trash2, color: "#f87171", onClick: () => handleDelete(fileItem.id) },
//                   ].map(({ icon: Icon, color, onClick }, i) => (
