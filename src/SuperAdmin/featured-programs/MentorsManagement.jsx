...
// import {
//   Users, CheckCircle, XCircle, Star, Plus, Search, Filter,
//   AlertTriangle, X,
// } from "lucide-react";
...
// const SEED_MENTORS = [
//   {
//     id: 1, candidateName: "Priya Sharma", designation: "Product Manager",
//     company: "Flipkart", rating: 5,
//     feedbackMessage: "ILM ORA transformed my career. The mentors are industry veterans who give real-world insights you won't find in textbooks. Highly recommend!",
//     profileImage: "", status: "active", isFeatured: true, createdAt: "2026-01-15T10:30:00Z",
//   },
//   {
//     id: 2, candidateName: "Rahul Verma", designation: "UX Designer",
//     company: "Zomato", rating: 5,
//     feedbackMessage: "The structured curriculum and live classes helped me land my dream job at Zomato. The platform is exceptionally well-designed for working professionals.",
//     profileImage: "", status: "active", isFeatured: true, createdAt: "2026-02-10T09:00:00Z",
//   },
//   {
//     id: 3, candidateName: "Ananya Singh", designation: "Growth Lead",
//     company: "CRED", rating: 4,
//     feedbackMessage: "Best learning investment I made. The instructors are top-notch and the peer community is incredibly supportive. Got promoted within 3 months!",
//     profileImage: "", status: "active", isFeatured: false, createdAt: "2026-03-05T14:00:00Z",
//   },
//   {
//     id: 4, candidateName: "Arjun Mehta", designation: "Data Scientist",
//     company: "Paytm", rating: 5,
//     feedbackMessage: "Exceptional quality content. The hands-on projects were exactly what I needed to build a strong portfolio and impress interviewers.",
//     profileImage: "", status: "inactive", isFeatured: false, createdAt: "2026-04-01T11:00:00Z",
//   },
// ];
...
//     >
//       <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 backdrop-blur-sm ${a.iconBg}`}>
//         <Icon size={20} className={a.iconColor} />
//       </div>
//       <div>
//         <div className={`text-2xl font-bold leading-none mb-0.5 ${a.val}`}>{value}</div>
...
//       }`}>
//         <div className={`px-6 pt-6 pb-4 flex flex-col items-center text-center ${dark ? "bg-red-500/5" : "bg-red-50"}`}>
//           <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${
//             dark ? "bg-red-500/10 text-red-400" : "bg-red-100 text-red-600"
//           }`}>
//             <AlertTriangle size={22} />
//           </div>
//           <h3 className={`font-bold text-base ${dark ? "text-white" : "text-gray-900"}`}>Delete Feedback?</h3>
...
//             <span className={`font-semibold ${dark ? "text-white" : "text-gray-900"}`}>{mentor.candidateName}</span>?
...
//   return (
//     <div className={`fixed bottom-6 right-6 z-[100] flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-2xl text-white text-sm font-medium ${
...
//   const nextId = React.useRef(SEED_MENTORS.length + 1);

//   // Theme
//   const pageBg    = dark ? "bg-[#0a0a14]"                   : "bg-gray-50";
//   const headerBg  = dark ? "bg-[#0f0f1a] border-white/8"    : "bg-white border-gray-100";
//   const cardBg    = dark ? "bg-white/[0.03] border-white/8" : "bg-white border-gray-200";
...
//     const avg      = total > 0
//       ? (mentors.reduce((s, m) => s + m.rating, 0) / total).toFixed(1)
//       : "0.0";
...
//         m.feedbackMessage.toLowerCase().includes(q);
//       const matchStatus = statusFilter === "all" || m.status === statusFilter;
//       const matchRating = ratingFilter === "all" || m.rating === Number(ratingFilter);
//       return matchSearch && matchStatus && matchRating;
//     });
//   }, [mentors, search, statusFilter, ratingFilter]);
...
//   const openAdd  = ()       => { setSelectedMentor(null);   setDrawerMode("add");  setDrawerOpen(true); };
//   const openEdit = (mentor) => { setSelectedMentor(mentor); setDrawerMode("edit"); setDrawerOpen(true); };
//   const openView = (mentor) => { setSelectedMentor(mentor); setDrawerMode("view"); setDrawerOpen(true); };
//   const closeDrawer = () => {
//     setDrawerOpen(false);
...
//     if (drawerMode === "add") {
//       setMentors((prev) => [
//         { ...formData, id: nextId.current++, createdAt: new Date().toISOString() },
//         ...prev,
//       ]);
//       showToast("Mentor feedback added successfully!");
//     } else if (drawerMode === "edit") {
//       setMentors((prev) =>
//         prev.map((m) => m.id === selectedMentor.id ? { ...m, ...formData } : m)
//       );
...
//     );
//     showToast(`Feedback marked as ${newStatus === "active" ? "Active" : "Inactive"}.`);
//   };
...
//   return (
//     // ── KEY CHANGE: flex row layout so drawer sits beside content ──
//     <div className={`min-h-screen flex ${pageBg}`}>

//       {/* ── LEFT: Main Content — shrinks when drawer opens ── */}
//       <div
//         className="flex-1 flex flex-col min-w-0 transition-all duration-300"
//         style={{ marginRight: drawerOpen ? 0 : 0 }}
//       >
//         {/* Page Header */}
//         <div className={`border-b px-6 py-5 md:px-8 flex-shrink-0 ${headerBg}`}>
//           <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
//             <div>
//               <h1 className={`text-2xl font-bold ${titleText}`}>Mentors Management</h1>
...
//             <StatCard dark={dark} accent="blue"    icon={Users}        value={stats.total}       label="Total Feedbacks"    />
//             <StatCard dark={dark} accent="emerald" icon={CheckCircle}  value={stats.active}      label="Active Feedbacks"   />
//             <StatCard dark={dark} accent="red"     icon={XCircle}      value={stats.inactive}    label="Inactive Feedbacks" />
//             <StatCard dark={dark} accent="amber"   icon={Star}         value={`${stats.avg} ★`}  label="Average Rating"     />
//           </div>
...
//                 <input
//                   type="text"
//                   placeholder="Search by name, company, feedback..."
//                   value={search}
...
//               <p className="text-white font-semibold">Landing Page Connection</p>
//               <p className="text-purple-200 text-sm mt-0.5">
//                 Only <strong className="text-white">Active</strong> feedbacks are shown in the{" "}
//                 <em>"Learn From Industry Experts"</em> section. Currently{" "}
//                 <strong className="text-white">{stats.active} feedback(s)</strong> are live.
//               </p>
//             </div>
...
//       {/* ── RIGHT: Drawer — sits beside content, NO overlay, NO blur ── */}
//       <MentorDrawer
//         isOpen={drawerOpen}
//         onClose={closeDrawer}
...
import {
  Users, CheckCircle, XCircle, Star, Plus, Search, Filter,
  AlertTriangle, X, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
} from "lucide-react";
import MentorTable from "./components/MentorTable";
import MentorDrawer from "./components/MentorDrawer";
...
// ─── Pagination ───────────────────────────────────────────────────────────────
const Pagination = ({ currentPage, totalPages, pageSize, totalItems, onPageChange, onPageSizeChange, dark }) => {
    ...
    <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 px-4 py-3 border-t ${
      ...
      <div className="flex items-center gap-3">
        <span className={`text-xs ${dark ? "text-slate-500" : "text-gray-500"}`}>
          Showing <span className={`font-semibold ${dark ? "text-slate-300" : "text-gray-700"}`}>{startItem}–{endItem}</span> of{" "}
          <span className={`font-semibold ${dark ? "text-slate-300" : "text-gray-700"}`}>{totalItems}</span> results
        </span>
        <div className="flex items-center gap-1.5">
          <span className={`text-xs ${dark ? "text-slate-500" : "text-gray-500"}`}>Per page:</span>
          <select
            value={pageSize}
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-1">
          ...
          {getPageNumbers().map((page, idx) =>
            page === "..." ? (
              <span key={`dots-${idx}`} className={`h-8 px-1 flex items-center text-xs ${dark ? "text-slate-600" : "text-gray-400"}`}>
                …
              </span>
            ) : (
              <button
...
const MentorsManagement = () => {
  ...
  const stats = useMemo(() => {
    ...
    const active   = mentors.filter((m) => m.status === "active").length;
    const inactive = mentors.filter((m) => m.status === "inactive").length;
    const featured = mentors.filter((m) => m.isFeatured).length;
    const avg      = total > 0
      ? (mentors.reduce((s, m) => s + m.rating, 0) / total).toFixed(1)
      : "0.0";
    return { total, active, inactive, featured, avg };
  }, [mentors]);

  // Filtered list (full, unpaginated)
  const filtered = useMemo(() => {
    return mentors.filter((m) => {
      const q = search.toLowerCase();
      const matchSearch =
      ...
      <div className="flex-1 flex flex-col min-w-0 transition-all duration-300">
        ...
        <div className="px-5 py-5 md:px-8 space-y-5 overflow-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            <StatCard dark={dark} accent="blue"    icon={Users}       value={stats.total}       label="Total Feedbacks"    />
            <StatCard dark={dark} accent="emerald" icon={CheckCircle} value={stats.active}      label="Active"             />
            <StatCard dark={dark} accent="red"     icon={XCircle}     value={stats.inactive}    label="Inactive"           />
            <StatCard dark={dark} accent="violet"  icon={Star}        value={stats.featured}    label="Featured"           />
            <StatCard dark={dark} accent="amber"   icon={Star}        value={`${stats.avg}★`}   label="Avg Rating"         />
          </div>
