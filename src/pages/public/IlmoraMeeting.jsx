...
/* ─── FEATURE: Live captions ────────────────────────────────────────
   Captions transcribe the LOCAL participant's own microphone using the
   browser's built-in SpeechRecognition engine — there's no separate
   speech-to-text backend wired up, so this captures what "you" say,
   labelled as such, the same honest scope Meet/Zoom use before you pay
   for a transcription add-on. Chrome, Edge, and Android Chrome support
   it; Firefox and iOS Safari currently do not, so we feature-detect and
   show a clear message instead of a dead toggle.
   ──────────────────────────────────────────────────────────────── */
function detectCaptionSupport() {
  ...
  if (!SR) {
    return {
      supported: false,
      message:
        "Live captions aren't supported in this browser yet. Try Chrome or Edge (desktop or Android).",
    };
  }
...
function MeetingRoom({
  ...
  // the call — and drives whether the Present button is enabled at all.
  const [screenShareSupport] = useState(() => detectScreenShareSupport());
  // FIX (Bug — Screen Share on Phone): tracks the brief window between a
  // tap on Present and getDisplayMedia's picker actually resolving, so the
  // button can show a spinner and can't be double-tapped while the OS
  // permission/picker UI is still opening — the most common reason a tap
  // "did nothing" on phone was a second tap firing before the first
  // request had settled.
  ...
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [captionsOn, setCaptionsOn] = useState(false);
  // FIX (Feature — Live captions): the captions bar used to be a static
  // "Live captions are enabled" label with no real transcription behind
  // it. captionLine holds the actual live/interim text; captionSupport
  // is detected once on mount so the toggle can explain itself instead
  // of silently doing nothing on unsupported browsers.
  const [captionLine, setCaptionLine] = useState("");
  const [captionSupport] = useState(() => detectCaptionSupport());
  ...
  //     never get stuck in a stale "on" state on any platform.
  const toggleScreen = useCallback(async () => {
    // FIX (Bug — Screen Share on Phone): a tap that arrives before the
    // Room has finished connecting used to just `return` with zero
    // feedback, which on a phone looks identical to "the button is
    // broken." Every early-exit path below now sets mediaError so a tap
    // always produces a visible result.
    if (screenBusy) return;
    const room = roomRef.current;
    if (!room) {
      setMediaError(
        "Still connecting to the meeting — try Present again in a moment.",
      );
      return;
    }
    ...
    // FIX (Bug — Screen Share on Phone): re-detect support at click time
    // instead of trusting only the one-time mount check. Some Android
    // browsers/WebViews add getDisplayMedia after an app update or only
    // expose it once a page has been interacted with, so a live check
    // catches devices that have become capable since the page loaded.
    const liveSupport = detectScreenShareSupport();
    if (!liveSupport.supported) {
      setMediaError(
        liveSupport.message ||
          screenShareSupport.message ||
          "Screen sharing isn't available on this device or browser. On phones, only recent Android Chrome/Edge/Firefox support it — iPhone/iPad Safari cannot screen share from the browser at all.",
      );
      return;
    }
  ...
  }, [screenOn, screenBusy, screenShareSupport, rebuild]);

  // FIX (Feature — Live captions): drives an actual SpeechRecognition
  // session off the local mic whenever captionsOn is true. Runs
  // `continuous` + `interim` so text streams in live like real captions,
  // and auto-restarts on the engine's natural "onend" (browsers stop
  // recognition after a few seconds of silence) so captions keep working
  // for the whole meeting instead of dying after the first pause.
  useEffect(() => {
    if (!captionsOn || !captionSupport.supported || !micOn) {
      captionRecognitionRef.current?.stop?.();
      captionRecognitionRef.current = null;
      if (!captionsOn) setCaptionLine("");
      return undefined;
    }
    const recognition = new captionSupport.SR();
    ...
    recognition.onresult = (event) => {
      ...
      setCaptionLine(text.trim());
    };
    recognition.onerror = (event) => {
      // "no-speech" / "aborted" fire constantly during normal silence —
      // not real errors, so only surface something for the user for
      // genuine permission/hardware problems.
      if (event.error === "not-allowed" || event.error === "service-not-allowed") {
        setMediaError(
          "Live captions need microphone access. Check your browser's site permissions.",
        );
        setCaptionsOn(false);
      }
    };
    recognition.onend = () => {
      if (!stoppedByUs && captionsOn && micOn) {
        try {
          recognition.start();
        } catch (_) {
          // already starting/stopping — next effect run will retry cleanly
        }
      }
    };
    try {
    ...
    return () => {
      ...
      if (captionRecognitionRef.current === recognition) {
        captionRecognitionRef.current = null;
      }
    };
  }, [captionsOn, captionSupport, micOn]);
    ...
    <div style={S.root} className="im-root" data-theme={theme}>
          ...
          {isCompactDevice ? (
            ...
            <div style={S.mobileStackWrap} className="im-mobile-stack">
              ...
              {captionsOn && (
                <div style={S.captionsBar}>
                  <Captions size={13} />
                  <span>{captionLine || "Listening for your voice…"}</span>
                </div>
              )}
            </div>
          ) : gridMode ? (
              ...
              <div style={S.settingsRow}>
                <span>
                  ...
                  {!captionSupport.supported && (
                    <div style={{ fontSize: 11, color: "var(--im-text-mute)", fontWeight: 400, marginTop: 2 }}>
                      {captionSupport.message}
                    </div>
                  )}
                </span>
                <button
                  style={{
                    ...S.settingsToggle,
                    ...(captionsOn ? S.settingsToggleOn : {}),
                    ...(captionSupport.supported ? {} : { opacity: 0.5, cursor: "not-allowed" }),
                  }}
                  disabled={!captionSupport.supported}
                  onClick={() => {
                    if (!captionSupport.supported) return;
                    setCaptionsOn((v) => !v);
                  }}
                >
                  {captionsOn ? "On" : "Off"}
      ...
      >
        ...
        <Btn
          icon={
            screenBusy ? (
              <Loader2 size={18} className="im-spin" />
            ) : screenOn ? (
              <MonitorOff size={18} />
            ) : (
              <MonitorUp size={18} />
          ...
          label="Present"
          active={screenOn}
          // FIX (Bug — Screen Share on Phone): the button used to be fully
          // `disabled` when the platform lacked getDisplayMedia support.
          // A disabled <button> never fires onClick at all, and its
          // `title` tooltip never shows on a touch tap — so on phones
          // where support detection said "unsupported" (or hadn't been
          // re-checked), tapping the icon did visibly nothing, which is
          // exactly the bug reported. The button is now ALWAYS clickable;
          // toggleScreen() itself decides what happens and always surfaces
          // a real on-screen message (mediaError banner) so a tap never
          // goes silent.
          disabled={screenBusy}
          title={
            !screenOn && !screenShareSupport.supported
